...
Cloudflare + GitHub + Slack + Paperless + Stirling + Tika + Gotenberg**). Single-provider **`cloudflare`** and other presets are
...
2. **`CLAWQL_PROVIDER`** — if it names a **merged preset** (`google`, `default-multi-provider`, `all-providers`, `atlassian`, …), load that bundle.
3. **`CLAWQL_GOOGLE_CLOUD_SPECS`** or **`CLAWQL_GOOGLE_TOP50_SPECS`** (`1` / `true` / `yes`) — merge the **bundled Google Cloud API set** (same as `CLAWQL_PROVIDER=google`) if #1–2 did not already select multi-spec. The `TOP50` name is legacy only.
4. **Built-in default merge** — only if **`CLAWQL_SPEC_PATHS`**, **`CLAWQL_SPEC_PATH`**, **`CLAWQL_SPEC_URL`**, **`CLAWQL_DISCOVERY_URL`**, **`CLAWQL_PROVIDER`**, **`CLAWQL_GOOGLE_CLOUD_SPECS`**, and **`CLAWQL_GOOGLE_TOP50_SPECS`** are all unset: **Google Cloud (bundled) + Cloudflare + GitHub + Slack + Paperless + Stirling + Tika + Gotenberg** (`default-multi-provider`).
...
| `CLAWQL_GOOGLE_CLOUD_SPECS` / `CLAWQL_GOOGLE_TOP50_SPECS`                  | Merge bundled **Google Cloud** Discovery set (`1`/`true`/`yes`). Prefer **`CLAWQL_GOOGLE_CLOUD_SPECS`**; **`TOP50`** is a legacy alias.                                                                                                                                                                                                                                             |
...
**GCP multi-service:** use **`CLAWQL_GOOGLE_CLOUD_SPECS=1`** (or legacy **`CLAWQL_GOOGLE_TOP50_SPECS=1`**), **`CLAWQL_PROVIDER=google`**, or **`CLAWQL_SPEC_PATHS`** so `search` / `execute` see every merged Google Cloud API in one server; `execute` uses REST in that mode. For **every bundled vendor** (Google Cloud bundle + Jira, Bitbucket, Cloudflare, GitHub, Slack, Sentry, n8n, Paperless, Stirling, Tika, Gotenberg, …), use **`CLAWQL_PROVIDER=all-providers`**. The **default** merge (no spec env) is already **Google Cloud (bundled) + Cloudflare + GitHub + Slack + the document stack** (Paperless, Stirling, Tika, Gotenberg). See [`docs/workflow-gcp-multi-service.md`](docs/workflow-gcp-multi-service.md).  
...
The default **first-run** bundle is **Stage 1 #4** above (**Google Cloud (bundled) + Cloudflare + GitHub + Slack + Paperless + Stirling + Tika + Gotenberg**). For **one JSON blob** covering every merged vendor at once, set **`CLAWQL_PROVIDER_AUTH_JSON`** (keys = `specLabel` values from **`search`** / **`execute`**). Otherwise set per-vendor env vars (**`CLAWQL_GOOGLE_ACCESS_TOKEN`**, **`CLAWQL_CLOUDFLARE_API_TOKEN`**, **`CLAWQL_GITHUB_TOKEN`**, **`SLACK_BOT_TOKEN`**, **`SENTRY_AUTH_TOKEN`**, **`N8N_API_KEY`**, **`CLAWQL_BEARER_TOKEN`** (scoped), **`PAPERLESS_API_TOKEN`**, **`STIRLING_API_KEY`**, …) — see **`src/auth-headers.ts`**. To use another bundled spec or merged preset, set **`CLAWQL_PROVIDER`** — merged **`google`** or **`atlassian`**, or single vendors like
**`cloudflare`**, **`github`**, **`slack`**, **`sentry`**, or **`n8n`**
