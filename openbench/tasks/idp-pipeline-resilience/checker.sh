#!/usr/bin/env bash
# Grades idp-pipeline-resilience (B-2.2): stubbed IDP + Onyx outage recovery.
set -euo pipefail
...
ok_recovered = recovered is True or str(recovered).lower() in ("true", "1", "yes")
fail_stage = str(d.get("failure_stage") or d.get("failureStage") or "").strip()
...
ok_src = "idp" in src or "resilience" in src or "pipeline" in src
ok = (
    corr == "idp-resilience-openbench-01"
    and deal == "deal-idp-resilience-01"
    and ok_dry
    and ok_recovered
    and fail_stage in ("onyx_cite", "onyx", "recover_onyx")
    and onyx == "quartz-21"
...
if not ok:
    print(f"FAIL: expected IDP resilience artifact; got {d!r}", flush=True)
...
# ouroboros-on must invoke the evolutionary loop (recovery evidence).
if [ "$REQUIRE_OUROBOROS" = "1" ]; then
  if [ ! -f .openbench_agent.log ]; then
    echo "FAIL: missing .openbench_agent.log for ouroboros evidence" >&2
    cap_fail=1
  elif ! grep -Eq 'ouroboros_run_evolutionary_loop|clawql_ouroboros_run_evolutionary_loop' \
    .openbench_agent.log
  then
...
PY
    ...
    # Onyx live cite is intentionally unavailable — must NOT be required.
