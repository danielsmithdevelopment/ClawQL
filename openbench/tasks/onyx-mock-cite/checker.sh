#!/usr/bin/env bash
# Grades onyx-mock-cite: knowledge_search_onyx evidence + quartz-21 citation.
set -euo pipefail

REQUIRE_ONYX="${OPENBENCH_REQUIRE_ONYX:-0}"
...
if [ ! -f citations.json ]; then
  echo "FAIL: citations.json missing" >&2
  echo "SCORE: 0.0"
  exit 1
fi
...
try:
    d = json.loads(Path("citations.json").read_text(encoding="utf-8"))
except Exception as exc:
    print(f"FAIL: citations.json parse error: {exc}", flush=True)
    raise SystemExit(1)
code = str(d.get("code") or d.get("CLAWQL_ONYX_CODE") or "").strip()
src = str(d.get("source") or "").strip().lower()
if code == "zinc-00":
    print("FAIL: used decoy code", flush=True)
    raise SystemExit(1)
ok = code == "${EXPECTED}" and ("onyx" in src or "knowledge" in src)
if not ok:
    print(f"FAIL: expected code=${EXPECTED} source~onyx; got {d!r}", flush=True)
...
if [ "$REQUIRE_ONYX" = "1" ]; then
  if [ ! -f .openbench_agent.log ]; then
    echo "FAIL: missing .openbench_agent.log for onyx evidence" >&2
    cap_fail=1
  else
    ...
    if ! python3 "$helper" .openbench_agent.log 'clawql_knowledge_search_onyx|knowledge_search_onyx'; then
      echo "FAIL: required real knowledge_search_onyx tool_use" >&2
      cap_fail=1
    fi
  fi
fi
