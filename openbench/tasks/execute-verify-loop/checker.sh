...
if [ "$REQUIRE_EXECUTE" = "1" ]; then
  ...
  exec_hits="$(printf '%s' "$log" | grep -Eci 'clawql_execute|"execute"|tool.*execute' || true)"
  if [ "${exec_hits:-0}" -lt 2 ]; then
    echo "FAIL: clawql-on required ≥2 execute tool calls in the agent log (got ${exec_hits:-0})" >&2
    cap_fail=1
  fi
  if ! printf '%s' "$log" | grep -Eqi 'dry[_]?run'; then
    echo "FAIL: clawql-on required dry_run evidence in the agent log" >&2
    cap_fail=1
  fi
fi
