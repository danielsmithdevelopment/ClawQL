...
- **Frozen v0.6 routing-fresh confirmation suite** — `fast-decision-held-out-v0.6-routing-fresh` (n=75); digests in `FREEZE-v0.6-routing-fresh.md`. Score-once GHA 36199128704 + frontier 36199128630: Decide @ τ=0.80 → 37/39 among fires (≈82.7% LB / cite ≈83%), fire rate 52%; 2 above-τ search↔execute/hunt misses (not twins; escalation never sees them). Live cite remains twin-aware v0.5 rematch ≈92%; ≈83% is not the docs.clawql.com number. Closeout `DECIDE_V06_CONFIRMATION_CLOSEOUT.md`.
- **Decide live cutover (v0.6 spend)** — declared mcp↔skill twin equivalence (`candidate-equivalence.ts`); ship rule spend (equal fire-errors → prefer higher coverage); twin-aware rematch of v0.5 GHA dumps → both reject arms 0 errors, Decide 45/75 vs stock 32/75. Live defaults: model `fastino/GLiNER2.5-Decide`, T=0.75, routing τ=0.80. Docs: `DECIDE_V06_CUTOVER.md`, `CATALOG_TWIN_EQUIVALENCE.md`, `V06_SHIP_RULE_SPEND.md`.
- **Stock vs Decide four-arm compare** — `fast-decision-stock-vs-decide.yml` + `scripts/compare-stock-vs-decide-v04.mts` (marker `.run-stock-vs-decide`). Arms: stock/Decide forced EM + stock locked (T=4,τ=0.70) + Decide (T,τ) refit on same fit set; frozen v0.4 eval; reports fire rate, errors among fired, CP LB. Does not rewrite v0.4 closeout. Decide model: `fastino/GLiNER2.5-Decide`.
- **Fast Decision held-out v0.2 (Harvey LAB)** — `fast-decision-held-out-v0.2-harvey` suite (24 cases) mined from R2 `clawql-openbench-traces` nemotron-clawql transcripts (tasks 001/002/004/008/012/018). Covers tool routing, skill fast-path, doc type, ontology flags, field mapping, Pattern E, and pre-compaction cache. Provisional GT from successful vs failed Lab tool sequences; `adjudicated=false` until frontier labels. Select via `scripts/run-held-out-adjudication.mts --suite v0.2-harvey`. Provenance: `packages/clawql-core/src/classifier/held-out/fixtures/HARVEY_V02_PROVENANCE.md`.
- **Frontier GHA suite select** — `fast-decision-frontier-adjudication.yml` accepts `suite` (`v0.1` | `v0.2-harvey` | `v0.3-routing-fresh` | `v0.4-routing-fresh`) via `workflow_dispatch` / `repository_dispatch` `client_payload.suite`; daily schedule adjudicates v0.1 + v0.2-harvey. Artifacts named `held-out-frontier-adjudication-<suite>`. Fetch/rescore: `SUITE=v0.4-routing-fresh RESCORE=1 bash scripts/fetch-frontier-adjudication-artifact.sh`.
- **v0.4 frontier productionTrusted** — GHA run 36144911649 (PR #1147 marker): live Sonnet 4.6 + gliner2 on `fast-decision-held-out-v0.4-routing-fresh` → acc 0.80 / MCE 0.118 / `productionTrusted=true`; frontier GT ≡ provisional 40/40; @τ=0.70 cov 0.45, 18/18 among-accepted (n=40; 95% LB ≈82%). **v0.4 spent**; next retune starts at v0.5. Summaries under `fixtures/frontier-runs/`.
- **Frontier judge candidateId policy** — cosmetic remaps only (quote/case), logged `before→after` in run summary (`candidateIdRemaps`); semantic label/suffix remaps fail the case instead of silently re-pointing the judge answer.
- **Frontier Harvey-style marker PR** — path-filtered `pull_request`/`push` on `packages/clawql-core/src/classifier/held-out/.run-frontier-adjudication` (first non-comment line = suite). Same-repo PR runs live judge without agent `actions:write`; remove marker after labels land.
- **Ontology-enriched Fast Decision scoring** — catalog-generated ontology + optional `ToolRoutingHint`; held-out packs into GLiNER when **`CLAWQL_FAST_DECISION_ONTOLOGY=1`** (default **off** after clean v0.3). Harvey A/B = **contaminated-smoke**; clean v0.3 = **no evidence of benefit** (see `ONTOLOGY_ENRICHMENT_EVAL_LOG.md`). Spec: [`capability-ontology-from-catalog-v0.1.md`](docs/specs/classifier/capability-ontology-from-catalog-v0.1.md).
- **Capability ontology from live catalog (spec)** — draft v0.1: optional `whenToUse` / `whenNotToUse` / `distinguishFrom` on `ToolDefinition`/`SkillDefinition` (parameterNotes pattern); build-time ontology + digest; hand overrides audited; eval records digest.
- **Fresh routing held-out v0.3** — `fast-decision-held-out-v0.3-routing-fresh` (34 cases) isolated catalog draft; frozen digests in `FREEZE-v0.3-routing-fresh.md`. **Spent for tuning** after clean dual-arm.
- **v0.3 dual-arm readout** — pre-registered then both arms; ontology-on 0.794→0.706 provisional GT (point estimate negative, not significant; sibling/shell flat). Not productionTrusted.
- **Catalog-generated capability ontology (types + generator + per-ID enrichment)** — `ToolRoutingHint` on `McpToolDefinition`/`SkillDefinition`; `generate-capability-ontology` from frozen catalog; exact-id enrichment never packs `requiresStructuredCorpus` / `STRUCTURED_CORPUS_PREFERRED`; eval summary records `ontologyDigest`.
...
- **§7 frontier-judge HTTP sidecar + optional GHA** — `scripts/frontier-judge-http-server.mts` (Anthropic Messages / OpenRouter) implementing `CLAWQL_FAST_DECISION_JUDGE_URL`; workflow `.github/workflows/fast-decision-frontier-adjudication.yml` (workflow_dispatch / `repository_dispatch`, credential-gated like Harvey LAB).
- **Post-#1119 gap close** — MCP capability gate **default-on** (`CLAWQL_CAPABILITY_LIFECYCLE=0` opt-out) with lazy session-catalog bootstrap (`catalog-bootstrap.ts`, ATR tokens / `CLAWQL_CAPABILITY_SESSION_SEED` / Core default seed ∪ process-registered MCP tools via `noteProcessRegisteredCapabilityTools`); §7 `FrontierAdjudicator` dry-run + HTTP judge port + `scripts/run-held-out-adjudication.mts`; GLiNER sidecar `gliner2` Docker target + compose `--profile live` + `scripts/gliner-sidecar-smoke.sh`; `BurstWatchStub` / `BurstWatchLoop` controller stack + placement variance sim; §13 `loadtest/dry-run.mjs` (null `$Y`, `status: dry-run`); Istio denial NDJSON adapter + Karpenter lifecycle façade + celld fleet-health lease evaluator → watch queue / WORM types.
...
- **Fast Decision Primitive (leg 5 / 8.0.0)** — calibrated closed-category classifier in [`packages/clawql-core/src/classifier/`](packages/clawql-core/src/classifier/) (`clawql-core/classifier`): open use-site registry (9 built-ins), **GLiNER2/2.5 primary** scorer (`GlinerFastDecisionScorerLive` / `FastDecisionGlinerStackLive`; live HTTP via `CLAWQL_FAST_DECISION_GLINER_URL`, else honest `gliner2-stub`), Needle optional secondary stub, heuristic for tests, skill fast-path with per-call validity read, SGDOP peer prefilter, blocking `pre-compaction` hook + append-only stable cache block, §7 correctness/calibration harness, WORM entry types, optional `ProviderPlugin.preferredVocabulary`, ontology vocabulary-match glue. Full consolidated spec v0.4 (GLiNER primary, LensVLM §6.7a documented-not-adopted, §2.8 scope note excluding GUI-control models such as Cua-S1-4B from the candidate table, open registry): [`docs/specs/classifier/fast-decision-primitive-v0.4.md`](docs/specs/classifier/fast-decision-primitive-v0.4.md).
- **Unified Capability Lifecycle v0.2** — three-bucket `execute()` reachability (§3.5) in [`packages/clawql-core/src/capability-lifecycle/`](packages/clawql-core/src/capability-lifecycle/) plus MCP wiring (**default-on**; opt out `CLAWQL_CAPABILITY_LIFECYCLE=0` → [`capability-lifecycle-plugin`](packages/clawql-api/src/plugins/capability-lifecycle-plugin.ts) on `McpProxyPipeline`). Session catalog + rebind + lazy ATR/seed bootstrap, accept-time `validatedScope ⊆ S`, `CAPABILITY_WRITE_INTERCEPTED` / `SLOW_PATH_COMPLETED_NO_NEW_CAPABILITY` WORM, register-side port (§3.5.1 open). Spec: [`docs/specs/classifier/unified-capability-lifecycle-v0.2.md`](docs/specs/classifier/unified-capability-lifecycle-v0.2.md).
- **Bursty Streams on AWS** (draft) — [`docs/streams/aws-celld-burst.md`](docs/streams/aws-celld-burst.md) (`/streams/aws-celld-burst`): celld + Karpenter + Istio ambient under hard-zero bursts; Mechanism A vs B; §2.1 regulated IDP scenario; §13 three-arm load-test design (celld vs Karpenter scale-to-zero vs warm-pool); Modal-sourced §8.2 session-aware cell placement (thicc-session split / load-aware / new-capacity preferred) + WORM types; filler PriorityClasses; `clawql-k8s-operator` Effect scaffold (distinct from `clawql-operator`); rate-card projections only until §12/§13 measured. Manifests + k6 scaffold: [`infra/aws-celld-burst/`](infra/aws-celld-burst/).
...
- **clawql.com marketing** — flattened Tailwind Plus `landing-page/demo/` into [`apps/www/`](apps/www/); unused Oatmeal `pages/` + sibling `components/` copies removed.
- **docs.clawql.com** — Next + OpenNext app moved `website/` → [`apps/docs/`](apps/docs/). Markdown sources stay in [`docs/`](docs/); ops runbooks stay in [`docs/website/`](docs/website/). Worker name `clawql-docs` and image `clawql-website` are unchanged.
- **Eval code** — Harvey LAB, ExtractBench, Agents OpenBench, harness-compare, and MCP OpenBench tasks live under [`benchmarks/`](benchmarks/) (was `benches/` + repo-root `openbench/`). Specs stay in [`docs/benchmarks/`](docs/benchmarks/); executor-comparison scripts stay in [`scripts/benchmarks/`](scripts/benchmarks/). Repo-root `integrations/` is gitignored for local-only experiments (not [`docs/integrations/`](docs/integrations/)).
- **Security curriculum sources** — 30-module bodies + build scripts moved `tools/security-curriculum-v30/` → [`docs/security/security-curriculum-v30/`](docs/security/security-curriculum-v30/). Published modules stay in [`docs/security/security-best-practices-series/`](docs/security/security-best-practices-series/). Docs site `/tools` is still the MCP tool catalog (`apps/docs/src/app/tools/`).
- **Ontology entity JSON Schema** — canonical copy is [`packages/clawql-ontology/schemas/ontology/entity.schema.json`](packages/clawql-ontology/schemas/ontology/entity.schema.json) (npm-shipped). Duplicate repo-root `schemas/` removed. Example entities stay in [`docs/examples/ontology/`](docs/examples/ontology/). The `$id` URI `https://clawql.dev/schemas/ontology/entity.schema.json` is unchanged.
- **Sample stacks live under docs** — repo-root `examples/` moved to [`docs/examples/`](docs/examples/) (Compose, IDP sidecars, ontology `.cqe`, Streams celld). These are documentation samples, not a product surface. Docs site [`/examples`](https://docs.clawql.com/examples) remains case-study walkthroughs. CLI fallback for entities is `docs/examples/ontology/entities`.
- **npm is the lockfile** — removed leftover root `bun.lock`. CI is `npm ci` / `package-lock.json`. `yaml@1.10.3` pin stays in npm `overrides`.
- **Brand logo** — dropped unused repo-root `ClawQL-logo.jpeg` (same bytes as `apps/www/public/`, `apps/docs/public/`, `apps/dashboard/public/`).
- **Cluster install artifacts** — Helm charts live at [`manifests/charts/`](manifests/charts/) (Istio-style); Kustomize overlays at [`manifests/kustomize/`](manifests/kustomize/). Packer templates moved to [`infra/packer/`](infra/packer/) (`scripts/packer/` bake/boot scripts unchanged). Install: `helm upgrade --install clawql ./manifests/charts/clawql-mcp`. Argo CD `source.path` for the IDP umbrella is `manifests/charts/clawql-idp`.
...
- **Payments plan entitlements** — every plan sets `mcp_executions_unlimited: true`; inference caps remain separate from MCP search/execute (GTM unlimited-executions promise).
...
- **Hosted live IaC (Pulumi profiles + Argo CD GitOps)** — extend [`infra/pulumi`](infra/pulumi) with `edge` (Cloudflare R2/KV/D1/Queues + optional Worker stub), `idp-k3s` (r7i.2xlarge K3s bootstrap), and `eks` (EKS + Karpenter IAM) profiles. Expand [`infra/gitops`](infra/gitops) (AppProject, app-of-apps, workflows + Karpenter Applications). Add kinetic [`.cqw`](infra/gitops/workflows/) packs synced by Argo CD into Argo Workflows. Operator guide: [`docs/deployment/hosted-live-bootstrap.md`](docs/deployment/hosted-live-bootstrap.md). ADR 0007 amended.
...
- **IDP NATS agent bridge (#128)** — second JetStream durable `clawql-idp-agent-bridge` on `clawql.document.>` maps `pipeline.completed` / `pipeline.failed` / `coneshare.viewer` into ClawQL MCP `memory_ingest` (+ optional `notify` / `audit`). Runtime-agnostic for Hermes / Pi / OpenClaw. CLI: `npm run nats:agent-bridge`. Helm: `nats.agentBridge.enabled`. Samples: [`docs/examples/idp/idp-nats-agent/`](docs/examples/idp/idp-nats-agent/). Runbook: [`docs/runbooks/idp-nats-agent-bridge.md`](docs/runbooks/idp-nats-agent-bridge.md).

## [7.2.0] - 2026-08-04

Minor release on the **7.0 Agentic Gateway** line: **Memory Stack 2.0**, Convergence Week **MCP 2026-07-28 / OKF v0.2 / PorTAL**, native **CodeGraph**, **mcp-api-adapter**, Managed Edge Gateway wedge, Cloud Agent / team-sync hardening, and a completed **IDP** document-event wave (including Stirling + NATS). No intentional semver-major breaks vs **7.1.0** — see behavioral notes in release notes. Release notes: **[`RELEASE_NOTES_v7.2.0.md`](docs/release/RELEASE_NOTES_v7.2.0.md)**. Announcement drafts: **[`docs/announcements/announcement-drafts-v7.2.0.md`](docs/announcements/announcement-drafts-v7.2.0.md)**. Inventory: **48** product/deps PRs merged after `v7.1.0` (excluding the 7.1.0 prep PR itself).
...
- **Classifier train/promote BYO** ([#810](https://github.com/danielsmithdevelopment/ClawQL/pull/810)) — [`promote.sh`](docs/examples/idp/classifier-http/promote.sh); Helm pins image tag / `MODEL_VERSION` / optional model PVC.
- **NATS IDP e2e enablement** ([#812](https://github.com/danielsmithdevelopment/ClawQL/pull/812)) — Helm [`values-nats-idp.example.yaml`](manifests/charts/clawql-mcp/values-nats-idp.example.yaml); KEDA lag triggers; smoke `scripts/dev/smoke-nats-idp-webhooks.sh`; runbook [`nats-idp-e2e.md`](docs/runbooks/nats-idp-e2e.md); Agent contract [`clawql-agent-idp-nats.md`](docs/openclaw/clawql-agent-idp-nats.md) ([#128](https://github.com/danielsmithdevelopment/ClawQL/issues/128)).
...
- **Workspace + Helm** — all `clawql-*` packages and chart `appVersion` → **7.2.0** (`manifests/charts/clawql-mcp` **0.7.2**, operator **0.2.2**, idp **0.1.2**).
...
- **Workspace + Helm** — all `clawql-*` packages and chart `appVersion` → **7.1.0** (`manifests/charts/clawql-mcp` **0.7.1**).
...
Major release: **ClawQL provides the Agentic Gateway as the Foundational Platform for Auditable Production AI** — opinionated default stack everywhere (npm + Helm), **vault-first defense in depth**, **ClawQL Operator** with provider-secret reconciliation, **npm package separation at 7.0.0**, observability ADR 0005, and consolidated IDP wave. Release notes: **[`RELEASE_NOTES_v7.0.0.md`](docs/release/RELEASE_NOTES_v7.0.0.md)**. Announcement drafts: **[`docs/announcements/announcement-drafts-v7.0.0.md`](docs/announcements/announcement-drafts-v7.0.0.md)**. Fabric architecture: **[`docs/architecture/zero-trust-agentic-fabric.md`](docs/architecture/zero-trust-agentic-fabric.md)**.

### Breaking

- **Helm default provider = npm default stack:** **`manifests/charts/clawql-mcp`** and **`values-docker-desktop.yaml`** now set **`provider: default`** (Cloudflare, GitHub, Slack, Linear, Notion, Onyx). Use **`provider: all-providers`** or **`helm --set provider=all-providers`** for the full IDP + every bundled vendor merge.
...
- **ClawQL Desktop (macOS, Windows, Linux):** Electron app (`apps/desktop/`) bundling the dashboard — local **Provider secrets** (`~/.ClawQL/vault/providers.json`), **Agent Chat** via OpenClaw bridge, and **Custom sources** UI; `npm run dist:mac|dist:win|dist:linux`. Design: [`docs/design/clawql-desktop-macos.md`](docs/design/clawql-desktop-macos.md).
...
- **Phase 1 exit (7.0.0 finalized):**
  ...
  - **Tier 1 Docker Compose** — `docs/examples/clawql-local-docker-compose` (bootstrap, MCP + Tika + Gotenberg + Paperless + memory); `make compose-tier1-config-test`.
...
Patch release: fixes **broken npm and Docker distribution** for **`clawql-mcp@6.4.0`** — fresh **`npm install`** and **`docker pull`** failed without cloning and building from source. **`manifests/charts/clawql-mcp`** **Chart.version** **0.6.8** with **`appVersion` `6.4.1`**. Release notes: **[`RELEASE_NOTES_v6.4.1.md`](docs/release/RELEASE_NOTES_v6.4.1.md)**.
...
Minor release: **plugin Phase 2 (MCP registration)** for Memory, Documents, Automation (Argo **`workflow`** + **`argocd`**), Sandbox (Kata in-cluster), and Ouroboros; **eight-vendor IDP** merge (Docling, Nextcloud, ConeShare); full **Argo Workflows** control plane (wait, suspend/resume, cron, artifacts, notify-on-wait); **`loadSpec()` coalesce** for test stability. **No intentional MCP contract breaks** — same tool names and env gates; default **`all-providers`** merge is larger when documents are on. **`manifests/charts/clawql-mcp`** **Chart.version** **0.6.7** with **`appVersion` `6.4.0`**. Release notes: **[`RELEASE_NOTES_v6.4.0.md`](docs/release/RELEASE_NOTES_v6.4.0.md)**.
...
- **Lending W-2 sample pack** ([#253](https://github.com/danielsmithdevelopment/ClawQL/issues/253)): Argo **`WorkflowTemplate`**, Label Studio config, OpenClaw prompt, synthetic fixture under **`docs/examples/idp/lending-w2/`**.
- **Ouroboros plugin glue** moved to **`packages/clawql-ouroboros/src/glue/`**; Postgres pool shutdown owned by plugin **`onTeardown`**.
- **CI / smoke:** Helm template assertions for workflow + Argo CD RBAC ([#456](https://github.com/danielsmithdevelopment/ClawQL/pull/456)); optional **kind** + Argo Workflows integration script ([#459](https://github.com/danielsmithdevelopment/ClawQL/pull/459)); Argo smoke runbook [`infra/gitops/argo-workflows/SMOKE.md`](infra/gitops/argo-workflows/SMOKE.md).
...
- **`docs/providers/docling-onboarding.md`**, **`docs/runbooks/fine-tuned-classifier.md`**, **`docs/examples/idp/lending-w2/`** ([#248](https://github.com/danielsmithdevelopment/ClawQL/issues/248), [#253](https://github.com/danielsmithdevelopment/ClawQL/issues/253)); IDP matrix refresh ([#254](https://github.com/danielsmithdevelopment/ClawQL/issues/254) partial).
- IDP Platform + DAOS v2.7 documentation suite ([#445](https://github.com/danielsmithdevelopment/ClawQL/pull/445)).

## [6.3.0] - 2026-06-02

Minor release: **monorepo modularization phases 2–9** — workspace packages **`clawql-core`**, **`clawql-api`**, **`clawql-memory`**, **`clawql-documents`**, **`clawql-automation`** with thin **`src/`** shims; **`search`/`execute`** on Effect-TS via **`createClawQLApi()`**; **`PanguardProxyPlugin`**; plugin model + registry docs and **`/reference/plugins`** on docs.clawql.com. **No intentional MCP tool or env-flag breaks** — same **`search`**, **`execute`**, **`memory_*`**, optional tools behind existing **`CLAWQL_ENABLE_*`** gates. **`manifests/charts/clawql-mcp`** **Chart.version** **0.6.6** with **`appVersion` `6.3.0`**. Release notes: **[`RELEASE_NOTES_v6.3.0.md`](docs/release/RELEASE_NOTES_v6.3.0.md)**.
...
- **Supply chain (OSV):** bump **`brace-expansion`** override to **5.0.6** and **`ws`** to **8.20.1** (root + apps/docs/dashboard overrides).
...
Minor release: **MCP gRPC** defaults for large tool payloads (**`mcp-grpc-transport` `0.2.0`** — default **64 MiB** gRPC send/receive limits, **`callToolServerStreamingGrpc`** + helpers), **Helm** **`enableGrpc`** / **`grpcMaxMessageLength`**, REST **`execute`** query handling and HTTP body limits, and docs-site **Next.js 16.2.6** supply-chain alignment. **`manifests/charts/clawql-mcp`** **Chart.version** **0.6.4** with **`appVersion` `6.2.0`**. Release notes: **[`RELEASE_NOTES_v6.2.0.md`](docs/release/RELEASE_NOTES_v6.2.0.md)**.
...
- **Helm `vault` vs secrets manager ([#161](https://github.com/danielsmithdevelopment/ClawQL/issues/161)):** Chart header + Obsidian **`vault`** comments, **`deployment`** template note, **`manifests/charts/clawql-mcp/README.md`**, **`docs/deployment/helm.md`**, security docs — distinguish Obsidian memory mounts from **HashiCorp Vault**.
...
- **Helm / Kustomize — Prometheus scrape `clawql-mcp-http` `/metrics`:** **`metrics.prometheusScrapeAnnotations`** on **`manifests/charts/clawql-mcp`** (default **enabled** — **`prometheus.io/*`** on the MCP **Service** for Istio sample Prometheus **`kubernetes-service-endpoints`**); optional **`metrics.serviceMonitor`** (**`monitoring.coreos.com/v1`**) for Prometheus Operator; **`service.annotations`** merge; **`manifests/kustomize/base/service-mcp-http.yaml`** annotations for parity. Docs: **`manifests/charts/clawql-mcp/README.md`**, **`docs/deployment/helm.md`**, **`docs/grafana/README.md`** ([#210](https://github.com/danielsmithdevelopment/ClawQL/issues/210)).
...
- **`manifests/charts/clawql-mcp`:** value keys **`enableAudit`** and **`enableCache`** removed (Core tools are not configurable via Helm); **`appVersion`** **`5.0.0`** with **`clawql-mcp`** major (Helm **`Chart.version`** **0.5.x**—see **`manifests/charts/clawql-mcp/Chart.yaml`**).
...
- **`manifests/charts/clawql-mcp` (0.5.3):** **`kyverno.imageSignaturePolicy.enabled`** defaults to **`true`** — the chart renders a **`ClusterPolicy`** unless disabled. Install **[Kyverno](https://kyverno.io/)** before **`helm upgrade`**, or pass **`--set kyverno.imageSignaturePolicy.enabled=false`** on clusters that do not use Kyverno yet.
...
- **`manifests/charts/clawql-mcp`:** **`docs/deployment/helm.md`** and **`manifests/charts/clawql-mcp/README.md`** aligned with Core tools and optional flags.
...
- **`manifests/charts/clawql-mcp` (0.5.2–0.5.3):** Kyverno **`ClusterPolicy`** (**`templates/kyverno-clusterpolicy-cosign.yaml`**); **`enabled`** default **true** as of **0.5.3**; **`matchReleaseNamespaceOnly`** for Docker Desktop (**`values-docker-desktop.yaml`**). **`scripts/kubernetes/local-k8s-docker-desktop.sh`** installs **Kyverno** (Helm chart pin **`CLAWQL_KYVERNO_CHART_VERSION`**, default **3.7.2**), rejects unsigned local image env vars, pulls signed **`ghcr.io/.../clawql-website`**, and applies the policy on the Kustomize path via **`helm template --show-only`**. **`make helm-lint`** templates with policy on and off.
...
- **`manifests/charts/clawql-mcp`:** **`enableOnyx`** / **`onyxBaseUrl`** ([#118](https://github.com/danielsmithdevelopment/ClawQL/issues/118)); **`enableOuroboros`** / **`ouroborosDatabaseUrl`** ([#141](https://github.com/danielsmithdevelopment/ClawQL/issues/141), [#142](https://github.com/danielsmithdevelopment/ClawQL/issues/142)). Chart **0.4.0** (**`appVersion` 4.1.0**). **`docs/deployment/helm.md`** and **`manifests/charts/clawql-mcp/README.md`** updated.
...
- **Enterprise `audit` tool ([#89](https://github.com/danielsmithdevelopment/ClawQL/issues/89)):** optional MCP tool when **`CLAWQL_ENABLE_AUDIT=1`** — in-process ring buffer (`append` / `list` / `clear`); not on disk. Design: **[docs/mcp/enterprise-mcp-tools.md](docs/mcp/enterprise-mcp-tools.md)**. **Helm:** **`manifests/charts/clawql-mcp`** adds **`enableAudit`** (default **`false`**) → **`CLAWQL_ENABLE_AUDIT=1`**.
...
- **Helm chart** **`manifests/charts/clawql-mcp`**: **`enableCache`** defaults to **`true`** (sets **`CLAWQL_ENABLE_CACHE=1`**) so the in-process **`cache`** tool is registered unless **`--set enableCache=false`**.
...
- **`make local-k8s-up`:** defaults to **Helm** (**`manifests/charts/clawql-mcp/values-docker-desktop.yaml`**). **`CLAWQL_LOCAL_K8S_INSTALLER=kustomize`** uses **`manifests/kustomize/overlays/local`** (no Helm). **`vault.hostPath`** (Helm) / JSON patch (Kustomize) mount **`~/.ClawQL`**. **`CLAWQL_LOCAL_K8S_BUILD_IMAGE=1`** builds **`clawql-mcp:latest`** locally.
...
- **Developer tooling:** root **`npm run format`** / **`format:check`** now includes the docs site: **`npm run format --prefix apps/docs`** (Prettier on **`apps/docs/`** `mdx`/`ts`/`tsx`). **`.prettierignore`** no longer skips all of **`apps/docs/`**—only build artifacts (**`.next/`**, **`node_modules/`**, etc.). CI runs **`npm ci --prefix apps/docs`** before **`format:check`** so site Prettier plugins resolve.
...
- **Helm chart** **`manifests/charts/clawql-mcp`**: deploy **`clawql-mcp-http`** with configurable image (**GHCR** by default), Service (**LoadBalancer** / **ClusterIP**), optional **Ingress**, **`/vault`** via PVC or **`vault.hostPath`**, gRPC env toggles; **`values-docker-desktop.yaml`** for **`make local-k8s-up`**; **`make helm-lint`**. Docs: **`docs/deployment/helm.md`**; site: **`/helm`**.
...
- **Documentation site (`apps/docs/`):** **`sitemap.xml`**, **`robots.txt`**, canonical site URL helper (**`NEXT_PUBLIC_SITE_URL`** / **`VERCEL_URL`**), richer page metadata for SEO, **gRPC and Kubernetes** reference card.
...
- **Kustomize:** **`manifests/kustomize/overlays/grpc-enabled/`** sets **`ENABLE_GRPC=1`** and uses **Kubernetes `grpc`** readiness/liveness probes on port **50051**.
...
- **ClawQL documentation site** under `apps/docs/` (branding, deployment notes).
