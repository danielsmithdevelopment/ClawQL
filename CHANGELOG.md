...
- **Post-#1119 gap close** — MCP capability gate **default-on** (`CLAWQL_CAPABILITY_LIFECYCLE=0` opt-out) with lazy session-catalog bootstrap (`catalog-bootstrap.ts`, ATR tokens / `CLAWQL_CAPABILITY_SESSION_SEED` / Core default seed); §7 `FrontierAdjudicator` dry-run + HTTP judge port; GLiNER sidecar `gliner2` Docker target + compose `--profile live`; `BurstWatchStub` in-memory watches; §13 `loadtest/dry-run.mjs` (null `$Y`, `status: dry-run`).
