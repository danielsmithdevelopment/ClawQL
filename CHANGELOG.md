...
- **BREAKING (8.0.0 — loudest):** Bundled OpenAPI provider catalog and enforcement default **off** — bare install is a search/execute surface with no prescribed providers and no tool-scope enforcement until operators opt in. Empty catalog: `providers.pack` / `CLAWQL_PROVIDER=default`. Enforcement: install a blocking provider (e.g. `CLAWQL_PANGUARD_PROXY_PLUGIN=1` + `CLAWQL_PANGUARD_IN_PROCESS=1`). Boot warns if none active unless `CLAWQL_ALLOW_NO_ENFORCEMENT=1`.
- **BREAKING (8.0.0 — plugin interface):** Phase-2 `Plugin` / `onRegister` / `beforeCallTool` and `legacyPluginToProviderPlugin` are **removed**. Install only `ProviderPlugin` / `StandaloneSkillPlugin`. Rewrite out-of-tree plugins against the new shape — no soft landing. See [`docs/getting-started/migrate-to-8.0.md`](docs/getting-started/migrate-to-8.0.md).
...
- **`clawql-core` ProviderPlugin architecture (8.0)** — `ProviderPlugin` / `StandaloneSkillPlugin`, lifecycle `fireHook` (ATR never-loosen + `HOOK_SCOPE_VIOLATION_BLOCKED` / `HOOK_TRIGGERED`), two-tier skill index/fetch, Effect install/uninstall, dynamic `loadPluginModuleEffect`, reference `PanguardProviderPlugin`. **No** legacy `Plugin` bridge. Spec: [`docs/design/clawql-core-plugin-architecture.md`](docs/design/clawql-core-plugin-architecture.md). Action items: [`docs/design/clawql-8.0-plugin-architecture-action-items.md`](docs/design/clawql-8.0-plugin-architecture-action-items.md).
...
- **Process WORM host integration** — dual-write to `clawql-audit` when `CLAWQL_WORM_ENABLED=1` from MCP tools (`src/mcp-tool-wrap.ts`), issued API keys (`src/gateway-auth.ts`), inference gateway (`ObservedInferenceGateway`), Panguard allow/deny, execute, memory, auth OAuth, and payments.
