...
- **`clawql-core` ProviderPlugin architecture (8.0 foundation)** — `ProviderPlugin` / `StandaloneSkillPlugin`, lifecycle `fireHook` (ATR never-loosen + `HOOK_SCOPE_VIOLATION_BLOCKED` / `HOOK_TRIGGERED`), two-tier skill index/fetch, Effect install/uninstall, dynamic `loadPluginModuleEffect`, legacy `Plugin`→`ProviderPlugin` bridge, reference `PanguardProviderPlugin`. Spec: [`docs/design/clawql-core-plugin-architecture.md`](docs/design/clawql-core-plugin-architecture.md). Action items: [`docs/design/clawql-8.0-plugin-architecture-action-items.md`](docs/design/clawql-8.0-plugin-architecture-action-items.md).
