...
- **Bundled HashiCorp Vault + default guard:** Chart depends on **`hashicorp/vault`** (`hashicorpVault.*`); **`secretSourcing.requireVaultBackedSecrets=true`** requires **`envFromSecret`** or **`envFromSecrets`** unless overridden (desktop overlay disables both for labs).
