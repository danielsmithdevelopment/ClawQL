...
- **Helm — mandatory HashiCorp Vault dependency:** **`charts/clawql-mcp`** always installs the **`hashicorp/vault`** subchart (dependency **`condition` removed**). **`hashicorpvault.enabled: false`** and **`secretSourcing.requireVaultBackedSecrets: false`** are **rejected** at template time (**`templates/zzz-defense-in-depth-secrets-policies.yaml`**). **`values-docker-desktop.yaml`** matches production posture (**`envFromSecret: clawql-provider-env`**, **`requireVaultBackedSecrets: true`**); Obsidian **`vault.hostPath`** defaults **off** in the file so **`helm template`** passes — **`local-k8s-docker-desktop.sh`** enables hostPath with **`$HOME/.ClawQL`**. Chart **0.6.2** (Helm dependency alias **`hashicorpvault`**, lowercase, so **`ServiceAccount`/StatefulSet** names satisfy RFC 1123; Docker Desktop **`server.dev`** + **`make bootstrap-vault-eso`** for ESO-aligned **`Secret/clawql-provider-env`**).
- **Secret scanning:** mandatory **Gitleaks** job **`secret-scan`** in **`.github/workflows/ci.yml`**, **`.pre-commit-config.yaml`**, and **`.gitleaks.toml`**; **`.github/workflows/trufflehog-scheduled.yml`** for git history (**`providers/`** excluded). **Harbor** / private-registry consumption guidance in **`docs/security/golden-image-pipeline.md`** and **`docs/security/README.md`** ([#283](https://github.com/danielsmithdevelopment/ClawQL/issues/283)).
...
- **Bundled HashiCorp Vault + guard:** Chart always depends on **`hashicorp/vault`** (`hashicorpvault.*`); **`secretSourcing.requireVaultBackedSecrets=true`** is mandatory (**`envFromSecret`** / **`envFromSecrets`** required); **`hashicorpvault.enabled: false`** is unsupported.
...
- **`values-docker-desktop.yaml`:** Bundled Vault with lighter **`hashicorpvault.server.resources`**; **`secretSourcing.requireVaultBackedSecrets: true`** and **`envFromSecret: clawql-provider-env`**; Obsidian **`vault.hostPath`** off in values (script enables **`$HOME/.ClawQL`** on install).
