...
- **Frontier GHA suite select** — `fast-decision-frontier-adjudication.yml` accepts `suite` (`v0.1` | `v0.2-harvey`) via `workflow_dispatch` / `repository_dispatch` `client_payload.suite`; daily schedule adjudicates both. Artifacts named `held-out-frontier-adjudication-<suite>`. Fetch/rescore: `SUITE=v0.2-harvey RESCORE=1 bash scripts/fetch-frontier-adjudication-artifact.sh`.
...
- **Ontology-enriched Fast Decision scoring** — ClawQL capability ontology (`clawql-capability-ontology.json`) maps MCP tools/skills/anti-patterns with whenToUse/whenNotToUse; held-out + GLiNER classify pack ontology into `text`/`labels` (opt out: `CLAWQL_FAST_DECISION_ONTOLOGY=0`). The 2026-09-25 Harvey routing A/B is tagged **contaminated-smoke** (not §7 held-out lift); see `ONTOLOGY_ENRICHMENT_EVAL_LOG.md`. Next: catalog-generated ontology — [`docs/specs/classifier/capability-ontology-from-catalog-v0.1.md`](docs/specs/classifier/capability-ontology-from-catalog-v0.1.md).
...
- **Fresh routing held-out v0.3** — `fast-decision-held-out-v0.3-routing-fresh` (34 cases) drafted by an isolated model session from tool catalog only; frozen with SHA-256 digests in `FREEZE-v0.3-routing-fresh.md` before any hint text. Select via `--suite v0.3-routing-fresh`. Do not score until hints exist; then both arms same `ontologyDigest`.
- **v0.3 dual-arm readout** — pre-registered metrics then scored both arms (`scripts/score-v03-routing-fresh-dual-arm.mts`); ontology-on accuracy 0.794→0.706 on provisional GT (not productionTrusted). See `ONTOLOGY_ENRICHMENT_EVAL_LOG.md`.
