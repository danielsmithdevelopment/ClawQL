...
- **Effect v4 Stage 0 tooling ([#1037](https://github.com/danielsmithdevelopment/ClawQL/pull/1037))** — inventory + RC dry-run CI (no runtime Effect 4 bump in 8.0).
- **Docs site polish ([#1039](https://github.com/danielsmithdevelopment/ClawQL/pull/1039), [#1040](https://github.com/danielsmithdevelopment/ClawQL/pull/1040))** — mobile docs padding; promote off-sidebar pages + archive redirects.
- **Security status page ([#1033](https://github.com/danielsmithdevelopment/ClawQL/pull/1033))** — verifiable `/security/status`.
- **Streams celld Lab 5 / 5b ([#1041](https://github.com/danielsmithdevelopment/ClawQL/pull/1041)–[#1047](https://github.com/danielsmithdevelopment/ClawQL/pull/1047))** — celld **v0.4.0** Learn baseline; Helm celld backend; embed **`clawql-core/streams-slim`**; `fetch(CLAWQL_MCP_URL)` for `search` / `execute` / `memory_*`; `fetch(CLAWQL_MCP_ADAPTER_URL)` REST for mcp-api-adapter; isolate audit ring → DO LTX (`audit:ring` / `audit:seq:*`). Example: [`examples/streams-celld`](examples/streams-celld/). Spec: [`docs/streams/clawql-celld.md`](docs/streams/clawql-celld.md). Still deferred: offline Workers-safe `clawql-api` slim.
