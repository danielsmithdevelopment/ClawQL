...
Scheduling, notifications, and Argo Workflows extracted from `clawql-mcp` (modularization phase 9, [#430](https://github.com/danielsmithdevelopment/ClawQL/pull/430)): persisted synthetic checks (`schedule` MCP tool), Slack `notify` via the loaded spec's `chat_postMessage` operation, and optional **`workflow`** (Argo Workflows control plane).

## Plugin entry

When `CLAWQL_ENABLE_SCHEDULE=1`, `CLAWQL_ENABLE_NOTIFY=1`, and/or `CLAWQL_ENABLE_WORKFLOW=1`, **`AutomationPlugin`** (`createAutomationPlugin` from `clawql-automation/plugin`) registers MCP tools via `onRegister`, starts the schedule worker when schedule is enabled, and stops it on `onTeardown`. Composed from `buildMcpPlugins()` in `src/clawql-api-adapters.ts`.
...
Prefer subpath imports (`clawql-automation/schedule/schedule`, `clawql-automation/notify/notify`, `clawql-automation/workflow/workflow`) for direct handler access in tests and transport glue.
...
| NATS event bus for workflow orchestration | **Planned**                                                                        | [#127](https://github.com/danielsmithdevelopment/ClawQL/issues/127)                                                                                    |
