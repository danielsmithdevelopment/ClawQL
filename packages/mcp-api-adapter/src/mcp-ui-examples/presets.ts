  ...
  Effect.gen(function* () {
    ...
    if (steps.length < 2) {
      return yield* new McpUiPresetError({
        reason:
          "Agent Lab preset needs ≥2 matching tools. Point the adapter at docs/examples/mcp-api-adapter/docs-agent-lab-server.mjs (docs_*) or a Core catalog with search + memory_recall.",
      });
    }
    return {
