...
describe("warnIfMcpOAuthHs256Only", () => {
  ...
  it("does not warn when MCP OAuth is fully disabled", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    Effect.runSync(warnIfMcpOAuthHs256Only({}));
    expect(warn).not.toHaveBeenCalled();
    warn.mockRestore();
  });
});
...
describe("warnIfMcpOAuthAdminKeyMissing", () => {
  it("warns when MCP OAuth is on without CLAWQL_API_KEY", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    Effect.runSync(warnIfMcpOAuthAdminKeyMissing({}, { mcpOAuthEnabled: true }));
    expect(warn).toHaveBeenCalledWith(MCP_OAUTH_ADMIN_KEY_MISSING_WARNING);
    warn.mockRestore();
  });

  it("does not warn when CLAWQL_API_KEY is set", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    Effect.runSync(
      warnIfMcpOAuthAdminKeyMissing(
        { CLAWQL_API_KEY: "admin" },
        { mcpOAuthEnabled: true, idJagIssuerEnabled: true }
      )
    );
    expect(warn).not.toHaveBeenCalled();
    warn.mockRestore();
...
describe("warnIfMcpOAuthBootstrapInvalid", () => {
  it("warns with source detail", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    Effect.runSync(warnIfMcpOAuthBootstrapInvalid("CLAWQL_EMA_ORGS_JSON", new Error("bad json")));
    expect(warn).toHaveBeenCalled();
    expect(String(warn.mock.calls[0]?.[0])).toContain(MCP_OAUTH_BOOTSTRAP_INVALID_WARNING);
    expect(String(warn.mock.calls[0]?.[0])).toContain("CLAWQL_EMA_ORGS_JSON");
    warn.mockRestore();
  });
});
