...
export function isIdJagIssuerEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
 ...
 */
export async function createIdJagIssuerFromEnv(options: {
  ...
  const signing: McpOAuthSigningMaterial = await Effect.runPromise(
    loadMcpOAuthSigningMaterialEffect({
      signingSecret:
        env.CLAWQL_ID_JAG_ISSUER_SIGNING_SECRET?.trim() ||
