...
export async function enrichLabMemoryRecall(
  ...
  const guidance: Record<string, unknown> = {
    ...
    matterIds: sortedIds,
    evidenceRule:
      "HARD: For each HSR matter, cite a path from that hit's preferredEvidence (or SQL hsr_second_request_proof_doc). Read that file before writing. Do not substitute other second-request*.docx names from a different matter. Acceptable stems: joint-status-report, case-assessment-memo, letter-ftc-meet-and-confer, second-request-strategy-memo, hsr-withdrawal-letter, substantial-compliance-certification, custodian-identification-collection-protocol. Do not cite engagement letters as Second Request evidence.",
    contextDiscipline:
