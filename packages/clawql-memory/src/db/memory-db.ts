...
function extractTitle(markdown: string): string {
  ...
  if (body.startsWith("---\n")) {
    ...
    if (end !== -1) {
      ...
      const title = firstH1Title(rest);
      if (title) return title;
    }
  }
  const title = firstH1Title(body);
  if (title) return title;
