...
Prefer subpath imports (`clawql-memory/vault/config`, `clawql-memory/ingest/...`, `clawql-memory/recall/...`) at server startup to avoid loading the full barrel. MCP wrappers (`logMcpToolShape`) remain in `src/mcp/tools.ts`.
