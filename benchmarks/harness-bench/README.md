...
node docs/examples/mcp-api-adapter/server.mjs   # or your adapter URL

# Side-by-side compressed vs fat — exits non-zero if demos regress
TRACE_BASE=http://127.0.0.1:8090/mcp-ui node benchmarks/harness-bench/scripts/fetch-trace.mjs --compare
...
node benchmarks/harness-bench/scripts/compare.mjs
```

This is distinct from **Agents OpenBench** (`benchmarks/agents-bench/`) and **MCP OpenBench** (`benchmarks/openbench/`).
