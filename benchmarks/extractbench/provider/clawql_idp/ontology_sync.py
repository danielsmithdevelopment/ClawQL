...
Calls ``benchmarks/extractbench/scripts/run-ontology-pipeline.mjs`` which
...
def clawql_repo_root() -> Path:
    ...
    for _ in range(10):
        script = cur / "benchmarks/extractbench/scripts/run-ontology-pipeline.mjs"
        if script.is_file():
            return cur
        if cur.parent == cur:
            break
        cur = cur.parent
    # Fallback when provider is copied into ExtractBench (benchmarks/ not under cwd).
    return Path(__file__).resolve().parents[4]


def ontology_pipeline_script() -> Path:
    return clawql_repo_root() / "benchmarks/extractbench/scripts/run-ontology-pipeline.mjs"
