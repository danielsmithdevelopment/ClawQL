...
STACK_VERSION="$(node "${WT}/benchmarks/harvey-labs/scripts/lab-stack-version.mjs" | python3 -c 'import json,sys; print(json.load(sys.stdin)["stack_version"])')"
LOG="${LOG:-/tmp/harvey-lab-contiguous-001-025-${STACK_VERSION}.log}"
AGG="${AGG:-${WT}/benchmarks/harvey-labs/results/ts-v2/aggregate-contiguous-001-025.json}"
...
export CLAWQL_LAB_PREINGEST_SCRIPT="${WT}/benchmarks/harvey-labs/scripts/lab-pre-ingest.mjs"
export CLAWQL_LAB_MCP_PROXY="${WT}/benchmarks/harvey-labs/scripts/lab-mcp-proxy.mjs"
...
python3 "${WT}/benchmarks/harvey-labs/scripts/apply_clawql_adapter.py" \
...

{
  ...
  for i in $(seq 1 25); do
    ...
    set +e
    bash "${WT}/benchmarks/harvey-labs/scripts/run-lab-local.sh"
    ec=$?
    set -e
