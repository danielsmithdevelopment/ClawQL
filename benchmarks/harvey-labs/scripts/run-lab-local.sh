...
export CLAWQL_LAB_STACK_VERSION="${CLAWQL_LAB_STACK_VERSION:-$(node "${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/lab-stack-version.mjs" | python3 -c 'import json,sys; print(json.load(sys.stdin)["stack_version"])')}"
export CLAWQL_LAB_PREINGEST_SCRIPT="${CLAWQL_LAB_PREINGEST_SCRIPT:-${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/lab-pre-ingest.mjs}"
export CLAWQL_LAB_MCP_PROXY="${CLAWQL_LAB_MCP_PROXY:-${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/lab-mcp-proxy.mjs}"
...
RESULTS_OUT="${CLAWQL_ROOT}/benchmarks/harvey-labs/results"
mkdir -p "${RESULTS_OUT}"
...
if [[ "${CLAWQL_LAB_PODMAN_VIA_DOCKER:-0}" == "1" ]]; then
  SHIM_DIR="${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/podman-docker-shim"
...
if ! curl -fsS -m 5 "${AGENT_BASE%/v1}/v1/models" >/dev/null 2>&1 && \
  ...
  echo "  Start: bash benchmarks/harvey-labs/scripts/start-clawql-inference-for-lab.sh 8091 ${LAB_RUN_ID}" >&2
...
start_lab_idp_sidecars() {
  ...
  if ! curl -fsS -m 2 "http://127.0.0.1:8090/health" >/dev/null 2>&1; then
    nohup env LANGEXTRACT_MODE=demo PORT=8090 \
      python3 "${CLAWQL_ROOT}/docs/examples/idp/langextract-http/server.py" \
...
python3 "${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/apply_clawql_adapter.py" \
...
ensure_clawql_mcp() {
  ...
  export CLAWQL_LAB_PREINGEST_SCRIPT="${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/lab-pre-ingest.mjs"
  export CLAWQL_LAB_MCP_PROXY="${CLAWQL_ROOT}/benchmarks/harvey-labs/scripts/lab-mcp-proxy.mjs"
