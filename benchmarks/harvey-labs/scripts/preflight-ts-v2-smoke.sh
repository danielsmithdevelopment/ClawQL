...
for s in lab-pre-ingest.mjs lab-mcp-proxy.mjs lab-mcp-client.mjs lab-vault-seed.mjs; do
  p="${ROOT}/benchmarks/harvey-labs/scripts/${s}"
  if [[ -f "$p" ]]; then
...
STACK="$(node "${ROOT}/benchmarks/harvey-labs/scripts/lab-stack-version.mjs" | python3 -c 'import json,sys; print(json.load(sys.stdin)["stack_version"])')"
...
echo "Next: bash benchmarks/harvey-labs/scripts/quarantine-legacy-call-store.sh"
echo "Then: LAB_TASK=firm-knowledge/tasks/001 LAB_ARMS=nemotron-clawql bash benchmarks/harvey-labs/scripts/run-lab-local.sh"
