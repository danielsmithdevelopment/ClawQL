...
export CLAWQL_LAB_STACK_VERSION="${CLAWQL_LAB_STACK_VERSION:-$(node "${ROOT}/benchmarks/harvey-labs/scripts/lab-stack-version.mjs" | python3 -c 'import json,sys; print(json.load(sys.stdin)["stack_version"])')}"
...
elif [[ -f "$ROOT/docs/examples/inference/policy.yaml" ]]; then
  cp "$ROOT/docs/examples/inference/policy.yaml" "$HOME_DIR/Inference/policy.yaml"
