...
cp "$ROOT/benchmarks/openbench/adapters/clawql.py" "$OPENBENCH_ROOT/obench/adapters/clawql.py"

TASK_SRC="$ROOT/benchmarks/openbench/tasks/$TASK"
if [ ! -d "$TASK_SRC" ]; then
  echo "Unknown ClawQL task: $TASK" >&2
  echo "Available:" >&2
  ls "$ROOT/benchmarks/openbench/tasks" >&2
  exit 2
fi
