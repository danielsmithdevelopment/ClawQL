#!/usr/bin/env python3
"""Resolve OpenBench CI task matrix from benchmarks/openbench/ci-matrix.json.

Usage:
  python3 benchmarks/openbench/scripts/resolve-ci-matrix.py --mode pr
  python3 benchmarks/openbench/scripts/resolve-ci-matrix.py --mode dispatch --task all
  python3 benchmarks/openbench/scripts/resolve-ci-matrix.py --mode dispatch --task cache-scratch-handoff
  python3 benchmarks/openbench/scripts/resolve-ci-matrix.py --mode dispatch --task all-including-retired
...
MATRIX_PATH = ROOT / "benchmarks" / "openbench" / "ci-matrix.json"
TASKS_DIR = ROOT / "benchmarks" / "openbench" / "tasks"
...
def resolve(mode: str, task: str | None) -> list[str]:
    ...
    if mode == "pr":
        if not live_enabled:
            reason = cfg.get("live_pause_reason") or "live_enabled=false in benchmarks/openbench/ci-matrix.json"
