#!/usr/bin/env bash
# Grades memory-wikilink-hop: memory_recall evidence + opal-33 via wikilink.
set -euo pipefail

REQUIRE_WIKI="${OPENBENCH_REQUIRE_WIKILINK:-0}"
...
if token == "zinc-00":
    print("FAIL: used decoy token", flush=True)
    raise SystemExit(1)
...
if [ "$REQUIRE_WIKI" = "1" ]; then
  if [ ! -f .openbench_agent.log ]; then
    echo "FAIL: missing .openbench_agent.log for wikilink evidence" >&2
    cap_fail=1
  else
