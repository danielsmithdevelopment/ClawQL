...
For when **`cache` is** vs is **not** the right tool, see **[`docs/mcp/cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/cache-tool.md)**. **`cache`** is **in-process** and **LRU-bounded** — it is the right place for “temporary issue numbers and information we do **not** want to persist to long-term memory” for as long as the MCP server process (and the session) lasts.
...
**Important:** `cache` is **ephemeral in this MCP server process** (see [`docs/mcp/cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/cache-tool.md)). A restart, a different process, or another client does not see these keys. That is the intended behavior: **scratch**, not a substitute for **`memory_ingest`**.
...
**Full deck (this site):** [/vision/slide-deck](/vision/slide-deck) — same Markdown as the canonical file below; that route **inlines the deck in its static HTML** at build time (large route bundle, crawler-friendly).
