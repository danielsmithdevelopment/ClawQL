...
export const sections = caseStudySlideDeckGithubParitySections

<CaseStudyStructuredData
  headline="Case study: slide deck vs GitHub — memory_recall, cache, and parity issues"
  path="/case-studies/slide-deck-github-parity-cache-memory-recall-2026-04"
  description="How memory_recall grounded a gap pass before new issues, cache() held ephemeral scratch, and memory_ingest closed the loop — same detail as docs/case_studies in the repo."
/>

<Note>
  **Source:** This page and
  [`docs/case_studies/slide-deck-github-parity-cache-memory-recall-2026-04.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/case_studies/slide-deck-github-parity-cache-memory-recall-2026-04.md)
...
This case study walks through a **Cursor** session that aligned the **ClawQL** pitch deck in [`docs/clawql-slides.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-slides.md) with the **GitHub** backlog. The goal was to **close narrative gaps** by creating tracking issues for themes that appeared in the **80-slide** deck (including **§08** security) but had **no** or **incomplete** issue coverage.
...
For when **`cache` is** vs is **not** the right tool, see **[`docs/cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/cache-tool.md)**. **`cache`** is **in-process** and **LRU-bounded** — it is the right place for “temporary issue numbers and information we do **not** want to persist to long-term memory” for as long as the MCP server process (and the session) lasts.

---

## 2. The slide deck as product contract

- **Source:** [`docs/clawql-slides.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-slides.md) — **80** slides, sections **§01–§08** (Core through Defense in Depth), plus a closing slide.
- **Paired long-form security reference:** [`docs/clawql-security-defense-in-depth.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-security-defense-in-depth.md) (aligns with **§08**).
...
**Parameters used (illustrative):** `query` = short keyword phrase, **`limit`** small (e.g. 5–8), optional **`maxDepth`** when graph follow-through matters (wikilinks). See [`docs/mcp-tools.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp-tools.md) under **`memory_recall`**.
...
**Important:** `cache` is **ephemeral in this MCP server process** (see [`docs/cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/cache-tool.md)). A restart, a different process, or another client does not see these keys. That is the intended behavior: **scratch**, not a substitute for **`memory_ingest`**.
...
| **#163** | **Transcript** parity: [`clawql-slides-transcript.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-slides-transcript.md) vs **80** slides + **§08** |
...
| **`audit`** | In-process **operator** trail of MCP tool calls (optional) | Not the focus; see [`docs/enterprise-mcp-tools.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/enterprise-mcp-tools.md) and [#89](https://github.com/danielsmithdevelopment/ClawQL/issues/89). |
...
- [`docs/clawql-slides.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-slides.md) — canonical deck
- [`docs/clawql-security-defense-in-depth.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/clawql-security-defense-in-depth.md) — **§08** long-form
- [`docs/cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/cache-tool.md) — **cache** tool semantics
- [`docs/mcp-tools.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp-tools.md) — **memory_ingest**, **memory_recall**
...
**Readable layout (this site):** [docs.clawql.com/case-studies/slide-deck-github-parity-cache-memory-recall-2026-04](https://docs.clawql.com/case-studies/slide-deck-github-parity-cache-memory-recall-2026-04) — keep in **sync** with the repo `docs/case_studies/` file above.
