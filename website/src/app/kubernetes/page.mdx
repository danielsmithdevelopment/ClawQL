...
- **Namespace:** `clawql`
- **`CLAWQL_PROVIDER`:** **`default-multi-provider`** (Google Cloud bundled APIs + Cloudflare + GitHub + Slack + Paperless + Stirling + Tika + Gotenberg, same as bare `npx clawql-mcp`). Override via **`values-docker-desktop.yaml`** or **`helm --set provider=...`**.
