...
**Team sync (optional):** pushes and pulls the **`Memory/`** tree via object storage — see [Team vault sync](/getting-started/team-vault-sync). `memory.db` is rebuilt locally after pull. Code graph storage is local unless you sync **`codegraph.db.json`** yourself.
