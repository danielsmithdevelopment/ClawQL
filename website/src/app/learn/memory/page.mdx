**Canonical repo docs:** [memory-obsidian.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/memory-obsidian.md) · [OKF v0.2 memory guide](/memory/okf) · [MCP tools § memory](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/mcp-tools.md) · [Architecture](https://docs.clawql.com/architecture)
...
- **Ontology structured filters** — for exact field predicates (`schema` + `filters` → `ontology.db` via clawql-ontology), not semantic similarity. See [memory_recall structured filters](/specs/memory/memory-recall-structured-filter).
...
When vault notes carry machine-readable fields (for example legal `CLAWQL_*` tags from the [legal domain pack](/specs/ontology/legal-domain)), ClawQL syncs them into `ontology.db`. `memory_recall` with `schema` + `filters` evaluates typed predicates against that index — exact matches, no near-miss ranking. This is what closed OpenBench B-7 enumeration after semantic-only recall scored zero on false positives. Spec: [memory_recall structured filters](/specs/memory/memory-recall-structured-filter). Essay: [Memory Finds. Ontology Decides.](https://pragmaticvectors.com/posts/memory-finds-ontology-decides/).
...
- [Code graph plugin](https://docs.clawql.com/plugins/codegraph) — native sync, 30+ languages, explore/impact, optional Graphify import
