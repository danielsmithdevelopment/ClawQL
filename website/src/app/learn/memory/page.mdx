...
Parses source code (TypeScript compiler API for TS/JS; tree-sitter WASM for Python/Go) to produce nodes and edges (imports, calls, inheritance) with confidence tags (EXTRACTED / INFERRED / AMBIGUOUS). **Preferred ingestion** is the native TypeScript pipeline via `codegraph_sync` (index → Louvain communities → architecture report → vault ingest). Optional `codegraph_import_graphify` / `codegraph_sync_graphify` can ingest an external `graph.json` — **no Python dependency**. Exposed via `codegraph_*` tools and optional `memory_recall` `sources: ["codegraph"]`.
