...
Parses source code (TypeScript compiler API for TS/JS; tree-sitter WASM for Python/Go) to produce nodes and edges (imports, calls, inheritance) with confidence tags (EXTRACTED / INFERRED / AMBIGUOUS). Exposed via `codegraph_*` tools and optional `memory_recall` `sources: ["codegraph"]`. The deterministic structural index over source code, providing precise architecture traversal independent of the vault.
