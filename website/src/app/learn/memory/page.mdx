...
Parses source code (TypeScript compiler API for TS/JS; tree-sitter WASM for Python/Go) to produce nodes and edges (imports, calls, inheritance) with confidence tags (EXTRACTED / INFERRED / AMBIGUOUS). **Preferred ingestion** is the native TypeScript pipeline via `codegraph_sync` (index → Louvain → architecture report → vault ingest). TS/JS gets enclosing-scope call graphs, heritage/exports, cross-file linking, React/Next tags, plus **`codegraph_explore`** / **`codegraph_impact`**. Optional Graphify `graph.json` import remains — **no Python**. Exposed via `codegraph_*` and optional `memory_recall` `sources: ["codegraph"]`.
