...
<div className="not-prose my-6 rounded-2xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300">
  <strong>Not the WORM compliance trail.</strong> This page documents the MCP{' '}
  <code className="text-xs">audit</code> tool only. For tamper-evident durable logs, see{' '}
  <a href="/audit" className="font-medium underline underline-offset-2">clawql-audit</a>{' '}
  (<code className="text-xs">packages/clawql-audit</code>). Gateway operators often use both: live breadcrumbs via{' '}
  <code className="text-xs">audit</code>, long-term evidence via <code className="text-xs">clawql-audit</code>.
</div>
...
**See also:** [Tools](/tools) · [clawql-memory (Memory 2.0)](/learn/memory) · [Cache handoff between chats](/learn/cache-handoff-between-chats) · [ClawQL Learn overview](/learn)
