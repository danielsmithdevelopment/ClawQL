...
You can open the bundled docs UI at **`http://clawql.localhost`** and point MCP clients at **`http://clawql-mcp.localhost/mcp`** (**Ingress** — same hostname + **`/mcp`** pattern as production). The script installs **Kyverno** and only admits **Cosign-signed** ClawQL images from **GHCR** — see [Security](/security) and the repo doc **[golden-image-pipeline.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/security/golden-image-pipeline.md)**.
