...
**Learn walkthrough** (Nextcloud → pdf-inspector → Docling → Tika → Gotenberg → Stirling → Paperless → Onyx → Coneshare): [Document pipeline](/learn/document-pipeline) · [IDP pipeline hub (repo)](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/providers/idp-pipeline.md).

## Canonical REST-forward matrix (document stack + SaaS defaults)

Use this as the default-bundle reference for **REST / OpenAPI–heavy** workflows. The table includes the **eight-vendor IDP stack** (plus in-process **pdf-inspector** via MCP `inspect_pdf`) and common SaaS presets; **`google`** is a merged Discovery preset, not a single file.
