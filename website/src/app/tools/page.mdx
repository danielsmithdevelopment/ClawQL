...
ClawQL registers **six** tools by default (**`search`**, **`execute`**, **`sandbox_exec`**, **`memory_ingest`**, **`memory_recall`**, **`ingest_external_knowledge`**). A **seventh** optional tool, **`cache`**, is added when **`CLAWQL_ENABLE_CACHE`** is set — ephemeral **in-process LRU** storage, not the vault ([Session cache](/cache), [docs](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/cache-tool.md)). The **data plane** for REST/Discovery APIs is **`search`** then **`execute`**. Agents call **`search`** to discover operations and parameters, then **`execute`** to run a chosen **`operationId`** with optional field selection. [Canonical reference with examples](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp-tools.md). {{ className: 'lead' }}
