...
- **Namespace:** `clawql`
- **`CLAWQL_PROVIDER`:** **`all-providers`** in **`values-docker-desktop.yaml`** (full IDP + every bundled vendor + Google + AWS). Bare **`npx clawql-mcp`** without spec env uses the **opinionated default stack** instead — see [Bundled specs](/bundled-specs). Override via **`values-docker-desktop.yaml`** or **`helm --set provider=...`**.
