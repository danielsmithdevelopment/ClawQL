...
**Kustomize:** `CLAWQL_LOCAL_K8S_INSTALLER=kustomize make local-k8s-up` runs `kubectl apply -k docker/kustomize/overlays/local` for the MCP workload; Helm still installs Kyverno and applies the ClusterPolicy template from the chart.
...
- **Namespace:** `clawql`
- **`providers.pack` / `CLAWQL_PROVIDER`:** chart default is **`none`** (empty catalog). Docker Desktop overlay may still leave `provider: ""`. Opt in with `helm --set providers.pack=default` (curated six) or `providers.pack=all-providers` for full IDP + every bundled vendor + Google + AWS — see [Bundled specs](/bundled-specs) and [Migrate to 8.0](/getting-started/migrate-to-8.0).
