...
Production-style setups run **two services**: the **MCP HTTP** server (`clawql-mcp-http`) and the **GraphQL proxy** (`clawql-graphql`). Wire them on the same network and point **`GRAPHQL_URL`** at the proxy from the MCP service. {{ className: 'lead' }}
