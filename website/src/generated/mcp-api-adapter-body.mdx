...
Together with ClawQL Core (APIs → MCP), this is the **[Protocol Fabric](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/protocol-fabric.md)** — MCP as the common IR in both directions.
...
Related: [Custom sources](/getting-started/custom-sources) registers upstream MCP servers **into** ClawQL. `mcp-api-adapter` exposes an MCP server **outward** to non-MCP clients.
...
WebSocket was deferred on classic always-on servers (session hold + reconnect cost). With **Durable Objects** (hibernation on WebSocket), it becomes the natural persistent session transport for DO-hosted adapters: bidirectional progress / notifications for long tools, and DO sleep between calls. Document Streamable HTTP `/mcp` as the fallback for clients that cannot do WebSocket. **gen-cli** stays build-time (writes to disk) — not a DO runtime surface. See [ClawQL Streams](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/clawql-streams.md) for WebSocket as an **event source** into Core (inverse concern).
...
| Piece                                                      | Role                                                   |
| ---------------------------------------------------------- | ------------------------------------------------------ |
| **[Protocol Fabric](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/protocol-fabric.md)**           | Named claim for Core + adapter (any protocol ↔ any)    |
...
| **[ClawQL Streams](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/clawql-streams.md)** (draft)  | Event-driven agents; WebSocket sources into Core       |
...
- Protocol Fabric: [`docs/gtm/protocol-fabric.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/protocol-fabric.md)
- ClawQL Streams (draft): [`docs/design/clawql-streams.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/clawql-streams.md)
