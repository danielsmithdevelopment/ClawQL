...
**Package:** [`mcp-api-adapter`](https://github.com/danielsmithdevelopment/ClawQL/tree/main/packages/mcp-api-adapter) (in-repo; first npm publish targets `0.1.0` with 8.0 workspace release)  
**Status:** Shipped in-repo · **npm not published yet** (`npm view mcp-api-adapter` → 404)  
**Essay:** [Eight surfaces, one catalog](https://pragmaticvectors.com/posts/mcp-api-adapter/) (draft: [`docs/gtm/pragmaticvectors/mcp-api-adapter.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/pragmaticvectors/mcp-api-adapter.md))  
...
**Example:** [`examples/mcp-api-adapter/`](https://github.com/danielsmithdevelopment/ClawQL/tree/main/examples/mcp-api-adapter)  
**Protocol Fabric:** [`protocol-fabric.md`](/mcp/protocol-fabric) (proven WS → CLI → REST → vault loop)
...
| #   | Surface                                                    | Role                                       |
| --- | ---------------------------------------------------------- | ------------------------------------------ |
| 8   | [QR stream transport](/streams/clawql-qr-stream-transport) | Optical air-gap MCP + Streams event source |
...
> **Install note:** The package lives under [`packages/mcp-api-adapter`](https://github.com/danielsmithdevelopment/ClawQL/tree/main/packages/mcp-api-adapter) and is listed in `localPackExtras` for the npm publish workflow, but the name is not on the registry yet — bare `npx mcp-api-adapter` fails until the first publish. Use the **from-source** commands below until then.
...
Together with ClawQL Core (APIs → MCP), this is the **[Protocol Fabric](/mcp/protocol-fabric)** — MCP as the common IR in both directions. ClawQL Core turns any API into MCP; `/mcp-ui` then makes that catalog human-navigable in a browser.
...
[Custom sources](/getting-started/custom-sources) registers upstream MCP servers **into** ClawQL. `mcp-api-adapter` exposes an MCP server **outward** to non-MCP clients.
...
If the upstream is already gRPC, REST and GraphQL forward into it. If the upstream is stdio or Streamable HTTP, the adapter starts a local [`mcp-grpc-transport`](https://github.com/danielsmithdevelopment/ClawQL/tree/main/packages/mcp-grpc-transport) server that delegates into the session.
...
Protocol Fabric loop (WS → execute CLI source → gen-cli → `memory_ingest`): see [`protocol-fabric.md`](/mcp/protocol-fabric) and `scripts/dev/smoke-protocol-fabric-loop.sh`.
...
Full spec: [`mcp-ui.md`](/mcp/mcp-ui). Differentiation vs standalone MCP playgrounds: **embedded and zero-config** (ships with the adapter, like Swagger at `/docs`). With ClawQL Core ingesting any API into MCP, `/mcp-ui` becomes a browser UI for every connected source — REST, gRPC, CLI, WebSocket — without writing a frontend.

### QR stream — optical channel (planned, 8th)

Physical optical MCP / Streams channel for air-gapped deployments. Spec: [`../streams/clawql-qr-stream-transport.md`](/streams/clawql-qr-stream-transport).
...
| Piece                                                                  | Role                                                                                 |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **[Protocol Fabric](/mcp/protocol-fabric)**                            | Named claim for Core + adapter (any protocol ↔ any); proven loop                     |
| **`mcp-api-adapter`**                                                  | MCP → OpenAPI + GraphQL + `/mcp` + gRPC + `/ws` + gen-cli + `/mcp-ui` (+ QR planned) |
| **ClawQL `search` / `execute`**                                        | OpenAPI → MCP tools (inverse)                                                        |
| **[Custom sources](/getting-started/custom-sources)**                  | Register other MCP servers **into** the ClawQL gateway                               |
| **`mcp-grpc-transport`**                                               | Production TypeScript MCP gRPC transport                                             |
| **Panguard bridge**                                                    | Policy / JWT ATR in front of MCP                                                     |
| **[ClawQL Streams](/streams/clawql-streams)** (draft)                  | Event-driven agents; WebSocket / QR sources into Core                                |
| **[QR stream transport](/streams/clawql-qr-stream-transport)** (draft) | Planned **8th surface** — optical air-gap MCP + Streams source                       |
| **[`/mcp-ui`](/mcp/mcp-ui)**                                           | **7th surface** — HTMX / HATEOAS Swagger UI for MCP (shipped)                        |
...
- Essay: [Eight surfaces, one catalog](https://pragmaticvectors.com/posts/mcp-api-adapter/) · draft [`../gtm/pragmaticvectors/mcp-api-adapter.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/pragmaticvectors/mcp-api-adapter.md)
- Package README: [`packages/mcp-api-adapter/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/packages/mcp-api-adapter/README.md)
- Design & non-goals: [`docs/design/mcp-api-adapter.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/mcp-api-adapter.md)
- GTM positioning: [`docs/gtm/mcp-api-adapter-positioning.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/gtm/mcp-api-adapter-positioning.md)
- Protocol Fabric (proven loop): [`docs/mcp/protocol-fabric.md`](/mcp/protocol-fabric)
- ClawQL Streams (draft): [`docs/streams/clawql-streams.md`](/streams/clawql-streams)
- QR stream transport (draft, 8th surface): [`docs/streams/clawql-qr-stream-transport.md`](/streams/clawql-qr-stream-transport)
- `/mcp-ui` (7th surface, shipped): [`docs/mcp/mcp-ui.md`](/mcp/mcp-ui)
- Earlier post: [MCP tools as APIs](https://pragmaticvectors.com/posts/mcp-tools-as-apis/)
- gRPC transport: [`packages/mcp-grpc-transport`](https://github.com/danielsmithdevelopment/ClawQL/tree/main/packages/mcp-grpc-transport)
- Local smoke: [`scripts/dev/smoke-mcp-api-adapter.sh`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/scripts/dev/smoke-mcp-api-adapter.sh) · [`scripts/dev/smoke-protocol-fabric-loop.sh`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/scripts/dev/smoke-protocol-fabric-loop.sh)
- Protocol Fabric / event loop (draft): [`docs/streams/clawql-streams.md`](/streams/clawql-streams)
