...
_Authoritative mechanics (tool tiers, env flags, bundled spec paths): [`mcp-tools.md`](/tools), [`readme/configuration.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/readme/configuration.md), [`providers/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/providers/README.md)._
...
- MCP Server (stdio / HTTP / gRPC) + GraphQL projection; OpenAPI 3 + Swagger 2 + Google Discovery; optional **native GraphQL / gRPC** upstreams ([`adr/0002-multi-protocol-supergraph.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/adr/0002-multi-protocol-supergraph.md)); bundled **Linear** is GraphQL-only (`providers/linear/schema.graphql`)
- **Bundled vendor ids:** 13 entries in `BUNDLED_PROVIDERS` (e.g. GitHub, Cloudflare, Slack, Sentry, n8n, document stack, Onyx, **Linear**, Atlassian aliases, …) plus the **Google Cloud** manifest (`google/google-top50-apis.json`) when using `all-providers` — see [`providers/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/providers/README.md)
...
- `memory_recall` — keyword + optional vector KNN + wikilink traversal over the vault (see [`mcp-tools.md`](/tools))
...
- **Ouroboros** — evolutionary-loop library; optional MCP tools `ouroboros_*` when `CLAWQL_ENABLE_OUROBOROS=1` ([`clawql-ouroboros.md`](/ouroboros))
...
- **Hyperledger Fabric (roadmap)** — consortium provenance; not shipped as a bundled provider or Helm sub-chart in this repository yet ([issue #187](https://github.com/danielsmithdevelopment/ClawQL/issues/187), [`adr/0002-multi-protocol-supergraph.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/adr/0002-multi-protocol-supergraph.md))
...
ClawQL registers **more than ten** tools; tiers and flags are summarized in [`mcp-tools.md`](/tools) and [`readme/configuration.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/readme/configuration.md). Core pair: **`search`** + **`execute`**.
...
| `memory_ingest`             | Memory            | Writes durable Markdown under the vault; insights, receipts, `enterpriseCitations`, wikilinks ([`memory-obsidian.md`](/learn/memory)).                                                                                                                          |
| `knowledge_search_onyx`     | Knowledge         | Optional when **`CLAWQL_ENABLE_ONYX=1`** and documents stack is on — wraps Onyx `POST /search/send-search-message` ([`onyx-knowledge-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/onyx-knowledge-tool.md)).                    |
| `sandbox_exec`              | Execution         | Optional — **`CLAWQL_ENABLE_SANDBOX=1`** — bridge / Seatbelt / Docker ([`mcp-tools.md`](/tools) § **`sandbox_exec`**, [`cloudflare/sandbox-bridge/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/cloudflare/sandbox-bridge/README.md)). |
| `ingest_external_knowledge` | Knowledge         | Bulk Markdown ingest + optional URL fetch when enabled ([`external-ingest.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/external-ingest.md)).                                                                                        |
| `schedule`                  | Automation        | Optional — **`CLAWQL_ENABLE_SCHEDULE=1`** — persisted synthetic checks ([`schedule-synthetic-checks.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/schedule-synthetic-checks.md)).                                                    |
| `notify`                    | Notification      | Optional — **`CLAWQL_ENABLE_NOTIFY=1`** — Slack `chat.postMessage` wrapper ([`notify-tool.md`](/notify)).                                                                                                                                                       |
| `ouroboros_*` (×3)          | Workflow          | Optional — **`CLAWQL_ENABLE_OUROBOROS=1`** — seed, evolutionary loop, lineage ([`clawql-ouroboros.md`](/ouroboros)).                                                                                                                                            |
| `cache`                     | Core / State      | Always on — in-process **LRU** session scratch ([`cache-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/cache-tool.md)); **no** `CLAWQL_ENABLE_CACHE`.                                                                            |
| `audit`                     | Core / Compliance | Always on — in-process **ring buffer** ([`enterprise-mcp-tools.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/enterprise-mcp-tools.md)); **no** `CLAWQL_ENABLE_AUDIT`.                                                                |
...
**Configuration:** Keep **`CLAWQL_ENABLE_DOCUMENTS`** on (default); set **`CLAWQL_ENABLE_ONYX=1`**, **`ONYX_BASE_URL`**, and Bearer **`ONYX_API_TOKEN`** / **`CLAWQL_ONYX_API_TOKEN`** so **`knowledge_search_onyx`** registers ([`mcp-tools.md`](/tools)). The bundled minimal OpenAPI at `providers/onyx/openapi.yaml` covers `POST /search/send-search-message` and optional `POST /onyx-api/ingestion`. Refresh upstream specs with `npm run fetch-provider-specs` when `ONYX_BASE_URL` is set.
...
**5 — Evolve**
If any criterion fails, the loop adjusts and retries (behavior depends on orchestration — today’s **`ouroboros_*`** tools expose lineage; fully automatic Evolve across external services is **roadmap**). Retry history: Postgres when `CLAWQL_OUROBOROS_DATABASE_URL` is set**. Escalations can use **`notify`** when enabled; PagerDuty requires **your\\_\\_ bundled spec or webhook integration.

---

### Cuckoo Filters: O(1) Deduplication

Cuckoo predicates can be loaded from **`memory.db`** when **`CLAWQL_CUCKOO_*`** / sync paths are enabled ([`memory-db-schema.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/memory-db-schema.md)).
...
**Shipped today:** with **`CLAWQL_MERKLE_ENABLED=1`** and a synced **`memory.db`**, Merkle snapshots fingerprint **vault Markdown index state** and surface on **`memory_ingest`** / **`memory_recall`** / health paths ([`mcp-tools.md`](/tools), [`memory-db-schema.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/memory-db-schema.md)). Roots live in the **SQLite sidecar**, not implicitly in Postgres unless you add separate orchestration.
...
- **Roadmap:** per-step document pipeline leaves, Ouroboros phase outputs, Onyx citation attestations — see Merkle/Cuckoo issues and **[`memory-db-hybrid-implementation.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/memory-db-hybrid-implementation.md)**.

---

### notify() + Slack Integration

When **`CLAWQL_ENABLE_NOTIFY=1`**, **`notify`** wraps Slack **`chat.postMessage`** by delegating to **`execute`** on the bundled Slack spec ([`notify-tool.md`](/notify)). Authenticates like **`execute`** on **`slack`** (`CLAWQL_SLACK_TOKEN`, …; **`chat:write`** minimum).
...
| `tika`, `gotenberg`, `paperless`, `stirling`, `onyx` | `providers/<id>/openapi.yaml`                                                        | Document / knowledge stack; base URLs + tokens per [`providers/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/providers/README.md) |
...
- **`charts/clawql-mcp`** — see [`charts/clawql-mcp/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/charts/clawql-mcp/README.md) and `values.yaml` for optional document pipeline, Onyx, Flink, NATS, UI ingress, etc.
- `CLAWQL_BUNDLED_OFFLINE=1` — typical production stance so MCP does not fetch specs at runtime (see README / deployment docs)
- **Onyx stack** — gated by chart values (`onyx.enabled` pattern); MCP **`CLAWQL_ENABLE_ONYX`** aligns with [`onyx-knowledge-tool.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/onyx-knowledge-tool.md)
...
**Status:** Design and issue tracking only — **no** `providers/fabric/openapi.yaml`, **no** `CLAWQL_ENABLE_FABRIC` Helm toggle, and **no** `fabric_*` MCP tools ship in this repository today. See [#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187) and [`adr/0002-multi-protocol-supergraph.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/adr/0002-multi-protocol-supergraph.md).
...
- **`notify` templates:** richer default messages with Onyx citation links and Paperless document links (baseline **`notify`** exists behind **`CLAWQL_ENABLE_NOTIFY`** — [`notify-tool.md`](/notify))
...
- **Hybrid memory.db — extend vector sidecar:** SQLite graph + vault ship today; **`CLAWQL_VECTOR_BACKEND`** and sqlite-vec paths are partial ([`memory-db-schema.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/memory-db-schema.md), [`vector-search-design.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/memory/vector-search-design.md)). Planned: vector-ranked chunk retrieval at scale, content-hash incremental re-embedding as primary backend.
...
The body of this doc describes a **target experience**. The bullets below are **not** guaranteed by the open-source **`clawql-mcp`** repo alone — they are **aspirational**, **cross-repo**, or **partially implemented**. Use [`mcp-tools.md`](/tools) and linked issues for ground truth.

**Separate products / repos**

- **OpenClaw** — gateway / MCP host toward ClawQL: **not** shipped inside this repository; track integration via [#128](https://github.com/danielsmithdevelopment/ClawQL/issues/128). End-to-end guide: [`docs/openclaw/using-openclaw-with-clawql.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/openclaw/using-openclaw-with-clawql.md). Phase 0 MCP wiring and smokes: [`docs/openclaw/clawql-bootstrap.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/openclaw/clawql-bootstrap.md) ([#226](https://github.com/danielsmithdevelopment/ClawQL/issues/226)). Default IDP skill profile (tools + matrix): [`docs/openclaw/openclaw-idp-skill-profile.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/openclaw/openclaw-idp-skill-profile.md) ([#227](https://github.com/danielsmithdevelopment/ClawQL/issues/227)).
...
- **Argo Workflows + Argo CD (optional providers)** — agents drive **DAG pipelines** (Workflows) and optionally **GitOps sync** (CD) on Kubernetes; **proposed** post-**6.0.0**, default-off RBAC — see [**ADR 0004**](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/adr/0004-argo-cd-workflows-clawql-pipelines.md), roadmap [`argo-workflows-cd-provider.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/roadmap/argo-workflows-cd-provider.md), [#239](https://github.com/danielsmithdevelopment/ClawQL/issues/239).
