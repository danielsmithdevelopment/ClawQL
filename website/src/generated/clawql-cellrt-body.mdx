...
**Related:** [`clawql-tee.md`](/streams/clawql-tee) · [`clawql-tee-airgap-audit.md`](/streams/clawql-tee-airgap-audit) · [`clawql-durable-objects.md`](/streams/clawql-durable-objects) · [`../specs/network/clawql-network-v0.1.md`](/specs/network/clawql-network) · [`mcp-api-adapter`](/mcp/mcp-api-adapter) · [defense-in-depth](/security/defense-in-depth) · [codegraph](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/plugins/codegraph.md) · [correctness-by-construction](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/correctness-by-construction.md) (NASA/SPARK → Rust + TLA+ for key/WORM/liveness invariants)
...
cellrt as specified is **software security** (Rust, eBPF, WASM sandbox, Cosign, WORM). [`clawql-tee`](/streams/clawql-tee) adds hardware-enforced guarantees (AMD SEV-SNP / Intel TDX, attestation-gated Vault secrets, optional NVIDIA GPU CC) and [`air-gap QR audit transport`](/streams/clawql-tee-airgap-audit) so a verifier need not trust the operator or the network path.
...
| **Hardware TEE**       | No                             | Planned — [`clawql-tee`](/streams/clawql-tee) (SEV-SNP / TDX) |
| **Air-gap audit**      | No                             | Planned — [`QR transport`](/streams/clawql-tee-airgap-audit)  |
...
**TEE track (after week 4 bootstrap):** `cellrt-attestation` + Vault attestation auth → SEV-SNP demo → optional GPU CC → air-gap QR export. See [`clawql-tee.md`](/streams/clawql-tee) § build sequence.
