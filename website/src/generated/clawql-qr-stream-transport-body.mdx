...
The six shipped surfaces (OpenAPI, GraphQL, Streamable HTTP `/mcp`, gRPC, gen-cli, WebSocket) and the planned [`/mcp-ui`](/mcp/mcp-ui) playground assume a network (or a browser on one). QR stream is for when there is no network, must not be one, or the channel must be physically verifiable and unidirectional.
...
- [`docs/mcp/mcp-api-adapter.md`](/mcp/mcp-api-adapter) — six surfaces today; QR 7th · `/mcp-ui` 8th planned
- [`docs/mcp/mcp-ui.md`](/mcp/mcp-ui) — planned 8th surface (HTMX Swagger UI for MCP)
