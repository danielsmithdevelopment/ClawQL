...
**Cursor iOS:** Cloud Agents have no local **`~/.ClawQL`** on the phone — configure dashboard Secrets, stdio MCP, and end-of-session **`memory_sync`**. Full walkthrough: [Agent setup — Cursor iOS](/agent-setup#cursor-i-os-cloud-agent).
...
- [Agent setup](/agent-setup) — desktop, Cursor iOS Cloud Agents, local sandbox
