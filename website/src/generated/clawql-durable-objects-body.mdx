...
**Depends on:** [`clawql-streams`](/streams/clawql-streams) · [`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) · [`clawql-inference`](/inference/clawql-inference) · OpenBenchTrace / RTP  
...
On celld, a `GatewayDO` (or Gateway Worker) may sit in front for webhook routing and idempotent naming — see [`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) §4.
...
Append-only WORM writer. Threads **virtual key ID** through every entry (§5). Does not store PII event bodies — payload hashes only (Streams §6). On celld this is LTX-backed SQLite; see [`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) §6.
...
- Outbound source WebSockets: persist **reconnect intent** in SQLite and use **`setAlarm`** — never `setInterval` (throws on celld). Outbound sockets do not survive celld node migration — reconnect after activation ([`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) §3).
...
Parity target: **same session contract and WORM/RTP schemas**. Exact hibernation and cold-start numbers will differ — document in operator runbooks. Prefer celld over inventing a Node DO runtime; prefer K8s HPA when celld's alpha / single-app-fleet limits are unacceptable ([`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) §8, §11).
...
- [ ] celld: peers on private/WireGuard mesh; ingress TLS; bucket creds scoped ([`clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) §8)
...
- [`docs/streams/clawql-celld.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/streams/clawql-celld.md) — celld integration (constraints, LTX, deploy)
