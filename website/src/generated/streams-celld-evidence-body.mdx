...
This boots **real** `clawql-mcp-http` (vault memory + skills; empty OpenAPI catalog is OK) and **real** `mcp-api-adapter` (`--mcp-url` → that MCP), points celld at both, and asserts search / memory\_\* / adapter REST / inference health — while refusing mock `source` markers. `execute` of `streams.session.noop` may error on an empty catalog; the smoke still requires a Streamable HTTP hop.
