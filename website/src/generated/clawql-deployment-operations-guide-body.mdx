...
Operator-focused tier docs and natural-language ops tables remain in **[Operator target architecture (design)](/design/operator-target-architecture)**. An **opt-in operator scaffold** (CRD + reconcile + tier-spec ConfigMaps) is available separately — see **[clawql-operator-helm.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/deployment/clawql-operator-helm.md)** ([#255](https://github.com/danielsmithdevelopment/ClawQL/issues/255)).
...
| **Document pipeline** (Tika, Gotenberg, Stirling, Paperless) | ✅ Shipped         | [idp-pipeline.md](/learn/document-pipeline), `documentPipeline.enabled`                                                                                                                                                                                    |
...
| **`ingest_external_knowledge` + `DEFAULT_IDP_PIPELINE`**     | ✅ Shipped         | [idp-pipeline.md](/learn/document-pipeline) — agent-composed `search`/`execute`; automated runner [#307](https://github.com/danielsmithdevelopment/ClawQL/issues/307)                                                                                      |
...
| **Kubernetes Operator + `ClawQLInstance` CRD**               | 🚧 Opt-in scaffold | [clawql-operator-helm.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/deployment/clawql-operator-helm.md), [operator-target-architecture.md](/design/operator-target-architecture)                                                     |
...
Base URLs for document vendors are injected by the chart when subcharts are enabled — see [idp-pipeline.md § Helm](/learn/document-pipeline#helm-enable-the-full-stack).
...
- **`GET /metrics`** on MCP HTTP — [Grafana dashboard](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/grafana/README.md) ([#210](https://github.com/danielsmithdevelopment/ClawQL/issues/210))
...
Start with [troubleshooting](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/readme/troubleshooting.md) and [helm.md](/helm). Common checks: provider auth env, bundled spec load (`CLAWQL_PROVIDER`), Ingress host DNS, and Paperless token from Profile / `POST /api/token/` (not only chart placeholder secrets).
...
| IDP eight-vendor stack         | [idp-pipeline.md](/learn/document-pipeline) · [clawql-idp-helm.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/deployment/clawql-idp-helm.md) · [observability/README.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/observability/README.md) |
