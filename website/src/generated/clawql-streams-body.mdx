...
**Related:** [`mcp-api-adapter`](/mcp/mcp-api-adapter) · [Agentic Gateway](/inference/clawql-inference) · [Durable Objects](/streams/clawql-durable-objects) · [Ouroboros](../ouroboros/) · [OpenBenchTrace collection](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/benchmarks/openbench-trace-collection.md)
...
When the significance filter passes, ClawQL spawns an agent session. The **model layer is always [`clawql-inference`](/inference/clawql-inference)** — not a raw Anthropic/OpenAI credential. The `model` field on `stream_subscribe` is a **policy alias** that resolves through PAL routing (`Frugal` → `Standard` → `Frontier`), not a bare model string.
...
See [`clawql-durable-objects.md`](/streams/clawql-durable-objects) for the DO implementation contract.
...
4. **DO to self-hosted parity.** Some features (WebSocket hibernation, per-event DO isolation) are Cloudflare-native. The K8s path approximates them but does not have exact parity — document what differs in [`clawql-durable-objects.md`](/streams/clawql-durable-objects).
...
In OpenBench GHA, consent is often implicit (own infra); mint a gateway JWT at job start with the same scopes so datasets are commercially licensable without retroactive cleanup. See [`openbench-trace-collection.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/benchmarks/openbench-trace-collection.md#rtp-alignment).
...
- [`docs/streams/clawql-durable-objects.md`](/streams/clawql-durable-objects) — DO sidecars, virtual key bind/expire, training sidecar
...
- [`docs/benchmarks/openbench-results-ledger.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/benchmarks/openbench-results-ledger.md) — Convergence Week ledger
