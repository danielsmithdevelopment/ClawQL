...
> **8.0.0:** Phase-2 `Plugin.onRegister` / `beforeCallTool` is **removed**. Author **`ProviderPlugin`** / **`StandaloneSkillPlugin`** (`tools` / `hooks`). See [Migrate to 8.0](/getting-started/migrate-to-8.0) and [clawql-core plugin architecture](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/clawql-core-plugin-architecture.md). Sections below that still mention `onRegister` are **historical** until rewritten.
...
**Package extraction (phases 1–9) is on `main`.** Vertical contributions use the **`ProviderPlugin`** contract (`tools`, `hooks`, `requiredSpecs`) — see [Plugin model](/plugins#plugin-model), [clawql-core plugin architecture](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/clawql-core-plugin-architecture.md), and [implementation status](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/modularization-implementation-status.md). You can write and test a vertical now using the in-memory test layers described in §3.4.
...
**Horizontal packages (memory, documents, automation):** see [ClawQL plugin model](/plugins#plugin-model) for how these become plugins that register their MCP tools (`memory_ingest`, `ingest_external_knowledge`, `schedule`, etc.) and how that differs from the current `tools.ts` layout.
