...
For the full platform vision, see the [Master Enablement Guide](/vision/technical-enablement).
...
**Layer 0 today:** **`clawql-release` MVP** (manifest v0.1, Merkle root, SBOM/npm + GHCR digests, `clawql release *`, CI artifact) ships in **7.0.0**. Full permanence (Arweave, Rift, Radicle primary, Kyverno policy from manifest) remains roadmap — see [clawql-release README](https://github.com/danielsmithdevelopment/ClawQL/blob/main/packages/clawql-release/README.md) · [Immutable releases — shipped MVP](https://docs.clawql.com/vision/immutable-releases#shipped-mvp-clawql-release-in-7-0) and [Hybrid Decentralized GitHub Alternative](/vision/immutable-releases).
...
**Horizontal plugins:** memory, documents, automation, sandbox, and ouroboros **register MCP tools via `Plugin.onRegister`** when composed — see [ClawQL plugin model](/reference/plugins).
