...
For the full platform vision, see the [Master Enablement Guide](/architecture).

**Implementation status (July 2026):** [Modularization implementation status](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/modularization-implementation-status.md) — what is extracted to `packages/` today vs this document’s target boundaries.
...
**Vertical packages** (`clawql-lending`, `clawql-surveillance`, `clawql-legal`, …): 📋 Planned — none shipped. Spec for evidence integrity: [clawql-surveillance](/surveillance/clawql-surveillance).
...
- `ProviderPlugin` / `StandaloneSkillPlugin` (`tools`, `hooks` — Panguard is hooks-only and opt-in); Phase-2 `Plugin.onRegister` / `beforeCallTool` removed in 8.0.0 ([migration](/getting-started/migrate-to-8.0))
...
**Horizontal plugins:** memory, documents, automation, sandbox, and ouroboros are **`ProviderPlugin`s that declare `tools`** when composed — see [ClawQL plugin model](/plugins#plugin-model).
