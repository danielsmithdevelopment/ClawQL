...
**Companion:** [ClawQL plugin model](/reference/plugins#plugin-model) (concepts and target architecture)

This page is the **registry** of ClawQL plugins: what exists today, what horizontal packages are becoming plugins, and how to enable or compose each one. For the full explanation of “becoming a plugin” vs package extraction, read the [plugin model](/reference/plugins#plugin-model).
...
| **Vertical plugins**           | `verticals/clawql-*` (planned)       | `default`   | 📋 Not started                                    | Domain-specific                                                                                          | Operator `ClawQLInstance` CRD / tier flags                                                                  | e.g. `lending`, `legal-us` — see [Verticals guide](/reference/verticals)                                                                                 |
...
| -------------------------------- | ------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `search`, `execute`              | Gateway core              | `clawql-api` + `src/` shims | [MCP tools](/tools)                                                                                          |
| `cache`, `audit`                 | Gateway core              | `src/`                      | [MCP tools](/tools)                                                                                          |
| `memory_ingest`, `memory_recall` | `clawql-memory`           | `clawql-memory`             | [Vault memory](/learn/vault-memory-between-chats)                                                            |
| `ingest_external_knowledge`      | `clawql-documents`        | `clawql-documents`          | [external-ingest.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/external-ingest.md) |
| `knowledge_search_onyx`          | `clawql-documents` (Onyx) | `src/`                      | [Onyx knowledge](/learn/knowledge-search-onyx)                                                               |
| `schedule`                       | `clawql-automation`       | `clawql-automation`         | [Schedule](/schedule)                                                                                        |
| `notify`                         | `clawql-automation`       | `clawql-automation`         | [Notify](/notify)                                                                                            |
| `sandbox_exec`                   | `clawql-sandbox`          | `src/`                      | [Sandbox exec](/learn/sandbox-exec)                                                                          |
| `ouroboros_*`                    | `clawql-ouroboros`        | `clawql-ouroboros`          | [Ouroboros](/ouroboros)                                                                                      |
| `hitl_enqueue_label_studio`      | TBD                       | `src/`                      | [HITL](/hitl-label-studio)                                                                                   |
...
| Doc                                                                                                                                                    | Use when                                |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------- |
| [ClawQL plugin model](/reference/plugins#plugin-model)                                                                                                 | Concepts, today vs target, request flow |
| [Modularization implementation status](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/modularization-implementation-status.md) | Package extraction phases, shims        |
