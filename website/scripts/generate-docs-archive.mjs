...
const TITLE_OVERRIDES = {
  '/payments/clawql-payments': 'clawql-payments',
  '/plugins/payments': 'Payments plugin',
  '/onyx-knowledge': 'Onyx knowledge (legacy path)',
  '/vision/immutable-releases': 'Immutable releases (vision)',
  '/getting-started/immutable-releases': 'Immutable releases (getting started)',
...
function collectHubMeta() {
  ...
  for (const file of files) {
    ...
    // href + nearby name/description (card objects)
  ...
  for (const file of files) {
    const url =
      ...
      file
        ...
        .replace(/\\/g, '/')
        .replace(/\/$/, '')
    if (url.includes('[')) continue // dynamic templates expanded separately
