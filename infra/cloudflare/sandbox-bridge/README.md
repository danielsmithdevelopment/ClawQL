...
> Lives under **`infra/cloudflare/`** with the other hosted-edge Workers. The bridge HTTP contract must stay aligned with current `clawql-sandbox` / `sandbox_exec` (Kata, Docker, Seatbelt). The sibling **gateway** Worker lags ClawQL **8.0.0** — see [../README.md](../README.md).
...
1. Install deps (Docker must be running for `wrangler deploy`):
   ...
   ```bash
   cd infra/cloudflare/sandbox-bridge
   npm install
   ```
