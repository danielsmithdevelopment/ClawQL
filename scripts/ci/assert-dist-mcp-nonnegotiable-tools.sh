...
for needle in 'server.tool("cache"' 'server.tool("audit"'; do
  if ! grep -qF "${needle}" "${FILE}"; then
    echo "ERROR: dist/mcp/tools.js must contain ${needle} — non-negotiable MCP tools (#75 #89)" >&2
    exit 1
  fi
done
echo "OK: dist/mcp/tools.js contains cache + audit registration."
