...
for f in held-out-frontier-labels.json held-out-frontier-summary.json gliner2-healthz.json \
  ...
  held-out-frontier-labels-v0.5-routing-fresh.json held-out-frontier-summary-v0.5-routing-fresh.json \
  held-out-frontier-labels-v0.6-routing-fresh.json held-out-frontier-summary-v0.6-routing-fresh.json; do
  if [[ -f "$TMP/$f" ]]; then
    cp -f "$TMP/$f" "$OUT/$f"
  fi
done
...
if [[ -f "$SUMMARY" ]]; then
  ...
  if [[ -n "${SUITE}" ]]; then
    ...
    case "${SUITE}" in
      ...
      v0.6-routing-fresh) EXPECT="fast-decision-held-out-v0.6-routing-fresh" ;;
    esac
