...
checks = [
    ...
    (r"(?m)^kind: NetworkPolicy$", "expected managed-gateway NetworkPolicy"),
    (r"runAsNonRoot: true", "expected non-root managed-gateway securityContext"),
    (r"readOnlyRootFilesystem: true", "expected read-only root filesystem"),
    (r"error_log /dev/stderr warn;", "expected stderr nginx error_log for non-root"),
