...
| **`kubernetes/`** | Local desktop k8s bootstrap — Docker Desktop or Rancher Desktop (`local-k8s-docker-desktop.sh`; **Istio ambient + Gateway/VirtualService + heavy observability on by default**; `CLAWQL_LOCAL_K8S_ISTIO=sidecar` for legacy sidecars; `CLAWQL_LOCAL_K8S_ISTIO=0` to skip mesh), MCP delete, Kyverno/Helm UI checks (`test-helm-ui-templates.sh`), Secret sync for local MCP (`k8s-docker-desktop-set-mcp-auth.sh`; `k8s-docker-desktop-set-github-token.sh` is a deprecated alias). |
