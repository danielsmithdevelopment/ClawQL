...
#
# Usage:
#   bash scripts/git-release-tag.sh           # create tag only
#   bash scripts/git-release-tag.sh --push    # create tag and push to origin
#   bash scripts/git-release-tag.sh --dry-run # print commands, do nothing
#
...
for arg in "$@"; do
  case "${arg}" in
    ...
    *)
      ...
      echo "Usage: bash scripts/git-release-tag.sh [--push] [--dry-run] [--allow-dirty]" >&2
      exit 1
      ;;
  esac
done
