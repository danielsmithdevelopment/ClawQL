...
#
# Usage:
#   bash scripts/release/git-backfill-npm-release-tags.sh
#   bash scripts/release/git-backfill-npm-release-tags.sh --push   # push only newly created tags
#   bash scripts/release/git-backfill-npm-release-tags.sh --dry-run
...
for arg in "$@"; do
  case "${arg}" in
    ...
    *)
      ...
      echo "Usage: bash scripts/release/git-backfill-npm-release-tags.sh [--push] [--dry-run]" >&2
      exit 1
      ;;
  esac
done
