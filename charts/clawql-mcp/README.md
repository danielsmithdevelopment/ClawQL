...
**Optional Managed Edge Gateway:** set **`inference.enabled=true`** and **`managedGateway.enabled=true`** to deploy **clawql-inference** (`/v1`) plus an nginx edge that routes **`/mcp`** and **`/v1`** on one hostname (defaults **off**). Defaults include non-root nginx, read-only root + emptyDir, and **`managedGateway.networkPolicy.enabled=true`**. JWT ATR stays on the MCP/proxy upstream. See **[`docs/getting-started/inference.md`](../../docs/getting-started/inference.md)**, **[`examples/managed-gateway/`](../../examples/managed-gateway/)**, fabric ladder: **[`docs/deployment/gateway-fabric.md`](../../docs/deployment/gateway-fabric.md)**.
