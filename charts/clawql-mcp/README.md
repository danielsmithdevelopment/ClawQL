...
**Optional Managed Edge Gateway:** set **`inference.enabled=true`** and **`managedGateway.enabled=true`** to deploy **clawql-inference** (`/v1`) plus an nginx edge that routes **`/mcp`** and **`/v1`** on one hostname (defaults **off**). See **[`docs/getting-started/inference.md`](../../docs/getting-started/inference.md)** and **[`examples/managed-gateway/`](../../examples/managed-gateway/)**.
