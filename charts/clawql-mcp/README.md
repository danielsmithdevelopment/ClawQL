...
**Team vault sync (`teamSync`):** defaults **`enabled: false`**. Set **`true`** with **`teamSync.bucket`** and optional **`prefix`** to sync shared **`Memory/`** notes to **Cloudflare R2** (default **`provider: r2`**), S3, or GCS. Enable **`autoPush`** / **`autoPull`** for debounced push after **`memory_ingest`** and pull before **`memory_recall`**. Store **`r2AccessKeyId`**, **`r2SecretAccessKey`**, and **`cloudflareAccountId`** in Vault KV (via **`envFromSecret`**) — see [`docs/getting-started/team-vault-sync.md`](../../docs/getting-started/team-vault-sync.md).
