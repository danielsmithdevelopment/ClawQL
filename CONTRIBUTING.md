...
- New **single** OpenAPI/Discovery providers: add under `providers/<vendor>/`, register in **`packages/clawql-api/src/spec/provider-registry.ts`** (`BUNDLED_PROVIDERS`), extend **`scripts/providers/fetch-provider-specs.mjs`** if the spec is fetched from a URL, and document in **`providers/README.md`**.
...
- Optional **pre-commit** (**[`.pre-commit-config.yaml`](.pre-commit-config.yaml)**): install with **`pip install pre-commit`** then **`pre-commit install`** to run **Gitleaks** on staged changes before push (CI still runs a mandatory scan — **`pre-commit` can be skipped with `--no-verify`**).
