...
**Legacy URLs:** `/archive` lists redirect-only routes not in the sidebar (`scripts/generate-docs-archive.mjs` → `src/generated/docs-archive.json`). All other docs pages are in the sidebar or hub cards.
...
| `/specs/classifier/fast-decision-routing-closeout` | `docs/specs/classifier/fast-decision-routing-closeout-v0.4.md` | `scripts/sync-fast-decision-routing-closeout-doc.mjs` |
...
| `/streams/clawql-tee` | `docs/streams/clawql-tee.md` | `scripts/sync-clawql-tee-doc.mjs` |
| `/streams/clawql-tee-airgap-audit` | `docs/streams/clawql-tee-airgap-audit.md` | `scripts/sync-clawql-tee-airgap-audit-doc.mjs` |
...
**CI (automatic):** on push to `main` that changes **`apps/docs/**`**, **`.github/workflows/deploy-docs.yml`runs`npm run deploy`**. Configure repository **Secrets\*\* (Settings → Secrets and variables → Actions):
