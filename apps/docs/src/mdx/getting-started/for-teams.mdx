Getting startedTeamsTeam operations guide. Generated from docs/getting-started/getting-started-for-teams.md on main. Covers helm deploy, team vault sync, golden host seeding, and observability.On this pageWhat teams needArchitectureDeploy the shared MCP backendTeam vault syncGolden host imagesObservability for team MCPVerify end-to-endNext steps
...
Prerequisites: Helm Kubernetes (or Tier 1 Compose for a lab slice), Vault provider secrets synced via External Secrets Operator, and at least one shared bucket (R2, S3, or GCS).
...
Same Memory/ notes for every agent and engineer via memory_recall

Team vault sync

Managed hosts

Packer golden VMs + Pulumi that pull the team bucket at boot

Golden host images

Metrics

Throughput, errors, audit counters on /metrics

IDP trace &#x26; metrics guide

Audit logs

Structured MCP tool events for grep and dashboards

Audit tool &#x26; observability

Traces

Request latency across tools and mesh hops

OTEL → Tempo (lab) or your collector

Work traces

Token savings, eval scores, LLM spans
...
Wire provider keys through HashiCorp Vault → External Secrets → envFromSecret. See Vault provider secrets and the Operations guide.
...
Engineers can run npx clawql-mcp locally and use clawql sync pull before memory_recall and clawql sync push after memory_ingest — same bucket as the cluster. Details under Team vault sync.
...
Ontology entity definitions and static knowledge belong in Git (small, PR-reviewed). Dynamic Memory/ instances and generated indexes belong in the bucket (unbounded growth, not tracked in GitHub). See Enterprise Ontology — Git vs R2 and ADR 0009.

Schema vs data: Ontology entity definitions and static knowledge belong in Git (small, PR-reviewed). Dynamic Memory/ instances and generated indexes belong in the bucket (unbounded growth). Do not put the full vault in GitHub — see Enterprise Ontology — Git vs R2 and ADR 0009.
...
Yes — team Markdown notes for memory_recall
...
Yes — optional agent chat threads
...
Never — API secrets stay local

memory.db

No — rebuilt locally after pull

Quick start (R2)

Create an R2 bucket in Cloudflare (e.g. acme-clawql-team).

Create R2 S3 API credentials (Manage R2 API tokens → Create API token with Object Read &#x26; Write).
...
Create an S3 bucket (e.g. acme-clawql-team) and an IAM user with s3:GetObject, s3:PutObject, s3:ListBucket on that bucket.
...
Push and pull as with R2 (clawql sync push, clawql sync pull).
...
Create a GCS bucket (e.g. acme-clawql-team) in your GCP project.

Enable interoperability: Cloud Storage → Settings → Interoperability → Create a key for a service account (or user HMAC key).
...
Write ~/.ClawQL/sync.json (no secrets)

clawql sync push

Upload changed local files + update remote manifest
...
Compare local vs remote (conflicts listed)

--dry-run

Show plan without I/O

--force

Overwrite on conflict (push → remote wins locally; pull → remote wins)
...
Cursor iOS: Cloud Agents have no local ~/.ClawQL on the phone — configure dashboard Secrets, stdio MCP, and end-of-session memory_sync. Full walkthrough: Agent setup — Cursor iOS.
...
auto: pull remote, then push local. pull or push: one direction.

force

false

Overwrite on conflicts; otherwise listed in response only.

dryRun

false

Plan without object storage I/O.
...
CLAWQL_SYNC_PROVIDER

r2 (default), s3, or gcs

CLAWQL_SYNC_BUCKET

Bucket name (overrides sync.json)

CLAWQL_SYNC_PREFIX

Shared team prefix, e.g. teams/acme/

CLAWQL_SYNC_ACCESS_KEY_ID

R2 S3 API access key (or generic override)

CLAWQL_SYNC_SECRET_ACCESS_KEY

R2 S3 API secret (or generic override)
...
GCS interoperability HMAC access id

CLAWQL_GCS_HMAC_SECRET

GCS interoperability HMAC secret

CLAWQL_SYNC_ENDPOINT

Override endpoint URL

CLAWQL_SYNC_REGION

Region (auto for R2/GCS)
...
Debounced push after each successful memory_ingest (default debounce 30s)

CLAWQL_SYNC_AUTO_DEBOUNCE_MS

Push debounce interval (default 30000)

CLAWQL_SYNC_AUTO_PULL=1

Throttled pull before memory_recall (default min interval 60s)

CLAWQL_SYNC_AUTO_PULL_MIN_MS

Min ms between auto-pulls (default 60000)

CLAWQL_SYNC_AUTO_PULL_ON_START=1

Pull once when MCP starts
...
r2AccessKeyId, r2SecretAccessKey, cloudflareAccountId

S3

awsAccessKeyId, awsSecretAccessKey (+ CLAWQL_AWS_REGION via extraEnv if needed)

GCS

gcsHmacAccessId, gcsHmacSecret
...
Security constraints (credentials never baked; SHA-256 verify of pulled vault files; doctor gates at bake and boot) are documented in Golden image pipeline — Packer VMs.
...
sync.json (bucket/prefix)

Template only

Overridden from metadata/env

Sync credentials

Never baked

Injected (Vault, instance role, secrets manager)
...
clawql doctor --smoke (boot)

Managed Edge Gateway

—

Dedicated/enterprise: /mcp + /v1 after vault
...
See infra/packer/README.md for GCP and CI validate targets. Use a clawql-mcp version that includes clawql gateway (Managed Edge Gateway).
...
See infra/pulumi/README.md and ADR 0007. State lives on self-hosted R2 or S3 — not Pulumi Cloud.
...
CLAWQL_SYNC_PREFIX=shared/

Dedicated

CLAWQL_SYNC_PREFIX=tenant/&#x3C;tenant-id>/

Enterprise

Customer-owned bucket; same image, their credentials
...
In-cluster MCP uses Helm teamSync (autoPullOnStart, autoPull) — same semantics as golden-host boot. See Team vault sync above.
...
Release: .github/workflows/packer-publish.yml — matrix AWS/GCP on dispatch; docker validate on every run
...
Key series today: clawql*audit*_, clawql*native_protocol*_. Import dashboards from docs/grafana/.
...
See Audit tool &#x26; observability and Bring your own observability.
...
Lab stack: Docker Desktop observability — Prometheus, Loki, Tempo, Grafana, OTEL collector in one profile.

Work traces (Langfuse)

Langfuse is the default work-trace ledger for token savings and eval — opt out with CLAWQL_ENABLE_LANGFUSE=0. See ADR 0005 and Bundled observability.
...
Tier 1 Compose lab — Prometheus, Loki, Tempo, Grafana, Langfuse

external

Point at existing backends — bring-your-own

minimal

Metrics only; disable Langfuse and optional push

Index: Observability bundle.
...
Grafana: import clawql-core-observability.json and clawql-idp-observability.json.

After pull, run clawql doctor or set CLAWQL_MEMORY_DB_SYNC_ON_RECALL=1 so memory.db reflects new Markdown.

Next steps

Agent setup — desktop, Cursor iOS Cloud Agents, local sandbox

Local provider vault — ~/.ClawQL layout and secrets

Deployment &#x26; Operations guide — upgrades, secrets, day-2

Helm chart — full values.yaml reference

Memory / Obsidian — memory_ingest / memory_recall

Golden image pipeline — container + Packer VM security gates
