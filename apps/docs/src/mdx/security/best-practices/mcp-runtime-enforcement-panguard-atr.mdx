SecurityTraining · Part 12/32Agentic AI security curriculum · Security overviewOn this pageWhy Prompt Filtering Alone FailsPanguard ArchitectureATR (Agent Task Request) RulesJSON Schema ValidationHITL (Human-in-the-Loop) FlowsPrompt Guard Libraries (Secondary Layer Only)Sandboxed Tool ExecutionConfused Deputy PreventionKey Takeaways (Memorize These!)MCP Runtime Enforcement: Panguard, ATR Rules, Schema Validation, and Injection Defense
...
Panguard serializes the full call context (tool name, parameters, ATR claims, session ID).

It publishes the request to the HITL queue.

A human reviewer sees the exact action the agent is about to take.

The reviewer approves or denies.
