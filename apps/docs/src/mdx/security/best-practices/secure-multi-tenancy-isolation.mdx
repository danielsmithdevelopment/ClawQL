SecurityTraining · Part 27/32Agentic AI security curriculum · Security overviewOn this pageThe Multi-Tenancy Threat ModelKubernetes Namespace IsolationPer-Tenant Vault Paths and Policy BoundariesMemory Store PartitioningPer-Tenant Resource Quotas and NATS LimitsAudit Log SegregationCross-Tenant Leakage Through Shared InfrastructureKey Takeaways (Memorize These!)Secure Multi-Tenancy: Namespace Isolation, Per-Tenant Vault Paths, and Audit Segregation
...
Cross-tenant data access — silent and high-severity (e.g., one tenant reading another’s memory store).

Resource exhaustion as DoS — noisy but effective (one tenant starving others of GPU, NATS, or CPU).

Audit-log contamination — compliance failure (one tenant’s events mixing with another’s).
