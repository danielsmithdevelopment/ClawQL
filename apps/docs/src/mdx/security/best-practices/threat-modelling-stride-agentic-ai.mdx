SecurityTraining · Part 22/32Agentic AI security curriculum · Security overviewOn this pageWhy Agentic AI Requires a Distinct Threat ModelSTRIDE Applied to Each ComponentAttack TreesThe Living Threat ModelThreat Model Documentation FormatKey Takeaways (Memorize These!)Threat Modelling for Agentic AI: STRIDE, Attack Trees, and Living Threat Models
...
Review the attack tree against the change.

Identify new branches or modified leaves.

Assign or update controls.

Document residual risk.

Store the updated model in git with a signed commit.
