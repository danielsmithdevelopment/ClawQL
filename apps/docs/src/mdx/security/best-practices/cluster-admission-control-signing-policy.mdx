SecurityTraining · Part 2/32Agentic AI security curriculum · Security overviewOn this pageWhy the Cluster Is the Last Line of Supply Chain DefenseCosign Image Signing — Cryptographic Proof of OriginKyverno Policy Engine — Kubernetes-Native EnforcementOPA/Gatekeeper as an Alternative (or Companion)Admission Webhook Architecture — The Technical DetailsBlocking Namespace Escape and Privileged WorkloadsAudit Mode Before Enforcement — Safe RolloutKey Takeaways (Memorize These!)Cluster Admission Control: Image Signing, Kyverno, and Blocking Unsigned Workloads
...
Deploy the policy in audit mode first (violations are logged but not blocked).

Review audit logs for 7 days to identify any legitimate workloads that would be blocked.

Fix or adjust those workloads.

Switch the policy to enforce mode only after the audit period is clean.
