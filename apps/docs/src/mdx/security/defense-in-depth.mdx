SecurityDefense in depthDeployment reference. Generated from docs/security/clawql-defense-in-depth-security-guide.md on main. For reasoning, red-team cases, and configuration specifics, see the Security best practices curriculum. Overview: Security.On this pageProduction Security for Agentic AI + MCP Systems on Self-Hosted k3sArchitecture Philosophy1. Supply Chain &amp; Build2. Immutability &amp; Infrastructure as Code3. Identity &amp; Zero-Trust4. Network &amp; Containment5. AI Agent / MCP Runtime Protection6. Data Classification &amp; PII Redaction7. Model Integrity8. Runtime Monitoring9. Automated Response &amp; Blocking10. Audit, Recovery &amp; Process11. Compute Quotas (GPU Protection)12. Workstation / Local Development13. DeploymentSummary: What Each Layer Stops

title: ClawQL Defense-in-Depth Security Guide
...
Harbor (registry — everything else depends on it)

Vault (secrets — needed before workloads)
...
The umbrella clawql-full-stack chart
...
Harbor allowlist-only resolution
...
Post-training weight tampering

Init container SHA-256 + Cosign blob

Harbor manifest signing

Training-time backdoor

Training pipeline supply chain controls (Section 1)

Behavioral evaluation (informative only)

Agent privilege escalation

JWT ATR claims + Panguard

Deterministic governance layer

Prompt injection / jailbreak

MCP tool scoping + ATR rules
...
East-west baselining alerts

Rogue egress

Istio EgressGateway + ServiceEntries

DNS hardening

Supply chain (Helm chart)

YubiKey Git signing

Cosign + Kyverno

Vault credential leak

Short TTL + auto-revocation

Lease monitoring

PII in logs

Presidio pre-Loki redaction
...
Silent log tampering

WORM + Merkle roots

Git-backed signed commits

Panguard outage

Fail-closed + HA Panguard replicas

PICERL outage runbook

Incident without forensics

WORM prompt/response logs

Wazuh SIEM correlation

Compromised primary infra

Out-of-band comms (Matrix)
...
For the full security curriculum (reasoning, red-team test cases, configuration specifics), see the Security best practices curriculum. For MCP proxy JWT/ATR details: mcp-proxy-jwt-atr.md.
