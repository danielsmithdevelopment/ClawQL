ContributingContractsImplementation contracts. Generated from docs/contributing/clawql-contributor-technical-specification.md on main. Read Vision &amp; Roadmap first for platform context. To run locally first, see Deployment &amp; Operations Guide.On this pageBefore You Start1. Core Contracts2. Architecture Rules3. Effect-TS Patterns4. Writing a Vertical5. Writing a Provider6. Security Contracts Contributors Must Honour7. Testing Requirements8. CI Pipeline Reference9. Versioning for ContributorsAppendix A: Quick Reference ChecklistAppendix B: Key Imports Reference

title: ClawQL — Contributor Technical Specification
...
If you want to run ClawQL locally before reading further, start with the Deployment &#x26; Operations Guide (Helm or npx clawql-mcp). Come back here when you are ready to build something.

Package extraction (phases 1–9) is on main. Vertical contributions still require a stable Plugin Layer API (onRegister tool registration, requiredSpecs, third-party contract) — see Plugin model and implementation status. You can write and test a vertical now using the in-memory test layers described in §3.4. Do not merge a vertical to main until the Plugin interface exit criteria in §1.1 are met for the surfaces your vertical depends on.
...
Horizontal packages (memory, documents, automation): see ClawQL plugin model for how these become plugins that register their MCP tools (memory_ingest, ingest_external_knowledge, schedule, etc.) and how that differs from the current tools.ts layout.
...
All MCP tool operation IDs follow the format kind**provider**operation using double underscores as separators.
...
lending**underwriting**createDealRoom
...
legal**contracts**clause_extract
...
healthcare**fhir**parse_r4

Edge cases:

Single underscores within any segment are preserved as-is. clause_extract stays clause_extract.

Double underscores within a segment are escaped to **ESC** before the ID is constructed. If your operation name is create**room, it becomes create**ESC**room in the output, producing kind**provider**create**ESC\_\_room. This is the only safe way to include double underscores in a segment. In practice: do not use double underscores in segment names. It is avoidable and the escaping, while correct, is ugly.
...
api.execute('legal**contracts**clause_extract', {
...
const result = yield\* api.execute("legal**contracts**clause_extract", params);
...
const result = yield\* api.execute('lending**underwriting**createDealRoom', {
...
Shipped (7.0, opt-in): Gateway hooks redact on execute, memory_ingest, and ingest_external_knowledge when CLAWQL_ENABLE_PRESIDIO=1 — see MCP clients — Presidio. The contract below still applies to vertical plugins and future pipeline stages.
...
Pure functions (Merkle, Cuckoo, normalizeOperationId, ATR validation)

100%

Yes — CI blocks merge

Service layer in verticals and horizontals
...
TypeScript correctness across all project references

Lint

ESLint + custom rules

Import restrictions, architecture rules, code style

Unit tests

Vitest

Pure function coverage, typed error usage

Cycle detection

madge --circular

No circular imports anywhere in the graph

Architecture diff

pnpm arch:check

Dependency graph matches committed diagram

Integration tests

Vitest + Docker Compose

Live service tests with fixture documents

Coverage

c8

Coverage thresholds per §7.1

Contract tests

Pact

Consumer-provider contracts for clawql-api surface

E2E test

Playwright + Docker Compose

Full workflow test for affected verticals
...
No GPL-incompatible licenses in production code
...
Tool input schema changed non-additively
...
New optional field added to tool input/output

Patch (with Schema default)

Bug fix with no schema change

Patch

New required provider spec declared
...
ClawQL Contributor Technical Specification · May 2026 · Apache 2.0 / MIT

For platform vision and roadmap: see the Vision &#x26; Roadmap document.

For deployment instructions: see the Deployment &#x26; Operations Guide. Planned Operator model: Operator target architecture.
