ArchitectureToken efficiencyPerformance and cost. Context bloat is a reasoning failure mode (Lost in the Middle, length-alone degradation), not only a bill. Twelve optimization layers in three tiers (structural → smart inference → continuous/flywheel). Source: docs/architecture/clawql-token-efficiency.md. Hands-on MCP: Using search &amp; execute. Inference: clawql-inference.On this pageThe Problem: API Surfaces Don&#x27;t Fit in a Context WindowLayer 1: Code Mode — The FoundationLayer 2: Trimming What Comes BackLayer 3: Cutting Prose FillerLayer 4: Prompt Caching — Making Repetition CheapLayer 5: Skipping Repeated Work EntirelyLayer 6: Compressing History in Long SessionsLayer 7: Trimming the Final PromptLayer 8: Routing Tasks to the Right ModelBeyond These Eight LayersLayers 9–12 (Inference Gateway Extensions)Implementation MapPutting It TogetherKnown Trade-offsComparison to Published Benchmarks

title: How ClawQL Reduces Token Usage: A Layered Approach
...
clawql-inference — injects concise structured-output guidance
...
clawql-inference — derives word budget from max_tokens
...
clawql-inference — optional assistant prefill (CLAWQL_INFERENCE_PREFILL=1)
...
clawql-inference export → fine-tune → frugal tier registration
...
MCP search + execute (clawql-api) — always on

2 Response trim

field-projection.ts on execute output — always on

3 Terse output

TokenEfficiencyGateway post-processor — on (CLAWQL_INFERENCE_TERSE=0 to disable)

4 Prompt cache

Anthropic cache_control on stable system prefix — on (CLAWQL_INFERENCE_PROMPT_CACHE=0 to disable)

5 Semantic cache

SemanticCachedGateway with read/write safety — on when embeddings configured

6 History compress

Rolling transcript distillation — off (CLAWQL_INFERENCE_HISTORY_COMPRESS=1)

7 Prompt compress

Pre-send dedupe + truncation — off (CLAWQL_INFERENCE_PROMPT_COMPRESS=1)

8 Model routing

Ouroboros escalation + HTTP clawql/* aliases — off (CLAWQL_INFERENCE_ROUTING_ENABLED=1)

9–11 Extensions

Structured output, token budget, prefill — see env table in clawql-inference.md

12 Flywheel

Export pipeline + finetune register
...
For the search/execute workflow, see docs/mcp/mcp-tools.md. For inference gateway layers and env vars, see docs/inference/clawql-inference.md. For platform context, see the Vision &#x26; Roadmap document.
