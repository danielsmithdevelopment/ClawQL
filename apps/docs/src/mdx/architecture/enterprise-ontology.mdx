ArchitectureOntologyOpen enterprise Ontology. Typed objects for agents (entity · graph · kinetic actions), versioned in Git with OKF memory, not a proprietary console. Foundation CLI (clawql ontology lint / generate) ships; graph, kinetic sandbox, and Command Deck builder are phased. Source: docs/architecture/enterprise-ontology.md. Essay: Memory Finds. Ontology Decides. (OpenBench B-7 — why typed predicates beat memory alone). Specs: memory_recall structured filters · legal domain. Related: Token efficiency · Agentic Fabric · Memory.On this pageOne-Sentence PitchOntology Enables Token Efficiency (Tier 1)Why Formalize — and Why Differently from PalantirOOP Taken Seriously for Enterprise AIThree-Layer ArchitectureKinetic Governance: MCP First, GraphQL as Transport LaterKinetic Executors (the Triad + Native)OKF and the Memory VaultGit vs R2 — What Lives WhereOntology Builder (Yes — Sequenced)Try It TodayIndustry Context (Not Dependencies)Design-Partner GatePhased Delivery ChecklistSee Also

title: Enterprise Ontology — open, versioned, kinetic

Open enterprise Ontology. Typed objects for agents (entity · graph · kinetic actions), versioned in Git with OKF memory, not a proprietary console. Foundation CLI (clawql ontology lint / generate) ships; graph, kinetic sandbox, and Command Deck builder are phased. Source: docs/architecture/enterprise-ontology.md. Related: Token efficiency · Agentic Fabric · Memory.
...
Essay (why ontology beats memory alone — OpenBench B-7 proof): Memory Finds. Ontology Decides. · draft copy docs/gtm/pragmaticvectors/memory-finds-ontology-decides.md · site: structured filters · legal domain · repo: legal-domain-v0.1 · memory-recall-structured-filter-v0.1 · OpenBench B-7
...
.cqe / entity YAML → generated read tools; full catalogs stay server-side

2 Response trim

Project only declared properties (ATR-visible)

6 History distill

Compact transcripts into type: decision rationale / .cqk

7–9

Graph-aware recall + structured OKF output instead of free prose

8 Routing

kinetic_level / risk informs Frugal → Frontier

Full stack: Twelve layers of LLM cost. Ontology without those layers becomes a typed landfill; layers without ontology answer cheaply about the wrong objects.

Proof point (OpenBench B-7): vault memory_recall alone still scored zero on institutional enumeration when semantic near-misses became false positives. Typed predicates via clawql-ontology (schema + filters over ontology.db) closed the set. Narrative + linked GHA runs: Memory Finds. Ontology Decides..
...
Open YAML / OKF Markdown + JSON Schema; export is a file tree

Self-service build

Derive from SQL, OpenAPI, document classifiers; refine, don't construct from scratch

Pipeline-native

Schema in Git; schema changes are manifest version events (signed / WORM)
...
Typed object + sources + graph edges + kinetic methods
...
Provisional shape (YAML or OKF frontmatter + body). Full example: docs/examples/ontology/.
...
v1 decision (ADR 0009 §10): ship kinetic writes as MCP tools from Entity actions (kinetic: true + graded fields). PEP + Transaction Sandbox intercept MCP. GraphQL @kinetic remains the fabric-aligned transport target — not a v1 ship gate.
...
Plan / state / surgical rollback

Infrastructure kinetic actions (ADR 0007)

Argo Workflows

DAG, retries, artifacts, suspend

IDP pipeline + multi-step agent tasks (ADR 0004)

Argo Rollouts

Canary, analysis, promote/rollback

Deployment kinetic actions

Native sandbox

Same state machine in Effect-TS

SAP / CRM / payments and other app writes
...
Open Knowledge Format (OKF) v0.1 is a thin convention: directory of Markdown files, YAML frontmatter, path as identity, type required (announcement). Community critique ("rebrand of wiki/Obsidian") is partly fair; the value is interoperable frontmatter, not a new runtime.
...
Next (extensions): ADR 0010 — .cqe is primary in docs/examples; lint/generate still dual-accept .yaml / .yml / .json. Specs: docs/specs/cq-extensions/.
...
Schema + static knowledge

Git → R2 on release

Propagates with release / ontology generate

Memory + instances

R2 only

clawql sync / memory_sync

Hot tier (recent / active)
...
Now — format + CLI: clawql ontology lint / generate (read tools); CI with clawql-release lint; derivation from SQL/OpenAPI.

Next — visual builder in Command Deck: entity / relationship / action panels; kinetic fields non-accidental; emits YAML → Git PR. UX quests and inspector layout: command-deck-ontology-builder-ux.md (Fabric Ontology Playground as reference only).

Later — vertical packs: Legal, Healthcare, Financial Services, Real Estate as OKF bundles teams adopt and customize.
...
CLI reference: docs/ontology/cli.md. Examples: docs/examples/ontology/. SQL sources: are declarations / stubs in v1 — not a live query path.
...
Entity types, relationships, agent grounding, visual playground

Platform-bound (OneLake); ClawQL stays Git + open YAML/OKF + kinetic PEP

AIF (Argument Interchange Format)

Structured rationale so understanding transfers without ambiguity

Argumentation-specific; ClawQL reuses the idea in OKF type: decision for enterprise decisions

Palantir Ontology

Typed digital twin for agents

Proprietary console; ClawQL is portable and pipeline-native (ADR 0009)
...
v1alpha1 schema, lint, read-tool generate, source derivation, manifest version pin

This ADR — schema + clawql ontology lint / generate shipped; derivation / manifest pin still open

Graph

Relationship traversal, ATRClaim-aware edges, graph-aware recall

Foundation

Kinetic

Write tools, @kinetic, Transaction Sandbox, AP2, canary

Foundation + PEP

Builder UI

Command Deck schema editor
...
Builder + design partners

See Also

ADR 0009 — decision record

Ontology CLI — clawql ontology lint / generate

Token efficiency (12 layers)

OKF decision rationale template

Command Deck ontology builder UX

Essay gap closure backlog

Example entity: docs/examples/ontology/entities/Contract.cqe

Example decision note: docs/examples/ontology/okf/decision-rationale-template.md

JSON Schema: schemas/ontology/entity.schema.json

Ouroboros Seed ontology (task loop — different artifact): ADR 0001
