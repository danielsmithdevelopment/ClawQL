VerticalGovernmentDraftPlanned package — not yet shipped. Source: docs/government/clawql-government.md. Industry page: clawql.com/industries/government. Air-gap auditor export via QR stream transport.On this pageThe problem this vertical solvesThree requirements outcome measurement currently lacksArchitectureDomain tools (MCP)Core servicesEnvironment variablesCLI referenceUse casesDeployment topologyThe legislative opportunityNonprofit contractor accountabilityRelationship to other ClawQL verticalsPricing and GTMDesign principles specific to this verticalRelationship to QR / TEE air-gap transportFurther reading
...
Create a new government program with structured outcome definitions

program_authorize

Authorize a program, anchor baseline, begin outcome tracking

outcome_define

Define a measurable outcome metric with baseline, target, and measurement schedule

outcome_record

Record a periodic outcome measurement against a defined metric

outcome_compare

Compare current outcomes to baseline and stated targets

outcome_report

Generate a full outcome report for a program, publishable to Arweave

baseline_anchor

Anchor baseline metrics to Arweave at program inception

baseline_verify

Verify baseline against Arweave anchor — detects retroactive adjustment

document_ingest

Ingest a government document with hash, classification, and chain of custody

document_retrieve

Retrieve a document with full provenance proof

document_export_foia

Export documents in FOIA-compliant format with chain of custody

contract_ingest

Ingest a vendor contract with outcome requirements extracted

contract_monitor

Monitor vendor contract compliance against outcome requirements

audit_log_query

Query the immutable audit log for a program, agency, or time range

bond_validate

Validate bond measure language against measurability standards

bond_authorize

Create a program record from authorized bond measure

spending_record

Record spending against a program with correlation to outcome metrics

spending_outcome_correlate

Correlate spending records with outcome records for a program

auditor_export

Export program records for state auditor in standard format

public_dashboard_publish

Publish outcome report to public dashboard with Arweave anchor

whistleblower_ingest

Ingest a whistleblower report with identity protection and tamper-evident storage
...
bond_validate() analyzes the ballot text and identifies vague language, missing baselines, and unmeasurable promises

bond_authorize() creates a program record with structured outcome definitions extracted from the measure

baseline_anchor() captures current housing metrics (units available, affordability index, vacancy rates) and anchors them to Arweave immediately

The Arweave transaction ID is published in the program record — anyone can verify what the baseline was before any spending began

As money is spent:

spending_record() logs each disbursement against the program

outcome_record() logs periodic measurements from independent sources

outcome_report() generates quarterly compliance reports published to Arweave

public_dashboard_publish() makes progress visible without revealing sensitive documents

If outcomes aren't materializing:

OUTCOME_TARGET_MISSED audit events trigger alerts to the oversight authority

spending_outcome_correlate() surfaces the ratio of spending to outcomes achieved

The full record — baseline, spending, outcomes, discrepancies — is independently verifiable by the state auditor, journalists, and the public
...
contract_ingest() extracts outcome requirements from the contract

outcome_define() creates machine-readable metrics: response time improvement, crime rate change, false positive rate

baseline_anchor() captures pre-deployment crime statistics, response times, and false positive rates from independent sources

contract_monitor() runs periodic checks against defined outcome metrics

At contract renewal, outcome_report() provides an independently verifiable record of whether the vendor delivered
...
Relevant documents are in the Document Vault with full chain of custody

document_export_foia() generates a compliant response with:
...
The FOIA response itself is logged as a DOCUMENT_EXPORTED_FOIA audit event
...
whistleblower_ingest() accepts a report with identity protection — reporter identity is encrypted separately from report content

Report is stored in WORM storage — cannot be deleted by the agency being reported

Report generates an audit event that the oversight authority can see but the reported agency cannot

The Arweave-anchored baseline provides the independent reference point for evaluating the report
...
Industry page: clawql.com/industries/government
