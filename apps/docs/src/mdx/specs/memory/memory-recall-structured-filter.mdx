SpecsMemoryOntologyv0.1 specWhy we extended memory with clawql-ontology. Semantic memory_recall finds narrative context; typed schema + filters decide exact set membership against ontology.db. Narrative proof: Memory Finds. Ontology Decides.. Source: docs/specs/memory/memory-recall-structured-filter-v0.1.md. Companion: Legal domain ontology · Enterprise Ontology · Memory.On this page1. Problem2. New Parameters3. Routing Logic4. Ontology Query Implementation5. Result Format6. MCP Tool Specification Update7. Interaction with Existing Sources8. Benchmarks: Before and After9. Testing10. Effect-TS Implementation Notes
...
Structured predicate (schema + filters): pure ontology index query, O(1) per filter, deterministic

Schema-typed semantic (schema only): semantic search scoped to entities of the given type — "tell me about M&#x26;A matters" without specific field predicates

Untyped semantic (no schema): existing behavior, unchanged
