SpecsNetworkv0.1Headscale for standing relationships, Tailcat for genuinely ephemeral links. The package contribution is the selector (safe-by-default routing) and enforcement (ATR-gated tailcat with WORM audit). Operator bootstrap: clawql init --networking. Source: docs/specs/network/clawql-network-v0.1.md. Deployment: Tailscale / Headscale guide · clawql-cellrt.On this pagetitle: &#x27;clawql-network — Specification&#x27;

status: &#x27;August 2026&#x27;

version: &#x27;0.1&#x27;

package: &#x27;packages/clawql-network/&#x27;Specification v0.11. Purpose2. What Each Piece Is Actually For3. Package Structure4. Headscale: Persistent Mesh5. Tailcat: Ephemeral Connections6. The Selector: Deciding Which Transport to Use7. Enforcement: Tailcat Requires Explicit Scope, Every Time8. The clawql-network Init Flow9. Cross-Region Cost Awareness10. Package Boundaries — SummaryRelated
...
Headscale-managed mesh

Tailcat

Relationship

Persistent, known nodes

Ephemeral, one-off peers

Setup

Node registers with control plane, gets identity

Address string generated and shared out of band, no registration

ACLs / access control

Yes, centrally managed

None built in beyond an optional public-key allowlist

Best fit

A customer's fleet, your homelab's five machines

A serverless worker spun up for one task, then torn down

Wrong fit

—

Anything where standing governance matters (see §7)
...
Persistent mesh, ACLs, node identity

Headscale (self-hosted control plane)

The right tool for standing relationships

Ephemeral, one-off connections

Tailcat (subprocess, Go binary)

The right tool for genuinely transient links

Deciding which to use

clawql-network's selector.ts

The actual product — automatic, safe-by-default routing

Enforcement and audit for tailcat use

clawql-core hook + clawql-audit

Tailcat has no control plane of its own to log anything; this is the only place the record can live

Cloud-TEE-to-on-prem connections

Headscale mesh + clawql-tee attestation, never tailcat

Standing governance and hardware attestation both matter here; tailcat provides neither

One-time setup

clawql init --networking

Minimal-effort default, per the original goal

Related

Deployment: docs/deployment/tailscale-and-headscale-for-clawql.md · headscale-tailnet.md

Runtime coordination: docs/streams/clawql-cellrt.md

Trust: docs/streams/clawql-tee.md

Plugin hooks: docs/design/clawql-core-plugin-architecture.md
