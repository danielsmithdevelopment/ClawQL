.cqkPart of .cq\* extensions.On this pagePurposeSerializationRequired frontmatterRecommended frontmatterDistinguishing .cqk vs .md vs .cqeTooling hooksNon-goalsExample

title: .cqk — ClawQL Knowledge (draft)
...
OKF required — ClawQL taxonomy (decision, context, task_result, …)
...
WORM entry hash or null when not yet sealed
...
List of Ontology entity names or instance ids
...
Portable knowledge; ClawQL can read/write

.cqk

Same + required provenance fields; Onyx/doctor treat as ClawQL-owned

.cqe

Schema definition (entity type), not an instance note
...
Prefer the decision rationale template body sections when type: decision. Worked example: docs/examples/ontology/okf/decision-rationale-template.md.
