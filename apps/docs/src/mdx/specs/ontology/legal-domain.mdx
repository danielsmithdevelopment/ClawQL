SpecsOntologyv0.1 specLegal domain pack for clawql-ontology. Typed entities that seed ontology.db so memory_recall can evaluate exact field predicates. Source: docs/specs/ontology/legal-domain-v0.1.md. Related: memory_recall structured filters · Enterprise Ontology.On this pageRepo alignment1. Purpose2. Entity Definitions3. Field Extraction Rules4. Ontology Index5. Integration with memory_recall6. Ontology Linter7. B-7 Benchmark Impact8. Implementation Sequence9. Connection to clawql-streams and Training Pipeline
...
Essay (value + OpenBench proof): Memory Finds. Ontology Decides. — why typed predicates beat semantic vault recall for firm-knowledge enumeration.
...
clawql-ontology/domains/legal/

packages/clawql-ontology/packs/legal/

\*.cqe entity files

packs/legal/entities/\*.cqe (ADR 0010)

ontology.db

Colocated with memory.db under CLAWQL_OBSIDIAN_VAULT_PATH (packages/clawql-memory)
...
Parse float, validate 0-100

CLAWQL_NONCOMPETE_MONTHS

Matter.nonCompeteMonths

Integer

Parse int

CLAWQL_DEAL_VALUE_USD
...
CLAWQL_ESCROW_DURATION_MONTHS

Matter.escrowDurationMonths
...
memory*ingest populates the ontology index alongside the existing vault write. Structured memory_recall also lazy-syncs CLAWQL*\* blocks from vault Markdown when the index is empty (OpenBench seed paths that write files without ingest).

5. Integration with memory_recall

See companion memory-recall-structured-filter-v0.1.md.
...
Client joins, amortized multi-question sessions, and full C&#x26;H mount follow the suite plan in docs/benchmarks/openbench-b7-calderwood.md.
...
Machine-readable field parser for CLAWQL\_\* blocks
