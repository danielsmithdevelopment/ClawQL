Getting startedAgentsDesktop, iOS, and laptop sandbox. Generated from docs/getting-started/agent-setup.md on main. Related: Team vault sync· MCP clients· sandbox_exec.On this pagePick your pathDesktop / stdio (recommended)Cursor iOS + Cloud AgentLocal agent sandbox (macOS Seatbelt)Next steps
...
Setting up Cursor or Claude Desktop

Desktop / stdio

On Cursor iOS / Cloud Agent

Cursor iOS + Cloud Agent

Hardening local agent shell on macOS

Local agent sandbox

Private mesh / ephemeral transport

Networking · clawql-network spec

Sharing vault notes with a team

For teams — vault sync
...
See clawql-network and the Tailscale / Headscale guide.
...
~/.ClawQL/vault/providers.json (loaded at MCP startup)

Kubernetes

HashiCorp Vault → ESO → clawql-provider-env

Dashboard

Provider secrets UI → Vault KV

Guide: local-provider-vault.md
...
Related: For teams — Team vault sync · MCP clients
...
Object-storage bucket with a team prefix (R2 quick start: For teams — R2).

Cursor Cloud Agents enabled for your account (repo connected in the Cursor dashboard).

clawql-mcp on the agent VM — npx -p clawql-mcp resolves it at MCP startup (Node.js is preinstalled on Cloud Agent VMs).
...
Vault root on the VM, e.g. /home/ubuntu/.ClawQL
...
Shared prefix, e.g. teams/engineering/
...
1 — debounced push after memory_ingest

CLAWQL_SYNC_AUTO_PULL

1 — throttled pull before memory_recall

CLAWQL_SYNC_AUTO_PULL_ON_START

1 — pull once when MCP starts

For S3 or GCS, use the credential variables from For teams — Environment instead of R2 keys.
...
Auto-pull on MCP start (if CLAWQL_SYNC_AUTO_PULL_ON_START=1)

Recall context

memory_recall with a focused query

Persist outcomes

memory_ingest after decisions, debugging, or API contracts

End of session

memory_sync with \{ "direction": "auto" \} — pull remote changes, then push local

Next session (new VM)

Auto-pull → memory_recall sees prior notes

memory_sync replaces shell clawql sync push / pull on Cloud Agents. See For teams — memory_sync.
...
- Sync credentials are in Cursor Cloud Secrets (CLAWQL*SYNC*\* + CLAWQL_R2_ACCOUNT_ID).
...
If sync is not configured, say so and list which CLAWQL*SYNC*\* secrets are missing.
...
memory_recall empty on a new VM

Secrets set? memory_sync \{ "direction": "pull" \} or CLAWQL_SYNC_AUTO_PULL_ON_START=1

memory_sync errors

CLAWQL_SYNC_BUCKET, prefix, and R2/S3/GCS credentials in dashboard Secrets

execute auth failures

Provider keys in vault/providers.json or matching CLAWQL\_\* env secrets

MCP tools missing

Enable clawql server for the Cloud Agent run; confirm CLAWQL_ENABLE_MEMORY is not 0

Conflicts after parallel runs

memory_sync response lists conflicts; use force: true only deliberately
...
ClawQL sandbox-exec -f ~/.ClawQL/sandbox/claude.sb -D WORK_DIR=…

Inner

Claude Code native /sandbox via ~/.claude/settings.json
...
~/.ClawQL/sandbox/claude.sb

Seatbelt wrapper + Claude /sandbox

codex

~/.ClawQL/sandbox/codex.sb

sandbox-exec only

cursor

~/.ClawQL/sandbox/cursor.sb

sandbox-exec only

opencode

~/.ClawQL/sandbox/opencode.sb
...
Share only company repos folder

MCP-side sandbox_exec: Sandbox plugin · Learn: sandbox_exec. Decision record: ADR 0008.

Next steps

Quickstart

For teams — Team vault sync
