SecurityAir-gapDraftQR air-gap audit — not yet shipped. Generated from docs/streams/clawql-tee-airgap-audit.md, Learn walkthrough (§5 TEE + air-gap reading order). Companion to clawql-tee and clawql-cellrt.On this page1. Problem2. Transport model3. QR sequence format4. Display and scan protocol5. Frame count estimates6. Integration7. Threat model coverage8. Zero-trust story9. clawql-tee-verifier (standalone)Further reading

title: clawql-tee Air-Gap Audit Transport — Specification v0.1

clawql-tee Air-Gap Audit Transport — Specification v0.1

Hands-on walkthrough: Streams getting started (Learn) — §5 TEE + air-gap reading order (with clawql-tee).

Status: Draft · August 2026 · v0.1

Component: cellrt-attestation · cellrt-storage · clawql-tee-verifier (standalone)

Depends on: clawql-cellrt v0.1 · clawql-tee v0.1 · AMD SEV-SNP (or TDX) attestation
...
WORM audit trail — cell spawn, tool calls, inference calls, virtual key events, destruction; Merkle-linked

TEE attestation report — SEV-SNP/TDX report + cert chain

Session binding — cell ID, virtual key ID, event hash, RTP/OBT record hash
...
WORM entries as CBOR (compact vs JSON)

zstd compress (typical 5–10× for structured logs)

Chunk target ~2 KB (QR v40 ECC M ≈ 2.3 KB binary)

Optional ChaCha20-Poly1305 with session ephemeral key (confidentiality; integrity from Merkle/HMAC regardless)
...
Standard monitor on TEE VM — most practical

Serial

ASCII QR via console — headless

E-ink

Slower refresh; persistent between frames

Thermal print

Paper strip — physical artifact; scan later offline
...
Strictly sequential frame_index (gaps → reject / rescan)

payload_hash matches payload

prev_frame_hash links Merkle chain

HMAC under Header ephemeral key

On complete sequence:

Recompute Merkle root vs Footer

Verify SEV-SNP/TDX attestation against AMD KDS / Intel PCS (from verifier device, not from the TEE)

Verify Ed25519 session signature

Reconstruct WORM entries → VerificationResult
...
Without QR transport

With QR transport

Compromised network modifies audit entries

Undetected

Merkle break — rejected

Operator selectively filters events

Undetected

Frame gap — rejected

Cloud provider replays old trail

Partial (timestamps)

- session binding to event hash

TEE binary tampered

SEV-SNP measurement

Same — attestation in sequence

QR sequence from another session interleaved

N/A

HMAC / ephemeral key detects
...
1. WASM capability sandbox

1. Attestation-gated virtual keys

1. WORM audit (LTX, RPO=0)

1. QR air-gap transport ← this document

1. GPU confidential computing
...
docs/streams/clawql-qr-stream-transport.md — 7th mcp-api-adapter surface + Streams qr source (generalizes this frame format)

docs/streams/clawql-cellrt.md — owned cell runtime

docs/streams/clawql-streams.md — WORM event types / session lifecycle

docs/government/clawql-government.md — auditor on-site QR export consumer
