PlatformDurable ObjectsDraftPlanned runtime — not yet shipped. Generated from docs/streams/clawql-durable-objects.md, Learn walkthrough (§2 Durable Objects reading order). Companion to ClawQL Streams v0.2. Self-hosted DO runtime detail: celld integration · clawql-cellrt.On this page1. Purpose2. Object types3. Sidecars (AgentSessionDO)4. Spawn contract (gateway)5. Virtual key lifecycle6. WebSocket and hibernation7. SQLite schema (AgentSessionDO, illustrative)8. Session close sequence9. Self-hosted parity10. mcp-api-adapter on DOs11. Security checklist12. Open questionsFurther reading
...
Ephemeral compute per event — spawn → work → destroy; pay for what runs.

Ephemeral model credentials — virtual key bind-on-create, expire-on-destroy.

Three sidecars — forensic audit, inference, training-data emission — without conflating them.

Parity path — same session contract on K8s workers when DOs are unavailable (regulated / air-gapped).
...
Holds WebSocket/source connection, significance filter, subscription config + rtpConsent, event buffer

AgentSessionDO

Ephemeral (per event)

Runs one agent session with three sidecars; destroys itself on exit
...
Allocates doInstanceId (and stable celld/Cloudflare name, e.g. sess:\{subscriptionId\}:\{eventId\})

Asks clawql-inference to issue virtualKeyId scoped to doInstanceId

Writes WORM DO_CREATED with \{ doInstanceId, virtualKeyId, subscriptionId, eventHash, manifestId \}

If credit-gated: DeductionService.hold(estimatedCost)

Spawns DO with environment bindings: DO_INSTANCE_ID, VIRTUAL_KEY, SUBSCRIPTION_ID, MANIFEST_ID, RTP_CONSENT_JWT, POLICY_ALIAS, INFERENCE_URL
...
Issue key scoped to DO instance ID; budget = budgetTokens or derived from budgetUsd; TTL = maxTurns × estimated_turn_duration; PAL from subscription model alias

Run

Every model call uses only this key via fetch(clawql-inference); call-store rows include virtual_key_id

Close

Capture actual spend; flush training record; WORM DO_DESTROYED; expire key → subsequent use is 401; DO destroys itself

Abnormal

Eviction / timeout / cell migration: hold expires with key TTL; WORM best-effort close; key still expired by TTL
...
ordered RTP nodes (Intent → … → Verdict) as JSON rows

inference_calls

mirror of call-store fields needed for OBT (tier, tokens, cache)
...
pending / flushed / failed + destination
...
1. TrainingDataSidecar: finalize RTP Verdict node; wrap OBT; export

1. DeductionService.capture(actual) or release on failure (if held)

1. AuditSidecar: DO_DESTROYED + spend summary (storage.put → LTX on celld)

1. InferenceSidecar: expire virtual key

1. Clear sensitive SQLite; DO stub destroys / becomes reclaimable
...
Per-cell isolate (single writer)

Pod / Job per event (or pool with hard reset)

SQLite

DO storage

DO storage + LTX → operator bucket

Ephemeral volume or better-sqlite3 in worker

Hibernation

Native WS hibernation

Same DO state model

Scale-to-zero / idle timeout (not identical)

Virtual key

Same clawql-inference API
...
LTX on fleet bucket (sqlite3 audit)
...
Fleet density / alarms

NATS consumer lag → HPA (Streams §9)
...
Bundle fit: streams-slim + thin fetch clients under 64 MiB today; optional future Workers-safe clawql-api / adapter slim (Streams §15, evidence).

Whether SubscriptionDO and AgentSessionDO share a DO namespace or separate script names for IAM (Cloudflare) / fleet isolation (celld).

Miniflare maturity vs celld diagnose smoke for CI parity.

Batching: one AgentSessionDO per batch window vs N DOs with a parent batch coordinator.

celld multi-tenant: separate fleets per org until upstream scheduler exists.

Further reading

docs/streams/clawql-streams.md — Streams Specification v0.2

docs/streams/clawql-celld.md — celld integration (constraints, LTX, deploy)
...
docs/inference/clawql-inference.md — virtual keys, PAL, call store

docs/benchmarks/openbench-trace-collection.md — OBT + RTP

celld · docs · limitations · security
