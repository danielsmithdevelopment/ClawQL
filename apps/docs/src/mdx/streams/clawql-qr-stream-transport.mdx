PlatformQRDraftPlanned 8th mcp-api-adapter surface — not yet shipped. Extends TEE air-gap audit frames into Streams + MCP. Source: docs/streams/clawql-qr-stream-transport.md.On this page1. What this is2. Transport modes3. Frame format4. mcp-api-adapter — 8th surface (planned)5. clawql-streams — qr source type6. Election module7. clawql-tee-verifier additions8. Helm values (planned)9. PositioningFurther reading

title: ClawQL QR Stream Transport — Specification v0.1
...
Display → camera; no return channel required

Cryptographically verifiable

Merkle chain, per-frame HMAC, optional TEE attestation

Network-independent

Air gap is structural, not policy

Physically auditable

Thermal printer → paper QR sequence, scan/verify later
...
Env: CLAWQL*QR_OUTPUT, CLAWQL_QR_INPUT, MCP_API_ADAPTER_QR*\*.
...
Frame as PNG (testing)

POST /qr/scan

Submit scanned frame bytes (bidirectional)

GET /qr/stream

SSE of QR PNGs (dev preview only, not air-gap)
...
└─ significance filter → Agent DO (memory\_\*, notify, execute…)
...
Air-gapped machine; no inbound network

Software tampering

TEE attestation + Cosign measurement

Ballot stuffing

ZK validity proofs rejected if malformed

Post-cast manipulation

Merkle + voter commitment receipt

Vote suppression

Public commitment list vs QR frame count

Individual vote reveal

Homomorphic + ZK (math, not policy)

QR interception/mod

Merkle + HMAC + TEE attestation

6.5 Honest limitations

Voter interface — crypto cannot detect display-A / encrypt-B; paper review is mandatory.

Key ceremony — who holds ElGamal shares is a trust question; prefer threshold decryption.

ZK library correctness — buggy proofs undermine the system; TEE proves certified code runs, not that the certification is bug-free.

Physical perimeter — air gap only as strong as custody and seals.

Political context — ClawQL provides infrastructure, not electoral policy; certification and law are out of band.
...
Real-time / DO hibernation

/mcp-ui

Yes

Browser HTMX playground (7th surface)

QR stream

No

Air-gap, elections, regulated optical channel (8th planned)

Seven surfaces connect over a network today (including /mcp-ui). QR stream connects air-gapped systems to the ClawQL agent surface over a physical channel with per-frame crypto and optional TEE binding.
...
docs/streams/clawql-tee-airgap-audit.md — frame crypto shared with TEE audit

docs/streams/clawql-tee.md — hardware attestation

docs/streams/clawql-streams.md — event loop / significance

docs/mcp/mcp-api-adapter.md — seven surfaces shipped; QR 8th planned

docs/mcp/mcp-ui.md — 7th surface (HTMX Swagger UI for MCP, shipped)

docs/government/clawql-government.md — auditor air-gap export consumer
