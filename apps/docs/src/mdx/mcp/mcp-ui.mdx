MCPAdapter7th mcp-api-adapter surface. Auto-scaffolded HTMX playground at /mcp-ui — forms from inputSchema, inline results, no separate frontend. Templates cover ClawQL search, memory\_\*, cache, and audit. Source: docs/mcp/mcp-ui.md. Related: mcp-api-adapter · Protocol Fabric · QR stream (8th, planned).On this pageScreenshots1. What this is2. Why not the existing MCP UIs3. Surface contract4. ClawQL chain (the real product story)5. Implementation notes (draft)6. Non-goals (v1)7. Acceptance (draft)8. Relationship to other surfacesFurther reading
...
Implementation: packages/mcp-api-adapter/src/mcp-ui-\_.ts — catalog page + execute fragment, form UX (required/optional/defaults/Advanced), templates for search / memory\_\_ / cache / audit. Deferred: nested object/array UIs, file upload/IDP, SSE progress, agent-generated UIs, ATR scoping.
...
Standalone playground site / extension

User must go somewhere else or install something else

SEP-1865 MCP Apps

Server authors build UI resources; not auto from inputSchema
...
required attribute + visual marker
...
Expose the resulting tools through mcp-api-adapter.

Explore every operation in a browser at /mcp-ui — forms, buttons, inline results.
...
Full SEP-1865 MCP Apps host

Different problem (server-authored UI resources)
...
Utility HTML first; theme later

Multi-server Desktop config matrix

One upstream per adapter process

Untrusted HTML from tool results

Escape by default

7. Acceptance (draft)

mcp-api-adapter --stdio -- &#x3C;server> → GET /mcp-ui lists every tool from ListTools.

Each tool card’s form fields match top-level inputSchema properties (with JSON fallback).

Submit invokes the same call path as POST /\{toolName\} and renders a result fragment.

With API key set, /mcp-ui requires the same auth as /docs.

--no-mcp-ui removes the surface; /docs and /graphiql still work.
...
User guide: mcp-api-adapter.md

Design: ../design/mcp-api-adapter.md

Essay draft: ../gtm/pragmaticvectors/mcp-api-adapter.md

Protocol Fabric: protocol-fabric.md

QR (8th surface, planned): ../streams/clawql-qr-stream-transport.md
