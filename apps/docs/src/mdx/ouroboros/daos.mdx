OuroborosVision / roadmapDAOS v2.7Vision &amp; roadmap document. NSV, SGDOP, model fingerprinting, the Coordinator, and related DAOS coordination primitives are not shipped yet. The shipped Ouroboros library provides the evolutionary loop only. Generated from docs/ouroboros/daos-unified-architecture-specification-v2.7.md on main. Deep dives: Coordination layer, Build plan v2.7.1, Ouroboros library.
...
Trust anchor &#x26; verifiable artifacts

clawql-release, Arweave, Manifests, clawql-manifest-validator
...
Radicle + GitHub mirror

2

Execution &#x26; Intelligent Gateway

PEP &#x26; unified safe execution

clawql-api, clawql-core, clawql-auth, ActionType enforcement

3

Memory &#x26; Documents

Persistent hybrid knowledge + semantic pruning + pre-pruning snapshots

clawql-memory, clawql-documents, PageIndex

4

Strategic Coordination

Diversity, convergence control, evolution, circuit breaker

clawql-ouroboros evolutionary loop (shipped); DAOS coordination — NSV + SGDOP + model fingerprinting + Diversity Dividends (roadmap)

5

Security &#x26; Compliance

Zero-trust + air-gap breakout

ATRClaims, Presidio, WORM audit, Tetragon

6

Observability &#x26; Runtime Protection

Visibility + enforcement + Watchdog + Command Deck

LGTMP stack + Langfuse + Beyla + Falco
...
PEP Phase 2 passes, two-phase-commit required

STAGED

Any Phase 2 check fails → reject, no state created

STAGED

Command Deck opens Action View
...
Operator confirms with valid quorum

EXECUTED

Quorum not met → remain AWAITING_APPROVAL

AWAITING_APPROVAL

Operator cancels or TTL expires
...
WORM write fails → rollback to CANCELLED
...
WORM write fails → retry with backoff, alert operator
...
Bit-perfect cold-storage snapshots

Fidelity-weighted (see below)
