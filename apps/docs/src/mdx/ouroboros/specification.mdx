...
max*msgs_per_subject=200, max_age=7 * 24 \_ 60 \* 60,
...
V*pool_new = (1 - eta) * V*pool_old + eta * V

The full update, given Evaluator verdict V in \{0, 1\}:

delta*w_i = gamma * (S*i - S_bar) * (V - V_pool)
...
delta*w = gamma * (s*i - s_bar) * (verdict - v_pool)
...
delta*D_i = delta_d * VariancePenalty*i * IsolationScale_i [if AccrueDividend_i]
