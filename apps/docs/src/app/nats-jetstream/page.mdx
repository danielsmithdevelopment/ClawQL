...
NATS JetStream is available as an optional in-cluster event backbone in the `manifests/charts/clawql-mcp` Helm chart. In the [Zero-Trust Agentic Fabric](/architecture/agentic-fabric), each **Dedicated Virtual Gateway** hosts JetStream (with Valkey) so Edge Gateways and agents publish/subscribe for swarm workflows, Ouroboros events, and org-task broadcasts. **Document IDP async path:** [NATS IDP pipeline (Learn)](/learn/nats-idp-pipeline) · **KEDA worker scale:** [nats-keda-worker.md](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/deployment/nats-keda-worker.md).
...
Use stable subjects so multiple components can interoperate (mirrors **`nats.subjectConvention`** in **`manifests/charts/clawql-mcp/values.yaml`**):
...
helm template test ./manifests/charts/clawql-mcp -n clawql --set nats.enabled=true | rg "nats|jetstream" -n
```
