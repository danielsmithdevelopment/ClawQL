...
**Practical takeaway:** for predictable handoff across threads, run ClawQL as **one shared Streamable HTTP** (or gRPC) server—see [Quickstart](/quickstart#install-options) and [Deployment](/deployment)—and point every MCP client at **that** endpoint. Then **`cache`** is a reliable bridge between conversations until the server restarts.
...
Step-by-step pattern (writable vault dir or PVC): [clawql-memory (Memory 2.0)](/learn/memory#hands-on-memory-between-chats).
