...
It is a **library** you `import` from Node ≥ 22. In **`clawql-mcp`**, Ouroboros MCP tools **always register** via **`clawql-harness`** (`createOuroborosHarnessPlugin` / `makeHarnessLayer`): **`clawql_think`**, **`ouroboros_create_seed_from_document`**, **`ouroboros_run_evolutionary_loop`**, **`ouroboros_get_lineage_status`**, and **`ouroboros_measure_drift`**. For durable lineages, configure Postgres with **`CLAWQL_OUROBOROS_DATABASE_URL`** (or split **`CLAWQL_OUROBOROS_DB_*`** env vars). **`CLAWQL_ENABLE_OUROBOROS`** is **deprecated** as a registration gate (ignored for tool load). Langfuse eval remains opt-in via **`CLAWQL_ENABLE_LANGFUSE_EVAL`**.

**Learn walkthrough** (three tools together, benefits, full MCP example): [Ouroboros tools (seed, loop, lineage)](/learn/ouroboros-tools).
...
| `clawql-ouroboros/mcp-hooks` | `ouroborosMcpTools` — schemas + handlers for **embedding** in your own MCP host (not `clawql-mcp` registration; production uses `clawql-harness`) |
...
**`clawql-mcp`** always composes **`makeHarnessLayer(createOuroborosHarnessPlugin)`**, so **`listTools`** includes:
...
These are standard MCP tools exposed over stdio, Streamable HTTP, and optional gRPC. Do **not** use **`clawql-ouroboros/mcp-hooks`** as the registration path for **`clawql-mcp`** (that export is for embedding the library in your own server).

### 1) Durable lineage (optional)

Set **`CLAWQL_OUROBOROS_DATABASE_URL`** (or split `CLAWQL_OUROBOROS_DB_*`) for durable events in Postgres. Without database config, lineages are in-memory (process-local, cleared on restart).
...
- **Tools missing from ListTools**
  - Confirm you are on **clawql-mcp 8.0+** (harness always registers Ouroboros tools). Older docs that required `CLAWQL_ENABLE_OUROBOROS=1` are obsolete for registration.
...
- [Coordination layer specification](/ouroboros/specification) — NATS handoff, NSV/SGDOP, Diversity Dividends (math deep dive).
- [DAOS build plan v2.7.1](/ouroboros/build-plan) — P0–P3 implementation contract.
...
- [mcp-tools.md — Ouroboros section](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/mcp/mcp-tools.md#ouroboros_-optional) — tool names, flags, and persistence notes.
