...
| WORM replication       | Postgres / JSONL (K8s) or DO storage                   | On celld: **LTX → S3 replicates DO SQLite (RPO=0 platform durability)** for cell state; **compliance WORM remains host [`clawql-audit`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/audit/clawql-audit-spec-v0.1.md)** (tip-load / dual-ack) — Lab 5b dual-writes via `CLAWQL_AUDIT_WORM_URL` |
...
**Shipped today** ([`docs/examples/streams-celld`](../../docs/examples/streams-celld/)): Gateway / Subscription / AgentSession DOs, in-process `clawql-core/streams-slim`, optional `fetch(CLAWQL_MCP_URL)` + `fetch(CLAWQL_MCP_ADAPTER_URL)`, LTX audit flush. Evidence matrix: [`streams-celld-evidence.md`](/streams/streams-celld-evidence).
...
It is **not** a substitute for [`clawql-audit`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/audit/clawql-audit-spec-v0.1.md) `WORMAuditTrail` (tip-load on restart, dual-ack, Merkle). Compliance-grade forensic events for Streams sessions must append to the host trail (Lab 5b: `fetch(CLAWQL_AUDIT_WORM_URL)` → `POST /entries`). An operator reading only LTX `audit:ring` rows does **not** get no-fork-on-restart guarantees.
...
| `clawql-mcp` / Core  | **Out of process** — `fetch(CLAWQL_MCP_URL)` Streamable HTTP for search/execute/memory\_\* |
