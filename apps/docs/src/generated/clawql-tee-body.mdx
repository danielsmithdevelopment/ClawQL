...
**Hands-on walkthrough:** [Streams getting started (Learn)](https://docs.clawql.com/learn/streams-getting-started) — **§5** TEE + [air-gap audit](/streams/clawql-tee-airgap-audit) reading order; Labs 1–3 for event backbone today.

**Status:** Draft · August 2026 · v0.1  
**Builds on:** [`clawql-cellrt`](/streams/clawql-cellrt) v0.1 (software security baseline)  
**Companions:** [`clawql-tee-airgap-audit.md`](/streams/clawql-tee-airgap-audit) · [`clawql-streams`](/streams/clawql-streams) v0.2 · [`clawql-inference`](/inference/clawql-inference) · **correctness stack:** [`correctness-by-construction.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/design/correctness-by-construction.md) (NASA/SPARK → Rust + TLA+)
...
3. **Per-call capability accounting (planned)** — WORM records `capabilitiesExercised[]`; success via undeclared ambient host power is a Gate 3 failure ([security-ontology knowledge loop](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/security/security-ontology-knowledge-loop.md) §2.3)
...
6. **QR air-gap transport** — audit reaches verifier on a channel the operator cannot influence ([spec](/streams/clawql-tee-airgap-audit))
...
4. **Display path trust.** Compromised display driver can show false QR codes — document limitation; prefer TEE→display TCB when available ([air-gap §7](/streams/clawql-tee-airgap-audit#7-threat-model-coverage)).

---

## Further reading

- [`docs/streams/clawql-cellrt.md`](/streams/clawql-cellrt) — owned cell runtime
- [`docs/streams/clawql-tee-airgap-audit.md`](/streams/clawql-tee-airgap-audit) — QR air-gap audit transport
- [`docs/streams/clawql-streams.md`](/streams/clawql-streams) — event-driven agent sessions
- [`docs/security/clawql-security-defense-in-depth.md`](/security/defense-in-depth)
