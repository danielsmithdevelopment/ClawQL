...
**Schema vs data:** Ontology entity _definitions_ and static knowledge belong in **Git** (small, PR-reviewed). Dynamic **`Memory/`** instances and generated indexes belong in the **bucket** (unbounded growth). Do not put the full vault in GitHub — see [Enterprise Ontology — Git vs R2](/architecture/enterprise-ontology#git-vs-r2--what-lives-where) and [ADR 0009](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/adr/0009-enterprise-ontology.md).
...
See [`infra/packer/README.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/infra/packer/README.md) for GCP and CI validate targets. Use a **clawql-mcp** version that includes `clawql gateway` (Managed Edge Gateway).
