 ...
 *
 * Chart copy: manifests/charts/clawql-mcp/files/openclaw-chat-bridge.mjs (keep in sync with this file).
 *
 ...
 *
 * Usage:
 *   cd apps/dashboard && node scripts/openclaw-chat-bridge.mjs
...
function loadRepoRootEnvDotfile() {
  let envPath
  try {
    envPath = path.join(findRepoRoot(path.dirname(fileURLToPath(import.meta.url))), '.env')
  } catch {
    return
  }
