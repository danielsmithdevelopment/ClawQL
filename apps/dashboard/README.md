...
**Full docs:** [`docs/dashboard/agent-chat.md`](../../docs/dashboard/agent-chat.md) — API, SSE, vault schema, agent JSON contract, Helm, troubleshooting.
...
~/.ClawQL/                          # CLAWQL_OBSIDIAN_VAULT_PATH (default)
  ...
  Dashboard/
    chats/
      ...
      threads/
        <storageKey>/                  # digits only (thread id is thread-<storageKey>)
...
**Kubernetes / Rancher:** do not rely on `.env` in the image — set **`dashboard.openclawChatUrl`** in Helm so the Deployment gets **`CLAWQL_DASHBOARD_OPENCLAW_CHAT_URL`**, or leave it empty when **`openclaw.chatBridge.enabled`** (default) auto-wires to the OpenClaw sidecar (`http://<release>-openclaw.<ns>.svc.cluster.local:8787/v1/chat`). Start from **`manifests/charts/clawql-mcp/values-rancher.example.yaml`** and **`docs/deployment/helm.md`**.
...
## Unit tests (Agent Chat + bridge)

```bash
cd apps/dashboard
npm run test          # vitest (libs) + node:test (bridge enrich scripts)
npm run test:unit     # vitest only
npm run test:scripts  # openclaw-chat-enrich + reply-from-json
```

Covers upstream normalization, vault message parsing, SSE parsing, bridge session enrichment (Paperless/Stirling/Onyx/vault citations), and multi-turn session scoping. CI runs `npm run test --prefix apps/dashboard` after root vitest.

**Playwright (UI):** `e2e/agent-chat.spec.ts` includes mocked-API tests that assert tool steps, Paperless cards, and citations render (SSE + JSON paths). Requires dev server on port 3040 (`npm run dev`) or `CLAWQL_DASHBOARD_E2E=1` for production build on 3041.
