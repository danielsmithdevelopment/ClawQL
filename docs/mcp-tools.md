...
**Feature tiers (diagram-aligned):** **[`docs/readme/configuration.md` § Feature tiers](readme/configuration.md#feature-tiers-architecture-diagram)** — **ClawQL Core (no opt-out):** **`search`**, **`execute`**, **`audit`**, **`cache`**; **default on, opt out:** **`memory_*`**, **Documents** (`ingest_external_knowledge`, **`knowledge_search_onyx`** when **`CLAWQL_ENABLE_ONYX=1`**); **default off, opt in:** **`sandbox_exec`** (**`CLAWQL_ENABLE_SANDBOX=1`**), **`schedule`**, **`notify`**, **`ouroboros_*`**, Onyx wrapper (plus documents on).
