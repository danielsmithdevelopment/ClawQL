...
| Stage                    | Tool                                                                                                                         | Why                                                                    |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Catalogue / path flags   | DMS paths + Fix 7 `detect_credit_facility` / `detect_hsr_filing` / `detect_hsr_second_request` / `detect_billion_ma_signals` | Cheap credit, secured, HSR, second-request, antitrust-practice signals |
...
Matter {
  ...
  is_antitrust_matter,              # path signal (ontology)
  has_antitrust_practice,           # Antitrust/FTC/HSR folder OR ≥2 substantive docs OR SR
  has_equity_purchase_agreement,    # executed EPA (005 Cascadia-class)
...
| **005**     | view `billion_dollar_antitrust_ma`                  | SR rate **4/7 (~57%)** on full DMS                 |
...
**005 note:** population is the full-DMS view `billion_dollar_antitrust_ma`:
¬credit ∧ deal_value≥$1B ∧ (`has_antitrust_practice` ∨ `has_equity_purchase_agreement`).
Practice = dedicated Antitrust/FTC/HSR/DOJ folder, or ≥2 substantive antitrust/HSR
docs (excluding preparation/conflicts), or second-request received. EPA covers
infrastructure acquisitions without an Antitrust folder. Deal value prefers
defined TEV / Purchase Price (not AUM). Gold IDs appear only in pipeline asserts
(rate 4/7 + precision allow-list); extractors do not seed them.
...
- Automatic 017 population discovery without POP lists in the gold test.
