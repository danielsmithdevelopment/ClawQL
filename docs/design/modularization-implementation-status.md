...
| `effect/Schema` at MCP boundaries                                                | 🚧 Pilot: `search` / `execute` decode via `SearchInputSchema` / `ExecuteInputSchema`; thin Zod edge in `search-execute-zod-edge.ts`. Next: cache/audit → memory → documents → automation → registry types |
