...
| Extracted packages (`memory`, `documents`, `automation`, `sandbox`, `ouroboros`) | 🚧 Layer wrappers shipped; memory ingest/recall, documents IDP + classify/extract + external ingest, automation notify/schedule/workflow/argocd, + ouroboros hot paths use native `Effect.gen` (IO still `tryPromise`) |
