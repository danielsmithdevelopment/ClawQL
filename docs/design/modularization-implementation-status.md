...
| `effect/Schema` at MCP boundaries                                                | 🚧 `search` / `execute` / `cache` / `audit` / `memory_ingest` / `memory_recall` / `memory_sync` decode via Effect Schema; thin Zod edges for MCP SDK listing. Next: documents → automation → registry types |
