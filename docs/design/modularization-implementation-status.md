...
| `effect/Schema` at MCP boundaries                                                | 🚧 `search` / `execute` / `cache` / `audit` decode via Effect Schema (`*-input-schema.ts` / `search-execute-schema.ts`); thin Zod edges only for MCP SDK listing. Next: memory → documents → automation → registry types |
