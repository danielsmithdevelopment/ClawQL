...
It composes horizontals ClawQL already ships (payments rails, document IDP, vault memory, automation/HITL) and adds banking-shaped workflows, OpenAPI provider presets, and operator defaults for teams that want agents to operate _inside_ a regulated deposit/spend product.
...
KYC is an **identity + document + case workflow** problem. Payments cares about _whether a funding instrument is usable_ and _audit of money movement_ — not collecting IDs, running watchlists, or storing biometric evidence.
...
### What _may_ live in payments (thin)
