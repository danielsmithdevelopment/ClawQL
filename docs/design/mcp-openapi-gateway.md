...
```text
OpenAPI clients / Workers / OpenWebUI
        │
        ▼
  mcp-openapi-gateway
   GET  /openapi.json
...
```text
packages/mcp-openapi-gateway/
  package.json                 # name: mcp-openapi-gateway
  README.md
  src/
    index.ts                   # public exports
    cli.ts                     # `mcp-openapi-gateway` / `npx` entry
...
- **Refresh:** env `MCP_OPENAPI_GATEWAY_REFRESH_MS` (optional poll) and/or `POST /admin/refresh` (auth-gated) when tools change (`listChanged`).
