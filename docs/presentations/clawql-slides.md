...
**Verify shipped vs roadmap:** [`docs/clawql-ecosystem.md`](../clawql-ecosystem.md) (appendix **Fiction and roadmap**) and [`docs/security/clawql-security-defense-deliverables.md`](../security/clawql-security-defense-deliverables.md).

**2026 roadmap (both forks — not all flags exist in this repo today):** public MCP endpoint with **x402 / MPP** micropayments; agentic wallets (**Coinbase AgentKit** + **ERC-4337**); IPFS pinning; blockchain anchoring; **The Graph** (subgraph + x402 where applicable); **Chainlink** (CRE, oracles, **CCIP**, Functions, x402-native patterns); **Hyperledger Fabric** as a permissioned layer ([#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187) — **no** `CLAWQL_ENABLE_FABRIC` or `providers/fabric` in the open-source tree yet).
...
- **Hyperledger Fabric (roadmap):** permissioned provenance, private channels, RWA registration — design only in-repo today ([#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187))
...
- **Hyperledger Fabric (roadmap):** target model is peers, orderers, CouchDB, chaincode as an optional Helm sub-chart and bundled provider — **not shipped** in this repository ([#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187)); same `search` / `execute` story when it lands.
...
- **Hyperledger Fabric** (roadmap [#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187)) — peer / orderer / CA and optional REST gateway; target: **`search`/`execute`** to chaincode like any bundled provider
...
_**13** bundled vendor specs (+ Google preset) · **12+** runtime services in one **Helm** release · **optional Istio** + **Kiali** · **Trivy** + **OSV-Scanner** in **Golden Image** · **Vault**/ · Privacy- and **zero-trust**-first._
...
## Slide 38 — Hyperledger Fabric — Roadmap Layer
...
- **Non-breaking & no drift** — Same `search` + `execute` model; target bundled provider (`providers/fabric/openapi.yaml` + ABIs) like everything else.
...
clawql:
  web3:
    x402:
      enabled: true # roadmap
  fabric:
    enabled: true # roadmap — see issue #187
...
- **Fabric (roadmap):** when [#187](https://github.com/danielsmithdevelopment/ClawQL/issues/187) lands, chart/env toggles would wire peers, orderer, CA, and CouchDB from a **Fabric sub-chart**.
