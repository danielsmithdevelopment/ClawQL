...
- **Click-to-claim preset:** `GET /mcp-ui/presets/cloudflare-claim` + `POST …/start` (or `POST /mcp-ui/generate` with `{ "preset": "cloudflare-claim" }`) wraps third-party page WebMCP tools (`cf_reveal_challenge` → `cf_claim_coupon`) as a human **Click to claim** button. Demo: `examples/mcp-api-adapter/cloudflare-claim-server.mjs` — Chrome CDP → `document.modelContext` → thin MCP proxy → `/mcp-ui` (coupon logic stays on `cloudflare-claim/site.html`). Optional Core index: `clawql sources add <url> --kind webmcp`.
