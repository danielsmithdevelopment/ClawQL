...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 WebMCP      | Live `clawql.site.*` tools on clawql.com (or local `landing-page/demo`); optional `clawql sources add <url> --kind webmcp` when CDP WebMCP is available | Show browser `registerTool` in DevTools / agent host; or local page `docs/examples/mcp-api-adapter/cloudflare-claim/site.html` (tools on `document.modelContext`) |
...
3. **Optional (Protocol Fabric):** when Chromium WebMCP + CDP is available, index the page as a ClawQL source:
   ...
   ```
   Without CDP, Act 1 still works as page-local `registerTool` (see `WebMcpRegister.tsx` / `docs/examples/mcp-api-adapter/cloudflare-claim/site.html`).

**Pass:** navigate + page_context return sensible JSON (or mock `cf_*` tools appear in the adapter catalog).

**Demo path:** `node docs/examples/mcp-api-adapter/cloudflare-claim-server.mjs` launches Chrome CDP, proxies the page’s `document.modelContext` tools as MCP, and serves `/mcp-ui/presets/cloudflare-claim` (prove with `http://127.0.0.1:8765/__webmcp/page-state`).
...
node docs/examples/mcp-api-adapter/docs-agent-lab-server.mjs
...
```bash
npm run build -w mcp-grpc-transport -w mcp-api-adapter
node docs/examples/mcp-api-adapter/cloudflare-claim-server.mjs
