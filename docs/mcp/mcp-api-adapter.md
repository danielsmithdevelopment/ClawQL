...
**Changelog:** [`packages/mcp-api-adapter/CHANGELOG.md`](../../packages/mcp-api-adapter/CHANGELOG.md)
...
This is the inverse of ClawQL Core **`search` / `execute`** (those turn OpenAPI ops into MCP tools). Here, MCP tools become APIs (and can re-export MCP itself). Distinct from ClawQL’s **Agentic Gateway** branding — this package only adapts MCP tools onto call surfaces.
...
# Streamable HTTP MCP (same tools) for IDE / agent clients
# Connect your MCP client to http://127.0.0.1:8090/mcp
...
Everything after `--` is the child command. The adapter keeps the stdio session open, serves REST / GraphQL / `/mcp`, and (unless `--no-grpc`) starts a local gRPC MCP bridge that delegates to the same session.
...
No second gRPC server is started; `/openapi.json` advertises the upstream address in `info.x-clawql-grpc`. REST, GraphQL, and `/mcp` all forward into upstream `CallTool` (content is normalized to MCP `{ type: "text", text }` blocks for SDK clients).

### 4. Generate a thin CLI (`gen-cli`)

```bash
npx mcp-api-adapter gen-cli --out ./my-cli \
  --stdio -- npx -y @modelcontextprotocol/server-everything

# Point the generated CLI at a running adapter:
MCP_API_ADAPTER_URL=http://127.0.0.1:8090 node ./my-cli/bin/mcp-tools.mjs list
MCP_API_ADAPTER_URL=http://127.0.0.1:8090 node ./my-cli/bin/mcp-tools.mjs echo --message hi
```

`gen-cli` writes a **zero-dependency** Node package that `POST`s to `/{toolName}` on a running adapter. When **clawql-printingpress** ships, prefer it for signed binaries and richer packaging — same tool catalog.
...
const adapter = await startMcpApiAdapter({
  ...
  host: "0.0.0.0",
  port: 8090,
  mcpPath: "/mcp", // or false to disable
  grpcListen: "127.0.0.1:50051",
  apiKey: process.env.MCP_API_ADAPTER_API_KEY,
});

// adapter.url — OpenAPI + GraphQL + /mcp
// adapter.mcpPath — e.g. "/mcp" when enabled
