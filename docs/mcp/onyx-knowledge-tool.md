...
**Tracking:** Onyx epic [#118](https://github.com/danielsmithdevelopment/ClawQL/issues/118) (closed) · **Post-Paperless index push (workflow + `execute`):** [#120](https://github.com/danielsmithdevelopment/ClawQL/issues/120) — bundled **`onyx_ingest_document`** + optional default **Ouroboros** hook (`CLAWQL_OUROBOROS_ONYX_AFTER_PAPERLESS` + seed **`metadata.onyx_ingest_after_paperless`**) · **Vault citation handoff:** [#130](https://github.com/danielsmithdevelopment/ClawQL/issues/130) (closed: **`enterpriseCitations`** + helpers) · **Further tests (live/VCR, transport parity):** [#144](https://github.com/danielsmithdevelopment/ClawQL/issues/144) · **Deck / slides accuracy:** [#145](https://github.com/danielsmithdevelopment/ClawQL/issues/145) · Implementation: [`packages/clawql-documents/src/plugin/knowledge-search-onyx.ts`](../../packages/clawql-documents/src/plugin/knowledge-search-onyx.ts), [`providers/onyx/openapi.yaml`](../providers/onyx/openapi.yaml), [`packages/clawql-ouroboros/src/glue/default-engines.ts`](../../packages/clawql-ouroboros/src/glue/default-engines.ts) · Full MCP matrix: **[mcp-tools.md](mcp-tools.md)**.
...
Use **`knowledge_search_onyx`** when the task is **“find indexed company knowledge”**. Use **`execute`** when you need **other Onyx REST paths** from a larger checked-in OpenAPI (if you replace or extend the bundled spec). For **post-Paperless indexing**, call **`execute("onyx::onyx_ingest_document", …)`** with an ingestion-shaped body, or enable the **default Ouroboros executor** bridge described below.

### Post-Paperless → Onyx (default Ouroboros executor)

When **`CLAWQL_OUROBOROS_ONYX_AFTER_PAPERLESS`** is set (for example `1`) **and** the seed’s **`metadata.onyx_ingest_after_paperless`** is **`true`**, the executor looks at the last successful **Paperless** **`execute`** step, parses JSON from its **`result`** text for a document **`id`**, and appends **`onyx::onyx_ingest_document`** with a minimal **`document`** payload (`source: ingestion_api`, **`semantic_identifier`** / **`id`** stable under `paperless-<id>`). Optional **`CLAWQL_ONYX_CC_PAIR_ID`** (digits only) is forwarded as **`cc_pair_id`**. A failing Onyx step is recorded as **`kind: partial_failure`** so the Paperless outcome is still visible; tune acceptance criteria accordingly.
...
- **Unit / handler tests:** [`packages/clawql-documents/src/plugin/knowledge-search-onyx.test.ts`](../../packages/clawql-documents/src/plugin/knowledge-search-onyx.test.ts), [`src/server.test.ts`](../src/server.test.ts) (tool appears when **`CLAWQL_ENABLE_ONYX=1`**).
