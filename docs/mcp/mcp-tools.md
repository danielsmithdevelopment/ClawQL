...
| `data_query`, `clawql_sql`, `data_ingest`, `data_status`                                                                                                                                                                                    | **`CLAWQL_ENABLE_DATA=1`**                                                                                                                                                        | Node DuckDB structured SQL (`packages/clawql-data`). **`clawql_sql` is an alias of `data_query`** for Hermes/Cline / Harvey LAB tool names. Not Python `duckdb`. Not chDB. Read-only `SELECT`/`WITH`. Ingest `matters` / `matter_documents`.                                                                                                                                                                                                                                                                                                                                    |
| `memory_ingest`                                                                                                                                                                                                                             | _default on_; set **`CLAWQL_ENABLE_MEMORY=0`** to hide; `CLAWQL_OBSIDIAN_VAULT_PATH` (writable) for I/O                                                                           | Write Obsidian Markdown under `Memory/`; optional **`enterpriseCitations`** (short Onyx-style rows, [#130](https://github.com/danielsmithdevelopment/ClawQL/issues/130)); optional **`rebuild.pageindex`** / **`rebuild.embeddings`**; refreshes **`memory.db`** when enabled; optional **`_INDEX_*.md`** hub page ([#38](https://github.com/danielsmithdevelopment/ClawQL/issues/38)); when **`CLAWQL_MERKLE_ENABLED`** / **`CLAWQL_CUCKOO_ENABLED`**, JSON may include **`merkleSnapshotBefore`**, **`merkleSnapshot`**, **`merkleRootChanged`**, **`cuckooMembershipReady`** |
| `memory_recall`                                                                                                                                                                                                                             | _default on_; set **`CLAWQL_ENABLE_MEMORY=0`** to hide; `CLAWQL_OBSIDIAN_VAULT_PATH` (readable) for I/O                                                                           | Multi-source recall via optional **`sources`**: `vault` \| `vector` \| `codegraph` \| `pageindex` \| `onyx` → normalized **`hits[]`** + **`followUps`**; vault **`results[]`** kept for compatibility; keyword + `[[wikilink]]` hops; optional **vector KNN**; hybrid envs **`CLAWQL_MEMORY_RECALL_HYBRID_*`**; see [memory plugin](../plugins/memory.md)                                                                                                                                                                                                                       |
| `memory_sync`                                                                                                                                                                                                                               | _default on_; set **`CLAWQL_ENABLE_MEMORY=0`** to hide; **`CLAWQL_SYNC_BUCKET`** + credentials (R2/S3/GCS)                                                                        | Reconcile local vault with remote team bucket: default **`auto`** = pull then push; optional **`direction`**: `pull` \| `push`; **`force`** for conflicts; **`dryRun`** to plan only — use from Cloud Agents instead of shell **`clawql sync push/pull`**                                                                                                                                                                                                                                                                                                                       |
| `pageindex_build_tree`, `pageindex_traverse`, `pageindex_synthesize`, `pageindex_get_content`                                                                                                                                               | _default on_; **`CLAWQL_ENABLE_PAGEINDEX=0`** hides; memory tier on; vault path for stored trees                                                                                  | Vectorless hierarchical indexing via **`clawql-pageindex`** — [Memory plugin](../plugins/memory.md) · [PageIndex](../../packages/clawql-pageindex/README.md)                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `codegraph_index`, `codegraph_sync`, `codegraph_explore`, `codegraph_impact`, `codegraph_sync_graphify`, `codegraph_import_graphify`, `codegraph_query`, `codegraph_neighbors`, `codegraph_path`, `codegraph_explain`, `codegraph_subgraph` | **`CLAWQL_ENABLE_CODEGRAPH=1`**; memory tier on                                                                                                                                   | Structural code graph via **`clawql-codegraph`** — [codegraph plugin](../plugins/codegraph.md); preferred native sync: **`codegraph_sync`**; hybrid: **`CLAWQL_MEMORY_RECALL_HYBRID_CODEGRAPH=1`**                                                                                                                                                                                                                                                                                                                                                                              |
...
| `ouroboros_create_seed_from_document`, `ouroboros_run_evolutionary_loop`, `ouroboros_get_lineage_status`, `ouroboros_measure_drift`, `clawql_think`                                                                                         | Always on via **`clawql-harness`** (`createOuroborosHarnessPlugin` / `makeHarnessLayer`); optional **`CLAWQL_OUROBOROS_DATABASE_URL`** for Postgres-backed events | Harness plugin registers tools; MCP bridges them ([#141](https://github.com/danielsmithdevelopment/ClawQL/issues/141)): seed-from-document, run evolutionary loop, read lineage, **3-component drift** ([#557](https://github.com/danielsmithdevelopment/ClawQL/issues/557)), plus `clawql_think`. Without **`CLAWQL_OUROBOROS_DATABASE_URL`**, events stay in-memory ([#142](https://github.com/danielsmithdevelopment/ClawQL/issues/142)). **`CLAWQL_ENABLE_OUROBOROS`** is deprecated as a registration gate. |
| `ouroboros_propose_seed_revision_from_eval`                                                                                                                                                                                                 | **`CLAWQL_ENABLE_LANGFUSE_EVAL=1`** (tools already loaded via harness); webhook **`CLAWQL_LANGFUSE_WEBHOOK_TOKEN`** (prod)                                                         | Langfuse eval → seed revision gate ([#250](https://github.com/danielsmithdevelopment/ClawQL/issues/250)); **`POST /observability/langfuse/webhook`**; default dry-run — **`CLAWQL_LANGFUSE_EVAL_AUTO_APPLY=1`** to apply; see **[langfuse-eval-ouroboros.md](langfuse-eval-ouroboros.md)**.                                                                                                                                                                                                                                                                                     |
...
Boolean flags are parsed in one place — **[`packages/clawql-api/src/config/optional-flags.ts`](../../packages/clawql-api/src/config/optional-flags.ts)** ([GitHub #79](https://github.com/danielsmithdevelopment/ClawQL/issues/79)) — so env wiring stays consistent. **Truthy** means `1`, `true`, or `yes` (case-insensitive) when set. **Exception:** **`CLAWQL_ENABLE_MEMORY`** and **`CLAWQL_ENABLE_DOCUMENTS`**: **unset = on**; set **`0`**, **`false`**, or **`no`** to **opt out** (hide tools / shrink **`all-providers`** for document vendors). The **`audit`**, **`cache`**, and harness **`ouroboros_*`** / **`clawql_think`** tools are not gated by env — they are always registered.
...
| **Code graph**                       | **`CLAWQL_ENABLE_CODEGRAPH=1`**, **`CLAWQL_CODEGRAPH_ROOT`**, **`CLAWQL_CODEGRAPH_PATH`**; optional Graphify backend vars                                                                                                                                                                        | off                         | Registers **`codegraph_*`** via **`MemoryPlugin`**; see [codegraph plugin](../plugins/codegraph.md). Hybrid recall: **`CLAWQL_MEMORY_RECALL_HYBRID_CODEGRAPH=1`**, **`CLAWQL_MEMORY_RECALL_CODEGRAPH_LIMIT`**, or `memory_recall` **`sources`**.                                                                                                                                                                                                                                                                                                                                                     |
| **Hybrid PageIndex / Onyx recall**   | **`CLAWQL_MEMORY_RECALL_HYBRID_PAGEINDEX`**, **`CLAWQL_MEMORY_RECALL_HYBRID_ONYX`**, optional per-source limits                                                                                                                                                                                  | off                         | Include **`pageindex`** / **`onyx`** in default **`memory_recall`** sources; Onyx needs **`CLAWQL_ENABLE_ONYX=1`** + wired search.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Ingest PageIndex rebuild**         | **`CLAWQL_MEMORY_INGEST_REBUILD_PAGEINDEX=1`** or **`rebuild.pageindex`** on **`memory_ingest`**                                                                                                                                                                                                 | off                         | Rebuild PageIndex tree for the written vault note after successful ingest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **`ouroboros_*` / `clawql_think` tools** | Always on via **`clawql-harness`**; optional `CLAWQL_OUROBOROS_DATABASE_URL` or split `CLAWQL_OUROBOROS_DB_*` vars. **`CLAWQL_ENABLE_OUROBOROS`** deprecated (not a registration gate).                                                                                                                                                                                        | always                      | [#141](https://github.com/danielsmithdevelopment/ClawQL/issues/141) / [#142](https://github.com/danielsmithdevelopment/ClawQL/issues/142): **`clawql_think`**, **`ouroboros_create_seed_from_document`**, **`ouroboros_run_evolutionary_loop`**, **`ouroboros_get_lineage_status`**, **`ouroboros_measure_drift`** ([#557](https://github.com/danielsmithdevelopment/ClawQL/issues/557)); Postgres config enables durable **`clawql_ouroboros_events`**. **`ouroboros_propose_seed_revision_from_eval`** when **`CLAWQL_ENABLE_LANGFUSE_EVAL=1`** ([#250](https://github.com/danielsmithdevelopment/ClawQL/issues/250)). |
...
## `ouroboros_*` (always on via `clawql-harness`)

**`clawql-mcp`** always composes **`makeHarnessLayer`** with **`createOuroborosHarnessPlugin`** (`clawql-harness/plugin`). That harness plugin registers **`clawql_think`**, **`ouroboros_create_seed_from_document`**, **`ouroboros_run_evolutionary_loop`**, **`ouroboros_get_lineage_status`**, **`ouroboros_measure_drift`** ([#557](https://github.com/danielsmithdevelopment/ClawQL/issues/557)), and (with **`CLAWQL_ENABLE_LANGFUSE_EVAL=1`**) **`ouroboros_propose_seed_revision_from_eval`** ([#250](https://github.com/danielsmithdevelopment/ClawQL/issues/250)); the harness → MCP bridge exposes them. Handlers return JSON as MCP **text** content. There is no separate MCP-only `makeOuroborosLayer` path in composition. **`CLAWQL_ENABLE_OUROBOROS`** is **deprecated** as a registration gate (do not require it for `listTools`). Do **not** use **`clawql-ouroboros/mcp-hooks`** as the **`clawql-mcp`** registration path — that export is for embedding the library in your own host.
