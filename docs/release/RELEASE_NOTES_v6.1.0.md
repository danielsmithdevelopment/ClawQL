...
- **`manifests/charts/clawql-mcp`:** **Chart.version `0.6.3`**, **`appVersion` `6.1.0`** (aligns with npm). Review **`values.yaml`** / **`values-docker-desktop.yaml`** when upgrading; platform changes are **deployment-oriented**, not another **Dragonfly**-style values migration.
