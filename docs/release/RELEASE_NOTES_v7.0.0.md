...
The Agentic Gateway speaks OpenAI-compatible **`/v1`** and MCP **`/mcp`**. Enterprise deployments follow the [Zero-Trust Agentic Fabric](../architecture/zero-trust-agentic-fabric.md): Regional Hubs (routing/billing), Dedicated Virtual Gateways (Audit-Trail Enforcement Points with NATS JetStream + Valkey), and Edge Gateways on developer machines.

→ Announcement drafts: [`docs/announcements/announcement-drafts-v7.0.0.md`](../announcements/announcement-drafts-v7.0.0.md) · GTM: [inference-first playbook](https://clawql.com/inference/gtm/)
...
# Helm:
helm upgrade --install clawql ./manifests/charts/clawql-mcp --set provider=all-providers
```
...
→ [vault-provider-secrets.md](../deployment/vault-provider-secrets.md)
...
→ [`.env.example`](../../.env.example) · [configuration.md](../readme/configuration.md)
...
**7.0.0 ships ADR + docs + env catalog** — runtime profile wiring is Phase 1 of [`7.0-observability-profiles-plan.md`](../observability/7.0-observability-profiles-plan.md). Existing signals unchanged: **`GET /metrics`** on by default; OTLP and Loki push remain opt-in via their env vars.

→ [ADR 0005](../adr/0005-langfuse-default-work-trace-store.md) · [bundled observability](../observability/bundled-observability.md) · [BYO observability](../observability/bring-your-own-observability.md)
...
```bash
cd apps/desktop && npm install && npm run dist:mac    # macOS .dmg
...
→ [Desktop design doc](../design/clawql-desktop-macos.md) · [desktop/README.md](../../apps/desktop/README.md)
...
→ [custom-sources.md](../getting-started/custom-sources.md)
...
→ [clawql-operator-helm.md](../deployment/clawql-operator-helm.md)
...
→ [clawql-release-mvp.md](../getting-started/clawql-release-mvp.md)
...
- **Tier 1 Compose** — `docs/examples/clawql-local-docker-compose` (`./bootstrap.sh`, `docker compose up`)

→ [modularization-implementation-status.md](../design/modularization-implementation-status.md)
...
- Shared doc link rewriter for generated MDX; consolidated nav via [`docs-nav-data.ts`](apps/docs/src/lib/docs-nav-data.ts)
- HITL: [`docs/mcp/hitl-label-studio.md`](../mcp/hitl-label-studio.md) is canonical; plugin index is a stub
