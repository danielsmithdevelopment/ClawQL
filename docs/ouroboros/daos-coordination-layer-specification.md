...
| Recruitment       | Escalation events with SGDOP blind-spot direction routed through marketplace, weighted by softmax over reputation (incorporating `w_floor_i` from Diversity Dividends). **Pay bridge (roadmap):** stage-only agent compensation via [`clawql-payments`](../payments/sgdop-coordinator-compensation-bridge.md) — Coordinator never confirms ledger / payout. |
