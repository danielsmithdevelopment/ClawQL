...
- **`memory_recall`** (`src/memory-recall.ts` shim → `packages/clawql-memory/recall/`) is **hybrid**: it always does **keyword scoring** over Markdown (body, headings, filenames) and **walks the wikilink graph** (forward + backward) up to **`CLAWQL_MEMORY_RECALL_MAX_DEPTH`**. Vector backend defaults to **`sqlite`**: **`memory_ingest`** embeds chunks **in-process** with a bundled ONNX MiniLM model (**no API key, no Ollama**). Set **`CLAWQL_VECTOR_BACKEND=off`** for keyword+wikilink only, or set **`CLAWQL_EMBEDDING_API_KEY`** / **`OPENAI_API_KEY`** to use an OpenAI-compatible HTTP embeddings API instead. Chunk vectors are ranked (SQLite BLOB KNN in-process and/or **Postgres + pgvector**), and hits can be seeded by similarity as well as keywords. Results include **`reason: "keyword" | "link" | "vector"`**. Defaults and tuning: **`CLAWQL_MEMORY_RECALL_*`**, **`CLAWQL_MEMORY_VECTOR_*`**, **`CLAWQL_VECTOR_*`**, **`CLAWQL_EMBEDDING_PROVIDER`** (`auto`/`local`/`http`/`off`) — see **[README.md](../../README.md)** and **[hybrid-memory-backends.md](hybrid-memory-backends.md)**.

If **`CLAWQL_VECTOR_BACKEND=off`**, recall stays **lexical + wikilinks only**. Otherwise (default **`sqlite`**) ingest embeds chunks locally in-process.
