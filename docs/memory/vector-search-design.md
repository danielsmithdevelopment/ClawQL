...
- **`memory_recall`:** **implemented** — hybrid lexical + vector seeds + wikilink graph; fusion via **`CLAWQL_MEMORY_VECTOR_*`** (canonical: **`packages/clawql-memory/recall/`**).
...
- [`mcp-tools.md`](../mcp/mcp-tools.md) — MCP tools reference (including **`memory_recall`** hybrid / vector behavior).
