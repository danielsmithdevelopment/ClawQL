...
| -------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Vectors (required for memory)**      | unset / `CLAWQL_VECTOR_BACKEND=sqlite` (default)                          | **Mandatory.** In-process MiniLM ONNX — no API key, no Ollama. Keyword-only recall measured worse than grep; do not disable.                                                                          |
| **Postgres + pgvector**                | `CLAWQL_VECTOR_BACKEND=postgres` + **`CLAWQL_VECTOR_DATABASE_URL`**       | Same mandatory embeddings (local or HTTP); pgvector for shared/HA storage.                                                                                                                            |
| **Keyword-only (break-glass / tests)** | `CLAWQL_VECTOR_BACKEND=off` **and** `CLAWQL_ALLOW_KEYWORD_ONLY_MEMORY=1`  | **Unsupported product mode.** Without the allow flag, `off` is ignored and sqlite+local is forced.                                                                                                    |
...
| Chunk vectors                                                                         | **`sqlite`** (mandatory default) — local MiniLM ONNX + KNN over `memory.db` BLOBs                                  | **`postgres`** + URL — pgvector; break-glass keyword-only requires `CLAWQL_ALLOW_KEYWORD_ONLY_MEMORY=1` |
