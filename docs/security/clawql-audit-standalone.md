...
[`clawql-merkle`](https://www.npmjs.com/package/clawql-merkle) and [`clawql-audit`](https://www.npmjs.com/package/clawql-audit) are published as an independent **0.1.x** wedge. You can use them from any Node ≥ 22 agent host without installing ClawQL MCP.

## Packages

| Package | Role | Semver |
| --- | --- | --- |
| `clawql-merkle` | Zero-dep Merkle snapshots + inclusion proofs | `0.1.x` |
| `clawql-audit` | Hash-chained WORM trail, dual-ack, TEE ECDSA, HTTP/QR | `0.1.x` |
...
Wedge-only (first registry publish of `0.1.x` without a monorepo tag):
