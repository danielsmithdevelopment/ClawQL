...
```bash
node docs/examples/mcp-api-adapter/demo-rest.mjs
node docs/examples/mcp-api-adapter/demo-graphql.mjs
node docs/examples/mcp-api-adapter/demo-grpc.mjs
node docs/examples/mcp-api-adapter/demo-all.mjs   # parity check
node docs/examples/mcp-api-adapter/clawql-with-trace.mjs   # live /mcp-ui/trace + shared inference JSONL
```

**PixelDrop smart-upload demo** (broken upload vs /mcp-ui fix):

```bash
cd docs/examples/mcp-api-adapter/pixeldrop && python3 -m http.server 8765
# open http://127.0.0.1:8765/smart-upload-test-harness.html
node docs/examples/mcp-api-adapter/pixeldrop/smoke-test.mjs
```
