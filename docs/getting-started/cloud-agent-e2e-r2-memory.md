...
| ClawQL MCP “failed during live tool discovery”                                             | Broken stdio launch: `npx -p clawql-mcp clawql-mcp`                                            | Prefer **`node /workspace/bin/clawql-mcp.mjs`** (this repo) or **`npx -y clawql-mcp`** (see §0); re-add MCP; new run                                       |
