...
- **Constant:** `SCHEMA_VERSION = 2` in `src/memory-db.ts` (not exported; bump when adding DDL).
- **Table `schema_migrations`:** `(version INTEGER PK, name TEXT, applied_at TEXT)`.
- **Bootstrap:** `migrate()` creates `schema_migrations` if missing, reads `MAX(version)`, and applies versioned DDL: **`1`** — `vault_document`, `vault_chunk`, `wikilink_edge`, indexes; **`2`** — optional **`clawql_cuckoo_chunk_membership`** and **`vault_merkle_snapshot`** (Cuckoo + Merkle artifacts).
...
| **`src/memory-backends/postgres-migrations.ts`**                         | Versioned DDL (`clawql_pg_schema_migrations`); migration **1** = chunk vectors; **2** = Cuckoo + Merkle artifact tables.                                                                                                                                                                                                        |
| **`src/memory-backends/types.ts`**                                       | Extension-point types (vectors + Cuckoo / Merkle backends).                                                                                                                                                                                                                                                                     |
| **`src/cuckoo-filter.ts`**, **`src/merkle-tree.ts`**                     | Cuckoo filter serialization + Merkle tree / proofs.                                                                                                                                                                                                                                                                             |
| **`src/memory-artifacts.ts`**                                            | Rebuild Cuckoo + Merkle rows in `memory.db`; optional Postgres mirror when **`CLAWQL_VECTOR_DATABASE_URL`** is set.                                                                                                                                                                                                             |
...
| **`CLAWQL_CUCKOO_ENABLED=1`**                                              | Rebuild a serialized **Cuckoo filter** over all **`chunk_id`** values after each sync ([#25](https://github.com/danielsmithdevelopment/ClawQL/issues/25)). Optional **`CLAWQL_CUCKOO_FINGERPRINT_BITS`** (default **12**, range **8–16**).                                                                                                                                                                 |
| **`CLAWQL_CUCKOO_METRICS=1`**                                              | **`chunkIdMaybeInMemoryIndex`:** on each lookup, verify filter-**maybe** keys against **`vault_chunk`** and update in-process counters (FPR among maybes). Skips the Cuckoo filter cache (extra DB open). **`GET /healthz`** with **`CLAWQL_HEALTHZ_MEMORY_ARTIFACTS=1`** exposes **`cuckooMetrics`** + **`cuckooFilterPersistedAt`**. ([#30](https://github.com/danielsmithdevelopment/ClawQL/issues/30)) |
| **`CLAWQL_MERKLE_ENABLED=1`**                                              | Store a **Merkle root** over documents (sorted `path` + `body_sha256`) in **`vault_merkle_snapshot`** ([#37](https://github.com/danielsmithdevelopment/ClawQL/issues/37)).                                                                                                                                                                                                                                 |
...
| **#25** | ~~Cuckoo membership layer~~ — **done** (`CLAWQL_CUCKOO_ENABLED`, `chunkIdMaybeInMemoryIndex`).          |
| **#30** | Observability: FPR, rebuild, multi-worker MCP HTTP.                                                     |
| **#37** | ~~Merkle root over documents~~ — **done** (`CLAWQL_MERKLE_ENABLED`, `loadVaultMerkleSnapshotFromDb`).   |
| **#28** | ~~Operator docs + optional tool-shape logging~~ — **done** (this section + **`CLAWQL_MCP_LOG_TOOLS`**). |
