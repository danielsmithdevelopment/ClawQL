...
| Path                                                              | Status                                    |
| ----------------------------------------------------------------- | ----------------------------------------- |
| `benchmarks/harvey-labs/harness/adapters/clawql.py`             | Anthropic + ClawQL (Arms B)               |
| `benchmarks/harvey-labs/harness/adapters/clawql_chat.py`        | OpenRouter chat + ClawQL (Arm C)          |
| `benchmarks/harvey-labs/harness/adapters/clawql_lab_session.py` | Node MCP proxy subprocess (~100 lines)    |
| `benchmarks/harvey-labs/scripts/lab-pre-ingest.mjs`             | Node vault seed + MCP `data_ingest`       |
| `benchmarks/harvey-labs/scripts/lab-mcp-proxy.mjs`              | Runtime MCP tool execution                |
| `benchmarks/harvey-labs/stack-version.json`                     | Canonical stack tag (`ts-clawql-data-v2`) |
| `docs/benchmarks/harvey-lab-stack-lineage.md`                     | Legacy quarantine + trace taint matrix    |
| `benchmarks/harvey-labs/harness/adapters/clawql_openrouter.py`  | Anthropic + OpenAI OpenRouter clients     |
| `benchmarks/harvey-labs/scripts/apply_clawql_adapter.py`        | Overlay + `clawql` / `clawql-cc` routing  |
| `benchmarks/harvey-labs/scripts/run-lab-gha.sh`                 | GHA entrypoint (3 arms)                   |
| `.github/workflows/harvey-lab-firm-knowledge.yml`                 | `workflow_dispatch`                       |
| `scripts/start-clawql-for-lab.sh`                                 | Task-scoped MCP + vault                   |
| `benchmarks/harvey-labs/tests/test_openrouter_mapping.py`       | OpenRouter model mapping unit tests       |
