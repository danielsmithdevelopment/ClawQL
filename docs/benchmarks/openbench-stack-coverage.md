...
| `idp-safe-pipeline-lite`         | Stubbed 7-stage IDP (search→dry×2→audit→onyx→notify→ingest) | **Live on `pr_active`** (awaiting WIN)                                                                                                                                                                                       |
...
**CI spend control:** only [`openbench/ci-matrix.json`](../../openbench/ci-matrix.json) → `pr_active` burns tokens on PR/push. Most graded cells above are **`retired`**; **`pr_active`** = `idp-safe-pipeline-lite` (B-2 stub). B-4.2 remains parked offline.
...
| `run_idp_pipeline` / classify / extract | IDP hops via execute           | Unit / runbooks; **OpenBench stub** `idp-safe-pipeline-lite` (live pending) | Live vendor matrix = scheduled B2.3, not PR |
