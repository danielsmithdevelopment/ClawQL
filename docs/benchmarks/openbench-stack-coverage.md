...
| Hybrid `sources` (`vector` / `pageindex` / `onyx` / `codegraph`) | Multi-backend recall                            | Unit + flags; PageIndex path proven via `hybrid-recall-source-pin`; **next:** `memory-recall-pageindex-pin` (`memory_recall({sources:["pageindex"]})`)                                                                       | Live A/B pending                                                                         |
