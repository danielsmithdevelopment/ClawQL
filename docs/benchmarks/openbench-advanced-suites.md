...
| **B — Moat / press** | Fine-tuned small + ClawQL ≥ frontier bare on *ClawQL-shaped* or *domain* tasks | Investors / press | Needs fine-tune + sometimes frontier BYOK |
...
{
  "conflict": true,
  "values": [{"price": 42, "asOf": "2026-01-01"}, {"price": 55, "asOf": "2026-06-01"}],
  "chosen": null,
...
### B-3.1 — Long-horizon codegraph (SWE-bench *lite*, not full SWE-bench yet)
...
| B3.1-f | *(Later)* Expand fixture toward SWE-bench medium difficulty | L | Separate epic |
...
**Product check first:** MCP `cache` is ephemeral KV; inference has `invalidateByTags`. Confirm the *agent-visible* claim (“second read after write is fresh”) matches real behavior.
...
| B2.1-a | Task `idp-safe-pipeline-lite`: agent must complete ordered stages; score stages_passed/N | L | Extend composed-safe-rollout |
| B2.1-b | Artifacts: `pipeline.json` + trail with correlation_id | M | |
...
3b. **[Trace-1]** Publish-ready corpus: OpenBenchTrace v1 schema, write-time scrub, WORM manifests, fail-loud R2 (`raw/` + `manifests/`). ✅ [`openbench-trace-collection.md`](./openbench-trace-collection.md) — **set `CLAWQL_R2_TRACES_BUCKET` (+ S3 keys) or live jobs fail**  
