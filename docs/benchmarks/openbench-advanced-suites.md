...
| B2.3   | Scheduled **integration** job with real services (not `pr_active`)                                                                                   | XL   | ✅ Ordered compose: Nextcloud→Tika→Gotenberg→Stirling→Paperless→Nextcloud; Docling/Onyx/ConeShare need secrets ([runbook](../runbooks/idp-pipeline-b23-smoke.md)) |
...
**B2.3 status:** Ordered compose smoke includes Nextcloud, Tika, Gotenberg, Stirling, Paperless. Docling optional (large image). Onyx + ConeShare require external secrets.
