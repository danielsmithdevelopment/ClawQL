...
Use a **manual** workflow to spin up an ephemeral runner, compare the same model
with and without ClawQL, post a Step Summary, upload JSON, then tear down.
...
4. Wait for the green/red conclusion. Open the run → **Summary** for the table.
...
export CLAWQL_OPENBENCH=1 CLAWQL_HARNESS_ALLOW_UNSANDBOXED=1 IS_SANDBOX=1
...
gh run watch   # or: gh run list --workflow=openbench-ab.yml
```
...
|-----|-------|-------------|
| **clawql-on** | `clawql claude --non-interactive` (MCP pre-wired) | Seeded into a temp Obsidian vault, then **removed** from the workspace |
| **clawql-off** | Raw `claude -p --bare --output-format json` | Seed removed; **no** vault env — agent only sees the misleading bcrypt comment |
...
- Each trial × arm spends real Anthropic tokens. Start with **1 trial**.
- The workflow is **manual only** (no `push` / `schedule` trigger).
- Job timeout is 120 minutes; per-cell timeout defaults to 300s.
- Artifact retention is 14 days, then GitHub deletes it (spin-down of stored results).
