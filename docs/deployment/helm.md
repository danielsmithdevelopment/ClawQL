...
**Default posture:** **`secretSourcing.requireVaultBackedSecrets`** is **`true`** in **`values.yaml`**. Set **`false`** only in lab overlays that accept non-Vault env sourcing (not recommended for production). When **`true`**, Helm must receive **`envFromSecret`** / **`envFromSecrets`** referencing _existing_ Kubernetes Secrets (normally synced from Vault — see **`external-secrets-operator-install.md`**).
...
| `providers.pack`                                                  | Opt-in provider bundle. Chart default is **`none`** (empty catalog, npm parity) — set **`default`** for the opinionated stack, **`all-providers`** for full IDP merge, or a single vendor id (e.g. **`google`**). Injects **`CLAWQL_PROVIDER`**. For custom id lists, set container env **`CLAWQL_BUNDLED_PROVIDERS`**; `provider` (top-level) is a legacy alias for `providers.pack`.                                                                                             |
...
| `enableDocuments`                                                 | When **`false`**, sets **`CLAWQL_ENABLE_DOCUMENTS=0`** (omits **docling**, **tika**, **gotenberg**, **paperless**, **stirling**, **onyx**, **nextcloud**, **coneshare** from **`all-providers`**; hides document MCP tools). Default **`true`** — [mcp-tools.md](../mcp/mcp-tools.md)                                                                                                                                                                                              |
| `enablePdfInspector`                                              | When **`true`**, sets **`CLAWQL_ENABLE_PDF_INSPECTOR=1`** (MCP **`inspect_pdf`** — in-process Firecrawl pdf-inspector; no sidecar). Default **`false`** — [pdf-inspector-onboarding.md](../providers/pdf-inspector-onboarding.md)                                                                                                                                                                                                                                                  |
| `enableAnydoc`                                                    | When **`true`**, sets **`CLAWQL_ENABLE_ANYDOC=1`** (MCP **`convert_document`** — in-process Firecrawl anydoc; no sidecar). Default **`false`** — [anydoc-onboarding.md](../providers/anydoc-onboarding.md)                                                                                                                                                                                                                                                                         |
| `enableIdpClassifier` / `enableLangextract` / `enableIdpPipeline` | When **`true`**, set **`CLAWQL_ENABLE_IDP_CLASSIFIER=1`**, **`CLAWQL_ENABLE_LANGEXTRACT=1`**, **`CLAWQL_ENABLE_IDP_PIPELINE=1`**. Default **`false`**. Pair with **`documentPipeline.classifier`** / **`documentPipeline.langextract`** for sidecars.                                                                                                                                                                                                                              |
...
| `secretSourcing.requireVaultBackedSecrets`                        | **`true`** (default). Set **`false`** only in lab overlays. When **`true`**, requires **`envFromSecret`** / **`envFromSecrets`**.                                                                                                                                                                                                                                                                                                                                                  |
