...
- The MCP **`cache`** tool is **ClawQL Core** (always registered) and is **per-pod**: each replica has its **own** empty in-process store (LRU). Durable cross-replica state belongs in the vault (**`memory_ingest`** / **`memory_recall`**) or your own backing service — see **[cache-tool.md](cache-tool.md)**.
- The MCP **`audit`** tool is **ClawQL Core** (with **`search`** / **`execute`**, no opt-out) and **per-pod**: each replica has its **own** in-process ring buffer — not on disk; use **`memory_ingest`** for durable trails — see **[enterprise-mcp-tools.md](enterprise-mcp-tools.md)** ([#89](https://github.com/danielsmithdevelopment/ClawQL/issues/89)).
