...
- **Docker Desktop / Rancher (Helm `values-docker-desktop.yaml`):** **Ingress** **`http://clawql-mcp.localhost/mcp`** (same hostname + **`/mcp`** pattern as production; **TLS** in prod only). **`svc/clawql-mcp-http`** remains **LoadBalancer** for plaintext gRPC **`<EXTERNAL-IP>:50051`** (or **`localhost:50051`** via the Istio gateway when installed). Use **`kubectl get svc`** for **`EXTERNAL-IP`** when you need the Service URL directly.
...
**Service meshes:** for **remote** clusters, route the **`grpc`** port to the workload and apply your mesh’s mTLS and policies (mesh identity is **orthogonal** to application **`GRPC_TLS_*`**). For **local desktop** clusters, an **optional** Istio path (**`CLAWQL_LOCAL_K8S_ISTIO=ambient`** default, or **`sidecar`**) installs Helm-managed Istio + **`istio/gateway`** (**`clawql-mcp-ingress`**) + **Istio `Gateway` / `VirtualService`** for MCP, sample **Prometheus** / **Kiali**, enrolls **`clawql`** and **`istio-ingress`** in the mesh ( **`ingress-nginx`** only when not auto-skipped — see **`docker/README.md`**), and applies **`PeerAuthentication` `STRICT`** in **`clawql`** by default — see **`scripts/kubernetes/install-istio-docker-desktop.sh`** and **[`docker/README.md`](../../docker/README.md)** (_Optional: Istio + Kiali_). With **ambient** (default), **`local-k8s-docker-desktop.sh`** sets **`CLAWQL_ISTIO_MCP_HTTP_SERVICE_CLUSTERIP=1`** when unset and passes **`--set service.type=ClusterIP`** on **`helm upgrade`** so **`svc/clawql-mcp-http`** stays Helm-owned (no **`kubectl patch`** on **`spec.type`** — avoids server-side apply conflicts). MCP HTTP uses **`http://localhost/mcp`** / **`localhost:50051`** via **`clawql-mcp-ingress`**: on **docker-desktop** / **rancher-desktop** / **docker-for-desktop** kube contexts **`install-istio-docker-desktop.sh`** sets **`svc/clawql-mcp-ingress`** to **LoadBalancer → localhost** ( **`hostNetwork`** binds inside the Kubernetes VM only on those distros). Set **`CLAWQL_ISTIO_MCP_HTTP_SERVICE_CLUSTERIP=0`** for a direct **LoadBalancer :8080** on **`clawql-mcp-http`**. Smoke **gRPC**: **`bash scripts/kubernetes/smoke-grpcurl-istio-gateway-mcp.sh`**.
...
- The MCP **`cache`** tool is **ClawQL Core** (always registered) and is **per-pod**: each replica has its **own** empty in-process store (LRU). Durable cross-replica state belongs in the vault (**`memory_ingest`** / **`memory_recall`**) or your own backing service — see **[cache-tool.md](../mcp/cache-tool.md)**.
- The MCP **`audit`** tool is **ClawQL Core** (with **`search`** / **`execute`**, no opt-out) and **per-pod**: each replica has its **own** in-process ring buffer — not on disk; use **`memory_ingest`** for durable trails — see **[enterprise-mcp-tools.md](../mcp/enterprise-mcp-tools.md)** ([#89](https://github.com/danielsmithdevelopment/ClawQL/issues/89)).
- With **`CLAWQL_ENABLE_NOTIFY`**, the MCP **`notify`** tool posts to Slack from each pod that has a valid bot token and a loaded spec that includes **`slack`** — see **[notify-tool.md](../mcp/notify-tool.md)** and **[mcp-tools.md](../mcp/mcp-tools.md#notify-optional)** ([#77](https://github.com/danielsmithdevelopment/ClawQL/issues/77)).
