...
**Service meshes:** for **remote** clusters, route the **`grpc`** port to the workload and apply your mesh’s mTLS and policies (mesh identity is **orthogonal** to application **`GRPC_TLS_*`**). For **local desktop** clusters, an **optional** Istio path (**`CLAWQL_LOCAL_K8S_ISTIO=ambient`** default, or **`sidecar`**) installs Helm-managed Istio + **`istio/gateway`** (**`clawql-mcp-ingress`**) + **Istio `Gateway` / `VirtualService`** for MCP, sample **Prometheus** / **Kiali**, enrolls **`clawql`**, **`istio-ingress`**, and (by default) **`ingress-nginx`** in the mesh, and applies **`PeerAuthentication` `STRICT`** in **`clawql`** by default — see **`scripts/kubernetes/install-istio-docker-desktop.sh`** and **[`docker/README.md`](../../docker/README.md)** (_Optional: Istio + Kiali_). With **ambient** (default), **`local-k8s-docker-desktop.sh`** sets **`CLAWQL_ISTIO_MCP_HTTP_SERVICE_CLUSTERIP=1`** when unset, patching **`svc/clawql-mcp-http`** to **`ClusterIP`** so MCP HTTP uses **`http://localhost/mcp`** / **`localhost:50051`** via **`clawql-mcp-ingress`**; set **`CLAWQL_ISTIO_MCP_HTTP_SERVICE_CLUSTERIP=0`** to keep a direct **LoadBalancer :8080** on **`clawql-mcp-http`**. Smoke **gRPC**: **`bash scripts/kubernetes/smoke-grpcurl-istio-gateway-mcp.sh`**.
