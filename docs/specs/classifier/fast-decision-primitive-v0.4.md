...
### 7.2b Routing closeout (search_provider_tool_routing, v0.4)

`search_provider_tool_routing` is **productionTrusted** on stock GLiNER 2.5 with reject rule (T=4, τ=0.70): frozen n=40, fire 18/40, 0 errors among fires, Clopper–Pearson 95% two-sided LB **81.5%** (cited ≈82%). GLiNER2.5-Decide was measured on the same frozen 40 (forced EM 92.5%; reject arm 34/40 with 1 error) and is **not** the live path. The instinct to put the same confidence gate on Decide is correct and is the point of **v0.5**; do not hot-swap tonight because of the one miss and because `productionTrusted` does not transfer across different (T,τ) without a new fit → freeze → score-once → adjudicate cycle. Public closeout: [`fast-decision-routing-closeout-v0.4.md`](./fast-decision-routing-closeout-v0.4.md) · [docs.clawql.com](https://docs.clawql.com/specs/classifier/fast-decision-routing-closeout).
...
| §7 harness                                | `validation.ts` + `held-out/` (`runHeldOutValidationSuite`; `productionTrusted` requires frontier adjudication)                                                                             |
| GLiNER sidecar                            | [`infra/gliner-sidecar/`](../../../infra/gliner-sidecar/) — `POST /v1/fast-decision/classify` (mock default)                                                                                |
