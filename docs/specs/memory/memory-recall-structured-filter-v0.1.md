...
Companion: [ClawQL Ontology Legal Domain Spec v0.1](../ontology/legal-domain-v0.1.md) · [Enterprise Ontology](../../architecture/enterprise-ontology.md) · [OpenBench B-7](../../benchmarks/openbench-b7-calderwood.md)
...
```typescript
async function ontologyQuery(params: MemoryRecallParams): Promise<RecallResult> {
...
function buildWhereClause(
  ...
  for (const [field, predicate] of Object.entries(filters)) {
    ...
    if ("eq" in predicate) {
      ...
      const sub = predicate.and.map((p) => buildWhereClause({ [field]: p }, undefined));
      clauses.push(`(${sub.map((s) => s.where).join(" AND ")})`);
      sub.forEach((s) => values.push(...s.values));
    } else if ("or" in predicate) {
      const sub = predicate.or.map((p) => buildWhereClause({ [field]: p }, undefined));
  ...
  if (confidenceMinimum) {
    ...
    clauses.push(`(fc.confidence IS NULL OR fc.confidence IN (${placeholders}))`);
...
function confidenceLevelsAtOrAbove(minimum: ConfidenceLevel): ConfidenceLevel[] {
...
ontology.db — see [meta-ontology-v0.1](../ontology/meta-ontology-v0.1.md).
...
describe("memory_recall structured filter", () => {
  ...
  test("O(1) query time independent of vault size", async () => {
    // Seed 12 matters
    const t1 = Date.now();
    await memoryRecall({ query: "", schema: "legal.Matter", filters: { escrowPct: { gte: 10 } } });
