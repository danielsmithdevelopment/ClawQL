...
1. Batch script `outbound-x402-pay` with hooks on every inner `execute`.
2. `X402Signer` as a `SecretStore` service (`clawql-auth` / `clawql-payments`).
...
5. One e2e against a public x402 fixture.
