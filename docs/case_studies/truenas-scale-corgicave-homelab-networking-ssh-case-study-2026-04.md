**Source:** This page and [`docs/case_studies/truenas-scale-corgicave-homelab-networking-ssh-case-study-2026-04.md`](https://github.com/danielsmithdevelopment/ClawQL/blob/main/docs/case_studies/truenas-scale-corgicave-homelab-networking-ssh-case-study-2026-04.md) are kept in sync; edit both when the narrative changes.
...
| **Thunderbolt** | Local console to NAS | TB to monitor/dock provided the terminal path for `systemctl` / reboots until TB video dropped after input switching |
...
2. **Tunnel / Network Extension bloat:** `netstat -rn` showed many `default` routes via `fe80::…%utun0`–`utun15`. Docker Desktop + Kubernetes were prime suspects. Any filter/VPN-like extension fits the pattern.
...
| Output | Informed |
|---|---|
| `EADDRNOTAVAIL` on `curl` despite `route get` → `en0` | Suspect source address / policy / tunnels, not NAS connectivity |
...
|---|---|---|---|---|
| 1 | User (NAS) | Web UI fails; restart `nginx`, `middlewared`; NAS reboot | UI still broken | Verify L3/L4 from a correct client before further service restarts |
...
| 9 | User | Describe office island (no router uplink) | — | No DHCP discovery from router; static IP discipline required |
...
| 13 | Mac mini | `curl --interface en0`, `nc` :443 / :80 | :443 succeeded once; :80 `EADDRNOTAVAIL`; `curl` still bad | Client stack + port-specific behavior; not pure nginx down |
