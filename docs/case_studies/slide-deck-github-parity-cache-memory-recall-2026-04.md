...
For when **`cache` is** vs is **not** the right tool, see **[`docs/mcp/cache-tool.md`](../mcp/cache-tool.md)**. **`cache`** is **in-process** and **LRU-bounded** — it is the right place for “temporary issue numbers and information we do **not** want to persist to long-term memory” for as long as the MCP server process (and the session) lasts.
...
**Parameters used (illustrative):** `query` = short keyword phrase, **`limit`** small (e.g. 5–8), optional **`maxDepth`** when graph follow-through matters (wikilinks). See [`docs/mcp/mcp-tools.md`](../mcp/mcp-tools.md) under **`memory_recall`**.
...
**Important:** `cache` is **ephemeral in this MCP server process** (see [`docs/mcp/cache-tool.md`](../mcp/cache-tool.md)). A restart, a different process, or another client does not see these keys. That is the intended behavior: **scratch**, not a substitute for **`memory_ingest`**.
...
| **`audit`**         | In-process **operator** trail of MCP tool calls (optional)             | Not the focus; see [`docs/mcp/enterprise-mcp-tools.md`](../mcp/enterprise-mcp-tools.md) and [#89](https://github.com/danielsmithdevelopment/ClawQL/issues/89). |
...
- [`docs/mcp/cache-tool.md`](../mcp/cache-tool.md) — **cache** tool semantics
- [`docs/mcp/mcp-tools.md`](../mcp/mcp-tools.md) — **memory_ingest**, **memory_recall**
