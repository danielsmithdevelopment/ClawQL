...
**Related:** [`clawql-durable-objects.md`](./clawql-durable-objects.md) · [`mcp-api-adapter`](../mcp/mcp-api-adapter.md) · [defense-in-depth](../security/clawql-security-defense-in-depth.md) · [codegraph](../plugins/codegraph.md)
...
clawql-cellrt/
  ...
  crates/
    cellrt-coordinator/         — fleet coordination (S3, LTX, peer auth)
      src/
        bucket.rs               — S3/R2 client (aws-sdk-rust)
        lease.rs                — compare-and-swap ownership leases
        ltx.rs                  — LTX segment writer/reader
        peer.rs                 — peer discovery + HMAC auth
        node.rs                 — node lifecycle

    cellrt-runtime/             — per-cell execution
      src/
        cell.rs                 — Cell struct (Tokio task, lifecycle)
        wasm.rs                 — Wasmtime engine + component instantiation
        http.rs                 — axum HTTP handler per cell
        websocket.rs            — tokio-tungstenite WebSocket handler
        alarm.rs                — durable alarm scheduler
        capabilities.rs         — WIT capability grant/revoke

    cellrt-storage/             — SQLite + LTX per cell
      src/
        db.rs                   — rusqlite WAL connection
        ltx_writer.rs           — WAL segment streaming to bucket
        worm.rs                 — WORM write guarantees (ack after replication)
        schema.rs               — SQLite schema migrations

    cellrt-security/            — clawql-security embedded
      src/
        ebpf.rs                 — libbpf-rs program loader
        rules.rs                — Falco-equivalent rules engine
        capability_enforcer.rs  — WASM WIT import validation
        virtual_key.rs          — virtual key struct + lifecycle
        attestation.rs          — Cosign binary verification (sigstore-rs)

    cellrt-inference/           — clawql-inference embedded
      src/
        pal.rs                  — PAL router (policy.yaml parsing)
        virtual_key_manager.rs  — issue/expire/rotate virtual keys
        budget.rs               — token budget enforcement
        call_store.rs           — WORM inference call log

    cellrt-vault/               — HashiCorp Vault client
      src/
        auth.rs                 — AppRole authentication
        lease.rs                — dynamic secret lease lifecycle
        inject.rs               — secret injection into WASM env

    cellrt-observability/       — OTel + Prometheus + Langfuse
      src/
        otel.rs                 — opentelemetry-rust SDK setup
        prometheus.rs           — metrics endpoint
        langfuse.rs             — RTP/OBT forwarder
        logging.rs              — tracing → JSON → Loki

    cellrt-cli/                 — binary entry point + CLI
      src/
        main.rs
        commands/
          start.rs              — start fleet node
          deploy.rs             — deploy WASM bundle to bucket
          diagnose.rs           — fleet health check
          cell.rs               — manual cell spawn/destroy
```
