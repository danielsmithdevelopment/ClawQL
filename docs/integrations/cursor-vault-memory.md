...
| **`CLAWQL_OBSIDIAN_VAULT_PATH`**    | Points the server at a writable vault directory. Without it, vault tools return an error; see **[README.md](../../README.md)** and **[memory-obsidian.md](../memory/memory-obsidian.md)**. |
...
- **`memory_ingest`** — Writes structured Markdown under **`Memory/`** in the vault (YAML frontmatter, optional **`[[wikilinks]]`**, dedup by content hash). Optional **`toolOutputsFile`**: point at a small path string so the **server** reads a large file from disk (allowlisted via **`CLAWQL_MEMORY_INGEST_FILE_ROOTS`**)—avoids multi‑hundred‑KB tool JSON in Cursor. See **[mcp-tools.md](../mcp/mcp-tools.md)** and implementation notes in **[memory-obsidian.md](../memory/memory-obsidian.md)**.
- **`memory_recall`** — Keyword search, wikilink graph walks, optional vector leg when configured. See **[memory-db-hybrid-implementation.md](../memory/memory-db-hybrid-implementation.md)** for the **`memory.db`** sidecar.
...
| Doc                                                                                    | Topic                                              |
| -------------------------------------------------------------------------------------- | -------------------------------------------------- |
| **[memory-obsidian.md](../memory/memory-obsidian.md)**                                 | Why a vault, `Memory/`, wikilinks, index pages.    |
| **[mcp-tools.md](../mcp/mcp-tools.md)**                                                | All MCP tools, JSON-shaped examples.               |
| **[cache-tool.md](../mcp/cache-tool.md)**                                              | **`cache`** vs vault memory, LRU, env.             |
| **[enterprise-mcp-tools.md](../mcp/enterprise-mcp-tools.md)**                          | Optional **`audit`** vs vault; enterprise roadmap. |
| **[memory-db-hybrid-implementation.md](../memory/memory-db-hybrid-implementation.md)** | `memory.db`, recall implementation.                |
