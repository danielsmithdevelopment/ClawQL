...
| **`clawql-documents`**         | `packages/clawql-documents`          | `default`   | 📋 Logic shipped; tools in `src/tools.ts`                                        | `ingest_external_knowledge`                                                                              | Default on; **`CLAWQL_ENABLE_DOCUMENTS=0`** to hide                                                         | Ingest + **`DEFAULT_IDP_PIPELINE`**; 7 bundled IDP vendors via **`execute`** — [idp-pipeline.md](../providers/idp-pipeline.md); automated runner roadmap |
...
Today **`PanguardProxyPlugin`** and **`MemoryPlugin`** are composed in `createClawQLApi()`. Other horizontal packages are still imported by `src/tools.ts` directly.
