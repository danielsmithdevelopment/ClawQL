...
Today **`PanguardProxyPlugin`**, **`MemoryPlugin`**, and **`DocumentsPlugin`** are composed at startup (`buildMcpPlugins` in `clawql-api-adapters.ts`). Automation and other optional tools are still registered in `src/tools.ts`.
