...
| `workflow`                       | `clawql-automation`       | `clawql-automation`         | [workflow-tool.md](../mcp/workflow-tool.md)                              |
| `argocd`                         | `clawql-automation`       | `clawql-automation`         | [argocd-tool.md](../mcp/argocd-tool.md)                                  |
...
Today **`PanguardProxyPlugin`**, **`MemoryPlugin`**, **`DocumentsPlugin`**, **`AutomationPlugin`**, **`SandboxPlugin`**, and **`OuroborosPlugin`** are composed at startup (`buildMcpPlugins` in `clawql-api-adapters.ts`). Remaining optional tools (HITL) are still registered in `src/tools.ts`.
