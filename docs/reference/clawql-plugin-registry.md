...
| **`clawql-documents`**         | `packages/clawql-documents`          | `default`   | ✅ **`DocumentsPlugin`** — `onRegister` registers ingest + optional Onyx / IDP runner | `ingest_external_knowledge`, `knowledge_search_onyx` (when Onyx on), **`run_idp_pipeline`** (when **`CLAWQL_ENABLE_IDP_PIPELINE=1`**) | Default on; **`CLAWQL_ENABLE_DOCUMENTS=0`** to omit; Onyx: **`CLAWQL_ENABLE_ONYX=1`**; runner: **`CLAWQL_ENABLE_IDP_PIPELINE=1`** | Ingest + **`DEFAULT_IDP_PIPELINE`**; bundled IDP vendors via **`execute`** — [idp-pipeline.md](../providers/idp-pipeline.md), [idp-pipeline-runner.md](../mcp/idp-pipeline-runner.md) ([#307](https://github.com/danielsmithdevelopment/ClawQL/issues/307)) |
