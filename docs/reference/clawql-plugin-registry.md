...
| **`clawql-automation`**        | `packages/clawql-automation`         | `default`   | ✅ **`AutomationPlugin`** — schedule/notify/workflow via `onRegister`            | `schedule`, `notify`, `workflow` (opt-in)                                                                | **`CLAWQL_ENABLE_SCHEDULE=1`**, **`CLAWQL_ENABLE_NOTIFY=1`**, **`CLAWQL_ENABLE_WORKFLOW=1`**                | Argo `workflow` — [design](../design/workflow-tool-argo.md), [operator guide](../mcp/workflow-tool.md), [#243](https://github.com/danielsmithdevelopment/ClawQL/issues/243) |
