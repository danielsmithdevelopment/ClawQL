...
| **`clawql-automation`**        | `packages/clawql-automation`         | `default`   | ✅ **`AutomationPlugin`** — `onRegister` registers schedule/notify; worker in `onRegister` | `schedule`, `notify` (+ **`workflow`** planned)                                                          | **`CLAWQL_ENABLE_SCHEDULE=1`**, **`CLAWQL_ENABLE_NOTIFY=1`**, **`CLAWQL_ENABLE_WORKFLOW=1`** (planned)      | Schedule worker + Slack notify via execute. **`workflow`:** template-only Argo submit, `@kubernetes/client-node`, Argo ≥ 3.4.0 — [design](../design/workflow-tool-argo.md), [#243](https://github.com/danielsmithdevelopment/ClawQL/issues/243) |
