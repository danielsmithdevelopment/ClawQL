...
The **ClawQL dashboard** (`apps/dashboard/` Next.js app) includes an **Agent Chat** panel that proxies to the same OpenClaw agent via an HTTP bridge — useful for operators who prefer a browser UI over the OpenClaw app.

| Piece                 | Location                                                                         |
| --------------------- | -------------------------------------------------------------------------------- |
| **Full reference**    | **[`docs/dashboard/agent-chat.md`](../dashboard/agent-chat.md)**                 |
| **Local bridge**      | `cd apps/dashboard && npm run openclaw:chat-bridge` → `http://127.0.0.1:8787/v1/chat` |
...
| **Cursor: MCP server failed during tool discovery** | (1) Non-JSON on **stdout** — `dotenv` ≥17 needs **`quiet: true`** in `src/load-env.ts`. (2) Slow Ready — stdio now connects **before** warming the default OpenAPI stack; rebuild/`dist/server.js` must include that change. Restart the MCP server / agent session after upgrading. |
