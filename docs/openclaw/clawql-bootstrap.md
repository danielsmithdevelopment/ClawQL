...
| ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Core **`search` / `execute`**                                                                                 | Always on; provider catalog is **empty by default** — opt in with **`CLAWQL_PROVIDER=default`** (opinionated stack) or **`CLAWQL_PROVIDER=all-providers`** (every bundled vendor)                                             |
...
| Onyx **`knowledge_search_onyx`**                                                                              | **`CLAWQL_ENABLE_ONYX=1`** and documents still enabled; set **`ONYX_BASE_URL`** + auth per **[`docs/mcp/onyx-knowledge-tool.md`](../mcp/onyx-knowledge-tool.md)**                                                              |
...
| HITL / Label Studio ([#228](https://github.com/danielsmithdevelopment/ClawQL/issues/228))                     | **`CLAWQL_ENABLE_HITL_LABEL_STUDIO=1`**, **`CLAWQL_LABEL_STUDIO_URL`**, **`CLAWQL_LABEL_STUDIO_API_TOKEN`**, webhook **`CLAWQL_HITL_WEBHOOK_TOKEN`** — see **[`docs/mcp/hitl-label-studio.md`](../mcp/hitl-label-studio.md)**. |

**Privacy / redaction:** Opt-in gateway layers: **Presidio** (`CLAWQL_ENABLE_PRESIDIO=1`) and local **Privacy Filter** backup (`CLAWQL_ENABLE_PRIVACY_FILTER=1`, [#245](https://github.com/danielsmithdevelopment/ClawQL/issues/245)) — see [`docs/security/privacy-filter-local.md`](../security/privacy-filter-local.md). Additional document-stage redaction (Stirling) is separate; validate against **your** chart and **[`docs/mcp/enterprise-mcp-tools.md`](../mcp/enterprise-mcp-tools.md#regulated-deployments)** for regulated environments.
...
| 2    | **Document path:** call **`ingest_external_knowledge`** per **[`docs/mcp/external-ingest.md`](../mcp/external-ingest.md)** (vault path + **`CLAWQL_EXTERNAL_INGEST=1`** as needed).                                                                                                                                                                                                                                                                                                                                                                           | Success payload or documented preview when dry-run.   |
