...
ClawQL is under active development. The horizontal platform through **7.1.0** is shipped; **8.0.0** adds provider opt-in, enforcement defaults, and the last horizontal packages below. See [Getting started](https://docs.clawql.com/getting-started). The table below is the honest current state; everything after it describes where ClawQL is going and why.
