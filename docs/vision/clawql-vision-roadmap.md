...
ClawQL is under active development. **`clawql-mcp@8.0.0`** on `main` is the current contract: **empty-by-default** provider catalog, **`ProviderPlugin` only** (Phase-2 `onRegister` removed), and **enforcement opt-in**. See [Migrate to 8.0](../getting-started/migrate-to-8.0.md) and [Getting started](https://docs.clawql.com/getting-started). The table below is the honest current state; everything after it describes where ClawQL is going and why.

| Package                    | Status                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clawql-mcp`               | ✅ Shipped (transport + MCP registration in `src/`)                                                                                                                                                                                                                                                                                                                                                                          |
| `clawql-core`              | ✅ Shipped (audit, Merkle, Cuckoo, **`ProviderPlugin`** / skills / hooks)                                                                                                                                                                                                                                                                                                                                                    |
...
| `clawql-data`              | ✅ Shipped — Node DuckDB MCP `data_query` / `data_ingest` (`CLAWQL_ENABLE_DATA=1`)                                                                                                                                                                                                                                                                                                                                           |
| `clawql-network`           | ✅ Shipped (8.0 prep) — Headscale mesh + Tailcat ephemeral transport, selector, ATR-gated audit; `clawql init --networking` ([spec](https://docs.clawql.com/specs/network/clawql-network))                                                                                                                                                                                                                                   |
| `clawql-agents`            | ✅ Shipped (8.0 prep) — seven agent adapters (OpenClaw, Hermes, Pi, Goose, DeepSeek Harness, OpenHands, Cline), ATR templates, Helm overlays ([spec](https://docs.clawql.com/agents/clawql-agents))                                                                                                                                                                                                                          |
| `clawql-audit`             | ✅ Shipped (8.0 prep) — durable WORM trail (SQLite/Postgres/S3), Merkle chain, replication hooks; complements in-process `audit` in `clawql-core` ([`/audit`](https://docs.clawql.com/audit))                                                                                                                                                                                                                                |
...
## 4. What Exists Today (7.1.0 + 8.0 prep on `main`)
...
**`mcp-api-adapter`** — Point at any MCP server (stdio / Streamable HTTP / gRPC) and serve **seven surfaces**: OpenAPI, GraphQL, Streamable HTTP `/mcp`, gRPC, WebSocket `/ws`, gen-cli, and HTMX **`/mcp-ui`**. QR stream transport is the planned eighth surface. Guide: [`docs/mcp/mcp-api-adapter.md`](https://docs.clawql.com/mcp/mcp-api-adapter). `/mcp-ui`: [`docs/mcp/mcp-ui.md`](https://docs.clawql.com/mcp/mcp-ui).

**`clawql-network`** — Headscale standing mesh + Tailcat ephemeral transport, safe-by-default selector, ATR-gated tailcat audit hooks. Bootstrap: `clawql init --networking`. Spec: [`docs/specs/network/clawql-network-v0.1.md`](https://docs.clawql.com/specs/network/clawql-network).

**`clawql-agents`** — Hardened adapters for seven open-source agents (Panguard, WORM hooks, vault memory, Helm overlays, dry OpenBench runner). Spec: [`docs/agents/clawql-agents-spec-v0.1.md`](https://docs.clawql.com/agents/clawql-agents).

**`clawql-audit`** — Durable WORM audit trail package (Merkle chain, SQLite/Postgres/S3 backends, replication). Distinct from the in-process MCP `audit` ring buffer in gateway core. Docs: [`/audit`](https://docs.clawql.com/audit).
