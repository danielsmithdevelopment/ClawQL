...
**This document is the new canonical north star.** It merges the modular platform vision with a production-ready intelligent MCP gateway architecture that makes ClawQL the single unified front-end for the entire MCP ecosystem.

**How to read this:** Product, engineering, platform, and operator teams should treat this as the authoritative target architecture. Many capabilities described here are already partially shipped; others are now explicitly prioritized for v2.0.

**Canonical Markdown:** docs/vision/clawql-modularization-v2.md (this file).

---

## Document scope & evidence (read this first)

This document is the **v2.0 north star**: modular platform plus **intelligent MCP gateway** (single front-end for native specs, verticals, and proxy MCP backends). It mixes **shipped today**, **partial**, and **planned** work in one narrative.

**Where to ground claims in Git:**

- **Ecosystem and shipped surface:** [`docs/clawql-ecosystem.md`](../clawql-ecosystem.md)
- **v1.9 package vision (companion):** [`clawql-modularization.md`](./clawql-modularization.md)
...
**v2.0 Architectural Shift (the new canonical direction):**  
ClawQL no longer competes as “yet another MCP server.” It becomes the **intelligent front-end gateway** for the entire MCP ecosystem. Every downstream MCP backend (native verticals, external servers, OpenAPI services, gRPC endpoints, GraphQL subgraphs, Fabric gateways, etc.) registers as a plugin inside clawql-api. Agents and LLMs interact with **exactly one** high-level tool:

```ts

{
  name: "clawql_execute",
  description: "Execute any task using the unified ClawQL platform (memory, documents, sandbox, automation, multi-backend orchestration, vertical workflows, etc.)",
  parameters: {
...
All intelligence lives in ClawQL: ATR enforcement, Memory 2.0 enrichment, intelligent routing, GraphQL projection, Merkle auditing, and Presidio redaction happen **before** the call reaches any downstream backend. This delivers 50–85% token reduction, deterministic performance, centralized security, and zero reconfiguration when new backends are added.
...
**High-Level Tool Surface:** The single `clawql_execute` tool exposed to all downstream LLMs/agents.
...
| -------------------- | -------------------------- | --------------------------------------- |
| Tools seen by LLM    | Dozens–hundreds of schemas | 1 high-level tool                       |
