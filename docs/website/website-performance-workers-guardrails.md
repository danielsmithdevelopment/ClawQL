...
- [ ] **OpenNext static cache:** **`staticAssetsIncrementalCache`** + **`enableCacheInterception`** in **`apps/docs/open-next.config.ts`**; root **`force-static`** in **`apps/docs/src/app/layout.tsx`**.
- [ ] **Cache strategy:** keep **`Cache-Control`** sensible (`apps/docs/next.config.mjs` + `public/_headers` + **`apps/docs/src/lib/edge-cache-control.mjs`**); heavy MDX routes use **30d `s-maxage`** — avoid unnecessary **purge-all** after deploy (purge only when you need immediate HTML consistency).
...
- [ ] Ensure markdown images have **meaningful `alt`** (WCAG); default **`loading="lazy"`** is set in **`apps/docs/src/components/mdx.tsx`**.
...
- [ ] **Playwright + axe** (`apps/docs/tests/a11y-axe.spec.ts`, same job as **`npm run test:smoke`** in root CI): blocks **critical** and **serious** axe violations on **`/`**, **`/learn`**, **`/install`** (complements Lighthouse). Contrast: **Shiki** comment token in **`apps/docs/src/styles/tailwind.css`**, search shortcut **kbd** colors in **`apps/docs/src/components/Search.tsx`**.
- [ ] **Skip link** and **`#main-content`** target (see `apps/docs/src/app/layout.tsx`, `Layout.tsx`).
...
- [ ] **Sitemap** stays **static** (`export const dynamic = 'force-static'` in `apps/docs/src/app/sitemap.ts`) — avoid fake **`lastModified: new Date()`** on every build.
- [ ] **Canonical + `docsPageMetadata`** per page (`apps/docs/src/lib/seo.ts`).
...
- [ ] **`Referrer-Policy`** and **`X-Content-Type-Options`** on HTML routes (`apps/docs/next.config.mjs`).
...
| **Synthetic checks** for `/`, `/api/health`, and **one case-study URL** | Early warning when **1102** or **5xx** spikes; see [`schedule-synthetic-checks.md`](../mcp/schedule-synthetic-checks.md).                     |
...
| **`eslint-plugin-jsx-a11y`** in `apps/docs/`                              | Catches **a11y** issues Lighthouse does not (e.g. some interactive patterns).                                                                 |
...
- **Website package scripts:** [`apps/docs/README.md`](../apps/docs/README.md)
