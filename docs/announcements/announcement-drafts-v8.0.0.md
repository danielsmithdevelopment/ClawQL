...
9. **Streams celld Lab 5b** — `streams-slim` in-cell + MCP/adapter `fetch` + audit LTX (not embedding full `clawql-api`).
...
Headlines: **available ≠ loaded** providers · **`ProviderPlugin` hard break** · enforcement **opt-in** · skills-unified search · LGTM+/Faro observability · `clawql-network` + analytics · Streams celld Lab 5b · Learn/migrate-to-8 docs · Managed Edge Gateway.
