...
This chart installs the **ClawQL Operator scaffold** — it does **not** replace `manifests/charts/clawql-mcp`. Existing Helm, Compose, and `npx clawql-mcp` workflows are unchanged.
...
```bash
# Chart installs the ClawQLInstance CRD when crd.install is true (default).
helm upgrade --install clawql-operator ./manifests/charts/clawql-operator \
  --namespace clawql-system \
  --create-namespace
kubectl apply -f docs/examples/operator/clawqlinstance-minimal.yaml
```
