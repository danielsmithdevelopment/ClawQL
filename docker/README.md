...
- **MCP URL:** default **`make local-k8s-up` + Istio** uses **`http://localhost/mcp`** (primary) and **`http://clawql-mcp.localhost/mcp`** (same VirtualService backends). **`http://clawql.localhost/mcp`** also works: **`clawql-localhost-vs-core.yaml`** sends **`/mcp`**, **`/graphql`**, and **`/healthz`** to **`clawql-mcp-http:8080`** before the docs UI catch-all (without that split, Cursor sees **404** on SSE fallback because **`/`** is the Next.js site). **`CLAWQL_LOCAL_K8S_ISTIO=0`** falls back to **ingress-nginx** + **`values-docker-desktop`** Ingress manifests. **Compose:** **`http://localhost:8080/mcp`**.
